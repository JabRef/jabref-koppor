...
- **Ecosystem fit**: SQLite is the storage layer of Zotero (`zotero.sqlite` "the database containing the majority of your data", [Zotero docs](https://www.zotero.org/support/zotero_data)) and was the SQL candidate shortlisted in JabRef's own ADR-0010. JDBC means the team's existing SQL code, tests (`SearchQuerySQLConversionTest`), and habits transfer; no new query paradigm to learn (unlike Lucene analyzers).

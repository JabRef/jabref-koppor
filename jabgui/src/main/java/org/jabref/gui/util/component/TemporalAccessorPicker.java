 ...
 * via the text field and the date can additionally be changed via the JavaFX default date
 * picker. Also supports incomplete dates.
 * <p>
 * First recall how the date picker normally works: - The user selects a date in the
 * popup, which sets {@link #valueProperty()} to the selected date. - The converter
 * ({@link #converterProperty()}) is used to transform the date to a string representation
 * and display it in the text field.
 * <p>
 * The idea is now to intercept the process and add an additional step: - The user selects
 * a date in the popup, which sets {@link #valueProperty()} to the selected date. - The
 * date is converted to a {@link TemporalAccessor} (i.e, enriched by a time component)
 * using {@link #addCurrentTime(LocalDate)} - The string converter
 * ({@link #stringConverterProperty()}) is used to transform the temporal accessor to a
 * string representation and display it in the text field.
 * <p>
 * Inspiration taken from <a href=
 * "https://github.com/edvin/tornadofx-controls/blob/master/src/main/java/tornadofx/control/DateTimePicker.java">Controlsfx
 * DateTimePicker</a>
 */
public class TemporalAccessorPicker extends DatePicker {
    ...
    public TemporalAccessorPicker() {
        ...
        BindingsHelper.bindBidirectional(valueProperty(), temporalAccessorValue, TemporalAccessorPicker::addCurrentTime,

...
public class FilteredListProxy {
    ...
    public static void refilterListReflection(
        FilteredList<BibEntryTableViewModel> filteredList
    ) {
        try {
            if (!initialized) {
                initReflection();
    ...
    public static void refilterListReflection(
        ...
        try {
            ...
            ENSURE_SIZE_METHOD.invoke(
                filteredList,
                filteredList.getSource().size()
            );

            @SuppressWarnings("unchecked")
            Predicate<BibEntryTableViewModel> predicateImpl = (Predicate<
                BibEntryTableViewModel
            >) GET_PREDICATE_IMPL_METHOD.invoke(filteredList);
            ListIterator<? extends BibEntryTableViewModel> it = filteredList
                .getSource()
                .listIterator(sourceFrom);
    ...
    private static void initReflection()
        ...
        BEGIN_CHANGE_METHOD = ObservableListBase.class.getDeclaredMethod(
            "beginChange"
        );
        END_CHANGE_METHOD = ObservableListBase.class.getDeclaredMethod(
            "endChange"
        );
        NEXT_ADD_METHOD = ObservableListBase.class.getDeclaredMethod(
            "nextAdd",
            int.class,
            int.class
        );
        NEXT_UPDATE_METHOD = ObservableListBase.class.getDeclaredMethod(
            "nextUpdate",
            int.class
        );
        NEXT_REMOVE_METHOD = ObservableListBase.class.getDeclaredMethod(
            "nextRemove",
            int.class,
            Object.class
        );
        ...
        ENSURE_SIZE_METHOD = FilteredList.class.getDeclaredMethod(
            "ensureSize",
            int.class
        );
        GET_PREDICATE_IMPL_METHOD = FilteredList.class.getDeclaredMethod(
            "getPredicateImpl"
        );

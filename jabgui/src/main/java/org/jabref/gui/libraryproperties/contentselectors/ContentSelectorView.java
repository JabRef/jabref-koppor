...
public class ContentSelectorView
    ...
    private void removeFieldName() {
        getSelectedField().ifPresent(
            viewModel::showRemoveFieldNameConfirmationDialog
        );
    }
    ...
    private Optional<Field> getSelectedField() {
        return Optional.of(fieldsListView.getSelectionModel()).map(
            SelectionModel::getSelectedItem
        );
    }

    private Optional<String> getSelectedKeyword() {
        return Optional.of(keywordsListView.getSelectionModel()).map(
            SelectionModel::getSelectedItem
        );
    }
}

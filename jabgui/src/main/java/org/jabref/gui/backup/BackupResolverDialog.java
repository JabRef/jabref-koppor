...
public class BackupResolverDialog extends FXDialog {
    ...
    public static final ButtonType RESTORE_FROM_BACKUP = new ButtonType(Localization.lang("Restore from backup"),
            ButtonBar.ButtonData.OK_DONE);

    public static final ButtonType REVIEW_BACKUP = new ButtonType(Localization.lang("Review backup"),
            ButtonBar.ButtonData.LEFT);

    public static final ButtonType IGNORE_BACKUP = new ButtonType(Localization.lang("Ignore backup"),
    ...
    public BackupResolverDialog(Path originalPath, Path backupDir,
        ...
        Optional<Path> backupPathOpt = BackupFileUtil.getPathOfLatestExistingBackupFile(originalPath,
                BackupFileType.BACKUP, backupDir);
        String backupFilename = backupPathOpt.map(Path::getFileName)
            .map(Path::toString)
        ...
        String content = Localization.lang("A backup file for '%0' was found at [%1]",
                ...
                + "\n"
                + Localization
                    .lang("This could indicate that JabRef did not shut down cleanly last time the file was used.")
        ...
        contentLabel.setOnAction(e -> {
            if (backupPathOpt.isPresent()) {
                ...
                if (backupFilename.equals(clickedLinkText)) {
                    try {
                        NativeDesktop.openFolderAndSelectFile(backupPathOpt.get(), externalApplicationsPreferences,
                                null);
                    }

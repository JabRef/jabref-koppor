...
public class ClipboardContentGenerator {
    ...
    public ClipboardContent generate(List<BibEntry> selectedEntries, CitationStyleOutputFormat outputFormat,
            BibDatabaseContext bibDatabaseContext) throws IOException {
    ...
    private List<String> generateCitations(List<BibEntry> selectedEntries, CitationStyleOutputFormat outputFormat,
            BibDatabaseContext bibDatabaseContext) throws IOException {
        // This worker stored the style as filename. The CSLAdapter and the
        // CitationStyleCache store the source of the
        ...
        if (styleSource != null) {
            return CitationStyleGenerator.generateBibliography(selectedEntries, styleSource, outputFormat,
    ...
    /**
     * Generates a plain text string out of the preview (based on
     * {@link org.jabref.logic.layout.TextBasedPreviewLayout} or
     * {@link org.jabref.logic.bst.BstPreviewLayout}) and copies it additionally to the
     * html to the clipboard (WYSIWYG Editors use the HTML, plain text editors the text)
     */
    @VisibleForTesting
    ...
    /**
     * Inserts each citation into a HTML body and copies it to the clipboard. The given
     * preview is based on
     * {@link org.jabref.logic.citationstyle.CitationStylePreviewLayout}.
     */
    @VisibleForTesting
    static ClipboardContent processHtml(List<String> citations) {
        String result = "<!DOCTYPE html>" + OS.NEWLINE + "<html>" + OS.NEWLINE + "   <head>" + OS.NEWLINE
    ...
    /**
     * Insert each citation into HTML. convert HTML to markdown using flexmark.
     */
    @VisibleForTesting
    ...
    private List<String> generateTextBasedPreviewLayoutCitations(List<BibEntry> selectedEntries,
            BibDatabaseContext bibDatabaseContext) throws IOException {
        ...
        Layout layout = new LayoutHelper(customLayoutReader, layoutFormatterPreferences, abbreviationRepository)
            .getLayoutFromText();

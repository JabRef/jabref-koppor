...
public class LookupIdentifierAction<T extends Identifier>
    ...
    public LookupIdentifierAction(
        ...
        this.executable.bind(
            needsDatabase(this.stateManager).and(
                needsEntriesSelected(this.stateManager)
            )
        );
    ...
    public void execute() {
        try {
            BackgroundTask.wrap(() ->
                lookupIdentifiers(stateManager.getSelectedEntries())
            )
    ...
    private String lookupIdentifiers(List<BibEntry> bibEntries) {
        ...
        for (BibEntry bibEntry : bibEntries) {
            ...
            UiTaskExecutor.runInJavaFXThread(() ->
                dialogService.notify(statusMessage)
            );
            ...
            if (
                ...
                if (fieldChange.isPresent()) {
                    ...
                    UiTaskExecutor.runInJavaFXThread(() ->
                        dialogService.notify(nextStatusMessage)
                    );
                }
            }
        }

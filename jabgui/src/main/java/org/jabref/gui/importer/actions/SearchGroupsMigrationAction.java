...
public class SearchGroupsMigrationAction implements GUIPostOpenAction {
    ...
    public static final Version VERSION_6_0_ALPHA_1 = Version.parse(
        "6.0-alpha_1"
    );
    ...
    public boolean isActionNecessary(
        ...
        Optional<Version> currentVersion = parserResult
            .getMetaData()
    ...
    public void performAction(
        ...
        parserResult
            .getMetaData()
            .getGroups()
            .ifPresent(groupTreeNode ->
                migrateGroups(groupTreeNode, dialogService)
            );
        parserResult
            .getMetaData()
            .setGroupSearchSyntaxVersion(VERSION_6_0_ALPHA_1);
            ...
            try {
                if (searchGroup.getSearchQuery().isValid()) {
                    String newSearchExpression =
                        SearchQueryConversion.flagsToSearchExpression(
                            searchGroup.getSearchQuery()
                        );
                    searchGroup.setSearchExpression(newSearchExpression);
                } else {
                    showAskForNewSearchExpressionDialog(
                        dialogService,
                        searchGroup
                    );
                }
    ...
    private void showAskForNewSearchExpressionDialog(
        DialogService dialogService,
        SearchGroup searchGroup
    ) {
        Optional<String> newSearchExpression =

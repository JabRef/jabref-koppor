...
public class NewEntryView extends BaseDialog<BibEntry> {
    ...
    public NewEntryView(NewEntryDialogTab initialApproach, GuiPreferences preferences, LibraryTab libraryTab,
    ...
    public void initialize() {
        viewModel = new NewEntryViewModel(guiPreferences, libraryTab, dialogService, stateManager,
                (UiTaskExecutor) taskExecutor, aiService, fileUpdateMonitor);

        visualizer.setDecoration(new IconValidationDecorator());

        EasyBind.subscribe(viewModel.executedSuccessfullyProperty(), succeeded -> {
            if (succeeded) {
    ...
    private void initializeAddEntry() {
        ...
        final BibDatabaseMode customTypesDatabaseMode = isBiblatexMode ? BibDatabaseMode.BIBLATEX
    ...
    private void initializeLookupIdentifier() {
        // TODO: It would be nice if this was a `TextArea`, so that users could enter
        // multiple IDs at once. The view
        // model would then iterate through all non-blank lines, passing each of them
        // through the specified lookup
        ...
        idErrorInvalidFetcher.visibleProperty()
            .bind(idLookupSpecify.selectedProperty().and(viewModel.idFetcherValidatorProperty().not()));
    ...
    private void initializeInterpretCitations() {
        ...
        PlainCitationParserChoice initialParser = parserFromName(preferences.getLatestInterpretParser(),
        ...
        interpretParser
            .setOnAction(_ -> preferences.setLatestInterpretParser(interpretParser.getValue().getLocalizedName()));
    }
    ...
    private void initializeSpecifyBibTeX() {
        ...
        final String clipboardText = ClipBoardManager.getContents().trim();
        if (!StringUtil.isBlank(clipboardText)) {
            // :TODO: Better validation would be nice here, so clipboard text is only
            // copied over if it matches a
    ...
    private void switchLookupIdentifier() {
        ...
        if (generateButton != null) {
            generateButton.disableProperty()
                .bind(idErrorInvalidText.visibleProperty().or(idErrorInvalidFetcher.visibleProperty()));
    ...
    private void execute() {
        // :TODO: These button text changes aren't actually visible, due to the UI thread
        // not being able to perform the
        // update before the button text is reset. The `viewModel.execute*()` and
        // `switch*()` calls could be wrapped in
    ...
    private static String descriptionOfStandardEntryType(StandardEntryType type) {
        // These descriptions are taken from subsection 2.1 of the biblatex package
        // documentation.
        // Biblatex is a superset of bibtex, with more elaborate descriptions, so its
        // documentation is preferred.
        // See
        // [https://mirrors.ibiblio.org/pub/mirrors/CTAN/macros/latex/contrib/biblatex/doc/biblatex.pdf].
        return switch (type) {
            ...
            case Thesis -> Localization
                .lang("A thesis written for an educational institution to satisfy the requirements for a degree.");
            case WWW -> Localization.lang("An alias for \"Online\", provided for jurabib compatibility.");
            case Software -> Localization
                .lang("Computer software. The standard styles will treat this entry type as an alias for \"Misc\".");
    ...
    private static PlainCitationParserChoice parserFromName(String parserName,

...
public class FrameDndHandler {
    ...
    void initDragAndDrop() {
        ...
        EasyBind.subscribe(tabPane.skinProperty(), skin -> {
            ...
            scene
                .get()
                .setOnDragOver(event -> onSceneDragOver(event, dndIndicator));
            scene
                .get()
                .setOnDragEntered(event -> {
                    ...
                    for (Node destinationTabNode : tabPane.lookupAll(".tab")) {
                        destinationTabNode.setOnDragOver(tabDragEvent ->
                            onTabDragOver(event, tabDragEvent, dndIndicator)
                        );
                        destinationTabNode.setOnDragExited(tabDragEvent ->
            ...
            scene
                ...
                .setOnDragDropped(event ->
                    onSceneDragDropped(event, dndIndicator)
                );
        });
    }

    private void onTabDragDropped(
        Node destinationTabNode,
        DragEvent tabDragEvent,
        Tab dndIndicator
    ) {
        ...
        if (hasBibFiles(dragboard)) {
            ...
            if (hasEntries(dragboard)) {
                List<BibEntry> entryCopies = stateManager
                    .getLocalDragboard()
                    .getBibEntries()
                    .stream()
                    .map(BibEntry::new)
    ...
    private void dropGroups(
        ...
        GroupTreeNode groupsTreeNode = stateManager
            .getActiveDatabase()
            .get()
            .getMetaData()
            .getGroups()
    ...
    private void copyRootNode(LibraryTab destinationLibraryTab) {
        ...
        GroupTreeNode currentLibraryGroupRoot = stateManager
            .getActiveDatabase()
            .get()
            .getMetaData()
            .getGroups()
    ...
    private void copyGroupTreeNode(
        ...
        List<BibEntry> allEntries = stateManager
            .getActiveDatabase()
            .get()
            .getEntries();

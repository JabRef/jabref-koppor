...
/**
 * Provides context menus for the text fields of the entry editor. Note that we use
 * {@link Supplier} to prevent an early instantiation of the menus. Therefore, they are
 * attached to each text field but instantiation happens on the first right-click of the
 * user in that field. The late instantiation is done by
 * {@link org.jabref.gui.fieldeditors.EditorTextArea#initContextMenu(java.util.function.Supplier, org.jabref.gui.keyboard.KeyBindingRepository)
 * EditorTextArea#initContextMenu}.
 */
public class EditorMenus {

    /**
     * The default context menu with a specific menu for normalizing person names
     * regarding to BibTex rules.
     * @param textInput text-input-control that this menu will be connected to
     * @return menu containing items of the default menu and an item for normalizing
     * person names
     */
    public static Supplier<List<MenuItem>> getNameMenu(final TextInputControl textInput) {
        return () -> {
            ...
            EasyBind.subscribe(textInput.textProperty(),
                    value -> normalizeNames.setDisable(StringUtil.isNullOrEmpty(value)));
            normalizeNames
                .setOnAction(event -> textInput.setText(new NormalizeNamesFormatter().format(textInput.getText())));
     ...
     * @return menu containing items of the default menu and an item for copying a DOI/DOI
     * URL
     */
    public static Supplier<List<MenuItem>> getDOIMenu(TextField textField, DialogService dialogService) {
        return () -> {
            ...
            MenuItem copyDoiMenuItem = factory.createMenuItem(StandardActions.COPY_DOI,
                    new CopyDoiUrlAction(textField, StandardActions.COPY_DOI, dialogService, clipBoardManager));
            MenuItem copyDoiUrlMenuItem = factory.createMenuItem(StandardActions.COPY_DOI_URL,
                    new CopyDoiUrlAction(textField, StandardActions.COPY_DOI_URL, dialogService, clipBoardManager));

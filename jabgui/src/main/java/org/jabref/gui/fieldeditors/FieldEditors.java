...
public class FieldEditors {
    ...
    public static FieldEditorFX getForField(final Field field,
            ...
            final BibDatabaseContext databaseContext, final EntryType entryType,
            final SuggestionProviders suggestionProviders, final UndoManager undoManager, final UndoAction undoAction,
            final RedoAction redoAction) {
        final Set<FieldProperty> fieldProperties = field.getProperties();

        final SuggestionProvider<?> suggestionProvider = getSuggestionProvider(field, suggestionProviders,
        ...
        boolean isMultiLine = FieldFactory.isMultiLineField(field,
        ...
        if (preferences.getTimestampPreferences().getTimestampField().equals(field)) {
            return new DateEditor(field,
                    DateTimeFormatter.ofPattern(preferences.getTimestampPreferences().getTimestampFormat()),
                    suggestionProvider, fieldCheckers, undoAction, redoAction);
        }
        else if (fieldProperties.contains(FieldProperty.DATE)) {
            return new DateEditor(field, DateTimeFormatter.ofPattern("[uuuu][-MM][-dd]"), suggestionProvider,
                    fieldCheckers, undoAction, redoAction);
        }
            ...
            // Identifier editor does not support PMID, therefore excluded at the
            // condition above
            ...
            return new OptionEditor<>(new MonthEditorViewModel(field, suggestionProvider, databaseContext.getMode(),
                    fieldCheckers, undoManager));
        }
        else if (fieldProperties.contains(FieldProperty.LANGUAGE)) {
            return new OptionEditor<>(new LanguageEditorViewModel(field, suggestionProvider, databaseContext.getMode(),
                    fieldCheckers, undoManager));
        }
            ...
            return new PersonsEditor(field, suggestionProvider, fieldCheckers, isMultiLine, undoManager, undoAction,
                    redoAction);
        }
            ...
            return new CitationKeyEditor(field, suggestionProvider, fieldCheckers, databaseContext, undoAction,
                    redoAction);
        }
        else if (fieldProperties.contains(FieldProperty.MARKDOWN)) {
            return new MarkdownEditor(field, suggestionProvider, fieldCheckers, preferences, undoManager, undoAction,
                    redoAction);
        }
        else {
            ...
            if (!isMultiLine && !selectorValues.isEmpty()) {
                return new OptionEditor<>(new CustomFieldEditorViewModel(field, suggestionProvider, fieldCheckers,
                        undoManager, selectorValues));
            }
            else {
    ...
    private static SuggestionProvider<?> getSuggestionProvider(Field field, SuggestionProviders suggestionProviders,
            MetaData metaData) {
        ...
        if (!contentSelectorValues.isEmpty()) {
            ...
            try {
                return new ContentSelectorSuggestionProvider((SuggestionProvider<String>) suggestionProvider,
                        contentSelectorValues);
            }
            catch (ClassCastException exception) {
                LOGGER
                    .error("Content selectors are only supported for normal fields with string-based auto completion.");

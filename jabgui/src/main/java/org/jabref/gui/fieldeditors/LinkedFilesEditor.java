...
public class LinkedFilesEditor extends HBox implements FieldEditorFX {
    ...
    public LinkedFilesEditor(
        ...
        decoratedModelList = new UiThreadObservableList<>(
            viewModel.filesProperty()
        );
    ...
    private void initialize() {
        ...
        progressIndicator
            .visibleProperty()
            .bind(viewModel.fulltextLookupInProgressProperty());
    ...
    private Node createFileDisplay(LinkedFileViewModel linkedFile) {
        ...
        EasyBind.subscribe(linkedFile.isAutomaticallyFoundProperty(), found ->
            link.pseudoClassStateChanged(opacity, found)
        );
        ...
        EasyBind.subscribe(linkedFile.isAutomaticallyFoundProperty(), found ->
            label.pseudoClassStateChanged(opacity, found)
        );
        ...
        writeMetadataToPdf.setTooltip(
            new Tooltip(
                Localization.lang("Write BibTeX to PDF (XMP and embedded)")
            )
        );
        writeMetadataToPdf
            .visibleProperty()
    ...
    private void handleItemMouseClick(
        ...
        if (
            ...
            SingleContextCommandFactory contextCommandFactory = (
                action,
                file
            ) ->
                new ContextAction(
            ...
            MultiContextCommandFactory multiContextCommandFactory = (
                action,
                files
            ) ->
            ...
            ContextMenu contextMenu = contextMenuFactory.createForSelection(
                listView.getSelectionModel().getSelectedItems()
            );

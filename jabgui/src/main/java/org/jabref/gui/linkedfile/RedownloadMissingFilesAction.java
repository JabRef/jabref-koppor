...
public class RedownloadMissingFilesAction extends SimpleCommand {
    ...
    public RedownloadMissingFilesAction(StateManager stateManager, DialogService dialogService,
    ...
    public void execute() {
        if (stateManager.getActiveDatabase().isPresent()) {
            ...
            boolean confirm = dialogService.showConfirmationDialogAndWait(Localization.lang("Redownload missing files"),
     ...
     * {@link org.jabref.gui.fieldeditors.LinkedFileViewModel#redownload}
     */
    private void redownloadMissing(BibDatabaseContext databaseContext) {
        ...
        databaseContext.getEntries().forEach(entry -> entry.getFiles().forEach(linkedFile -> {
            ...
            DownloadLinkedFileAction downloadAction = new DownloadLinkedFileAction(this.databaseContext, entry,
                    linkedFile, linkedFile.getSourceUrl(), dialogService, externalApplicationsPreferences,
                    filePreferences, taskExecutor, fileName, true);

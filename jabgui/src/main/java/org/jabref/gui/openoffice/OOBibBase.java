...
/**
 * Class for manipulating the Bibliography of the currently started document in
 * OpenOffice.
 */
public class OOBibBase {
    ...
    public void guiActionSelectDocument(boolean autoSelectForSingle)
        ...
        if (isConnectedToDocument()) {
            ...
            dialogService
                .notify(Localization.lang("Connected to document") + ": " + this.getCurrentDocumentTitle().orElse(""));
        }
    }
     ...
     * <p>
     * See also `isDocumentConnectionMissing` for a test actually attempting to use the
     * connection.
     */
    ...
    OOResult<FunctionalTextViewCursor, OOError> getFunctionalTextViewCursor(XTextDocument doc, String errorTitle) {
        String messageOnFailureToObtain = Localization.lang("Please move the cursor into the document text.") + "\n"
                + Localization.lang("To get the visual positions of your citations"
                        + " I need to move the cursor around," + " but could not get it.");
        ...
        try {
            ...
            if (recordingChanges || (nRedlines > 0)) {
                ...
                if (nRedlines > 0) {
                    ...
                    msg += Localization.lang("Changes by JabRef" + " could result in unexpected interactions with"
                            + " recorded changes.");
                    msg += "\n";
            ...
            String msg = Localization
                .lang("Error while checking if Writer" + " is recording changes or has recorded changes.");
     ...
     */
    private OOVoidResult<OOError> checkStyleExistsInTheDocument(String familyName, String styleName, XTextDocument doc,
            String labelInJstyleFile, String pathToStyleFile) throws WrappedTargetException {

        Optional<String> internalName = UnoStyle.getInternalNameOfStyle(doc, familyName, styleName);

        if (internalName.isEmpty()) {
            String msg = switch (familyName) {
                case UnoStyle.PARAGRAPH_STYLES -> Localization
                    .lang("The %0 paragraph style '%1' is missing from the document", labelInJstyleFile, styleName);
                case UnoStyle.CHARACTER_STYLES -> Localization
                    .lang("The %0 character style '%1' is missing from the document", labelInJstyleFile, styleName);
                default -> throw new IllegalArgumentException("Expected " + UnoStyle.CHARACTER_STYLES + " or "
                        + UnoStyle.PARAGRAPH_STYLES + " for familyName");
        ...
        if (!internalName.get().equals(styleName)) {
            String msg = switch (familyName) {
                case UnoStyle.PARAGRAPH_STYLES ->
                    Localization.lang("The %0 paragraph style '%1' is a display name for '%2'.", labelInJstyleFile,
                            styleName, internalName.get());
                case UnoStyle.CHARACTER_STYLES ->
                    Localization.lang("The %0 character style '%1' is a display name for '%2'.", labelInJstyleFile,
                            styleName, internalName.get());
                default -> throw new IllegalArgumentException("Expected " + UnoStyle.CHARACTER_STYLES + " or "
                        + UnoStyle.PARAGRAPH_STYLES + " for familyName");
            } + "\n" + Localization
                .lang("Please use the latter in the style file below" + " to avoid localization problems.") + "\n"
                    + pathToStyleFile;
    ...
    public OOVoidResult<OOError> checkStylesExistInTheDocument(JStyle jStyle, XTextDocument doc) {
        ...
        try {
            results.add(
                    checkStyleExistsInTheDocument(UnoStyle.PARAGRAPH_STYLES, jStyle.getReferenceHeaderParagraphFormat(),
                            doc, "ReferenceHeaderParagraphFormat", pathToStyleFile));
            results.add(checkStyleExistsInTheDocument(UnoStyle.PARAGRAPH_STYLES, jStyle.getReferenceParagraphFormat(),
                    doc, "ReferenceParagraphFormat", pathToStyleFile));
            if (jStyle.isFormatCitations()) {
                results.add(checkStyleExistsInTheDocument(UnoStyle.CHARACTER_STYLES,
                        jStyle.getCitationCharacterFormat(), doc, "CitationCharacterFormat", pathToStyleFile));
            }
        }
        catch (WrappedTargetException ex) {
            results.add(OOVoidResult
                .error(new OOError("Other error in checkStyleExistsInTheDocument", ex.getMessage(), ex)));
        }
     ...
     * DocumentConnection.setUserDefinedStringPropertyValue() is called, and Undo in LO
     * will not undo that.
     * <p>
     * GUI: "Manage citations" dialog "OK" button. Called from:
     * ManageCitationsDialogViewModel.storeSettings
     *
     * <p>
     * Currently the only editable part is pageInfo.
     * <p>
     * Since the only call to applyCitationEntries() only changes pageInfo w.r.t those
     * returned by getCitationEntries(), we can do with the following restrictions:
     * <ul>
     * <li>Missing pageInfo means no action.</li>
     * <li>Missing CitationEntry means no action (no attempt to remove citation from the
     * text).</li>
     * </ul>
     */
    public void guiActionApplyCitationEntries(List<CitationEntry> citationEntries) {
        ...
        catch (PropertyVetoException | IllegalTypeException | WrappedTargetException
     ...
     * @param bibDatabaseContext The database the entries belong to (all of them). Used
     * when creating the citation mark.
     * <p>
     * Consistency: for each entry in {@code entries}: looking it up in
     * {@code syncOptions.get().databases} (if present) should yield {@code database}.
     * @param style The bibliography style we are using.
     * @param citationType Indicates whether it is an in-text citation, a citation in
     * parenthesis or an invisible citation.
     * @param pageInfo A single page-info for these entries. Attributed to the last entry.
     * @param syncOptions Indicates whether in-text citations should be refreshed in the
     * document. Optional.empty() indicates no refresh. Otherwise provides options for
     * refreshing the reference list.
     */
    public void guiActionInsertEntry(List<BibEntry> entries, BibDatabaseContext bibDatabaseContext,
            BibEntryTypesManager bibEntryTypesManager, OOStyle style, CitationType citationType, String pageInfo,
        ...
        if (style instanceof JStyle jStyle) {
            if (testDialog(errorTitle, checkStylesExistInTheDocument(jStyle, doc),
        ...
        if (syncOptions.isPresent()) {
            ...
            if (testDialog(errorTitle, fcursor.asVoidResult())
                    || testDialog(databaseIsRequired(syncOptions.get().databases,
        ...
        try {
            ...
            if (style instanceof CitationStyle citationStyle) {
                ...
                try {
                    // Lock document controllers - disable refresh during the process
                    // (avoids document flicker during writing)
                    // MUST always be paired with an unlockControllers() call
                    doc.lockControllers();

                    if (citationType == CitationType.AUTHORYEAR_PAR) {
                        // "Cite" button
                        cslCitationOOAdapter.insertCitation(cursor.get(), citationStyle, entries, bibDatabaseContext,
                                bibEntryTypesManager);
                    }
                    else if (citationType == CitationType.AUTHORYEAR_INTEXT) {
                        // "Cite in-text" button
                        cslCitationOOAdapter.insertInTextCitation(cursor.get(), citationStyle, entries,
                                bibDatabaseContext, bibEntryTypesManager);
                    }
                    ...
                    // If "Automatically sync bibliography when inserting citations" is
                    // enabled
                ...
                EditInsert.insertCitationGroup(doc, frontend.get(), cursor.get(), entries,
                        bibDatabaseContext.getDatabase(), jStyle, citationType, pageInfo);
        ...
        catch (CreationException | WrappedTargetException | PropertyVetoException | IllegalTypeException
    ...
    public void guiActionMergeCitationGroups(List<BibDatabase> databases, OOStyle style) {
        ...
        if (style instanceof JStyle jStyle) {
            ...
            if (testDialog(errorTitle, fcursor.asVoidResult(), checkStylesExistInTheDocument(jStyle, doc),
                    checkIfOpenOfficeIsRecordingChanges(doc))) {
                return;
            }
            ...
            catch (CreationException | IllegalTypeException | NotRemoveableException | PropertyVetoException
                    | WrappedTargetException | com.sun.star.lang.IllegalArgumentException ex) {
     ...
     * <p>
     * Do the opposite of MergeCitationGroups. Combined markers are split, with a space
     * inserted between.
     */
     ...
     * @param returnPartialResult If there are some unresolved keys, shall we return an
     * otherwise nonempty result, or Optional.empty()?
     */
        ...
        try {
            ...
            if (!result.newDatabase.hasEntries()) {
                dialogService.showErrorDialogAndWait(Localization.lang("Unable to generate new library"),
                        Localization.lang("Your OpenOffice/LibreOffice document references" + " no citation keys"
            ...
            if (!unresolvedKeys.isEmpty()) {
                dialogService
                    .showErrorDialogAndWait(Localization.lang("Unable to generate new library"), Localization.lang(
                            "Your OpenOffice/LibreOffice document references" + " at least %0 citation keys"
                                    + " which could not be found in your current library." + " Some of these are %1.",
        ...
        if (style instanceof JStyle jStyle) {
            try {
                ...
                try {
                    ...
                    Update.SyncOptions syncOptions = new Update.SyncOptions(databases);
                    syncOptions.setUpdateBibliography(true)
                ...
                if (!unresolvedKeys.isEmpty()) {
                    String msg = Localization
                        .lang("Your OpenOffice/LibreOffice document references the citation key '%0',"
                                + " which could not be found in your current library.", unresolvedKeys.getFirst());
            ...
            try {
                ...
                try {
                    ...
                    if (citedEntries.isEmpty()) {
                        dialogService.showInformationDialogAndWait(Localization.lang("Bibliography"),
                    ...
                    cslUpdateBibliography.rebuildCSLBibliography(doc, cslCitationOOAdapter, citedEntries, citationStyle,
                            bibDatabaseContext, Injector.instantiateModelOrService(BibEntryTypesManager.class));
                }

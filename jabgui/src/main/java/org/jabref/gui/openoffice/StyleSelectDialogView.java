...
public class StyleSelectDialogView extends BaseDialog<OOStyle> {
    ...
    private final AtomicBoolean initialScrollPerformed = new AtomicBoolean(
        false
    );
    ...
    private void setupCslStylesTab() {
        ...
        cslNameColumn.setCellValueFactory(cellData ->
            cellData.getValue().nameProperty()
        );
        cslPathColumn.setCellValueFactory(cellData ->
            cellData.getValue().pathProperty()
        );
        cslDeleteColumn.setCellValueFactory(cellData ->
        ...
        new ValueTableCellFactory<CSLStyleSelectViewModel, Boolean>()
            ...
            .withOnMouseClickedEvent(
                item ->
                    evt -> {
                        CSLStyleSelectViewModel selectedStyle = cslStylesTable
                            .getSelectionModel()
                            .getSelectedItem();
                        if (selectedStyle != null) {
                            viewModel.deleteCslStyle(
                                selectedStyle.getLayout().getCitationStyle()
                            );
                        }
                    }
            )
        ...
        PreviewViewer cslPreviewViewer = initializePreviewViewer(
            TestEntry.getTestEntry()
        );
        EasyBind.subscribe(
            viewModel.selectedCslLayoutProperty(),
            cslPreviewViewer::setLayout
        );
        cslPreviewBox.getChildren().add(cslPreviewViewer);

        viewModel
            .getAvailableCslLayouts()
            .addListener(
    ...
    private void setupJStylesTab() {
        ...
        jStyleNameColumn.setCellValueFactory(cellData ->
            cellData.getValue().nameProperty()
        );
        jStyleJournalColumn.setCellValueFactory(cellData ->
            cellData.getValue().journalsProperty()
        );
        jStyleFileColumn.setCellValueFactory(cellData ->
            cellData.getValue().fileProperty()
        );
        jStyleDeleteColumn.setCellValueFactory(cellData ->
            cellData.getValue().internalStyleProperty()
        );
    ...
    private PreviewViewer initializePreviewViewer(BibEntry entry) {
        PreviewViewer viewer = new PreviewViewer(
            dialogService,
            preferences,
            themeManager,
            taskExecutor
        ...
        if (viewModel.selectedCslLayoutProperty().get() != null) {
            for (CSLStyleSelectViewModel model : cslStylesTable.getItems()) {
                if (
                    model
                        .getLayout()
                        .equals(viewModel.selectedCslLayoutProperty().get())
                ) {
            ...
            for (int i = 0; i < cslStylesTable.getItems().size(); i++) {
                ...
                if (
                    item
                        .getLayout()
                        .getFilePath()
                        .equals(currentCitationStyle.getFilePath())
                ) {

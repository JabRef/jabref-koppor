...
public class AutoRenameFileOnEntryChange {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(
        AutoRenameFileOnEntryChange.class
    );

    private final FilePreferences filePreferences;
    private final RenamePdfCleanup renamePdfCleanup;

    public AutoRenameFileOnEntryChange(
        BibDatabaseContext bibDatabaseContext,
        FilePreferences filePreferences
    ) {
        this.filePreferences = filePreferences;
        renamePdfCleanup = new RenamePdfCleanup(
            false,
            () -> bibDatabaseContext,
            filePreferences
        );
    ...
    public void listen(FieldChangedEvent event) {
        ...
        LOGGER.debug(
            "Field changed for entry {}: {}",
            entry.getCitationKey().orElse("defaultCitationKey"),

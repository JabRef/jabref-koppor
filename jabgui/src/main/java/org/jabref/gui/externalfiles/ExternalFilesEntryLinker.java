...
public class ExternalFilesEntryLinker {
     ...
     */
    public ExternalFilesEntryLinker(ExternalApplicationsPreferences externalApplicationsPreferences,
            FilePreferences filePreferences, NotificationService notificationService, StateManager stateManager) {
    ...
    public void linkFilesToEntry(BibEntry entry, List<Path> files) {
        ...
        List<LinkedFile> linkedFiles = files.stream().flatMap(file -> {
            String typeName = FileUtil.getFileExtension(file)
                .map(ext -> ExternalFileTypes.getExternalFileTypeByExt(ext, externalApplicationsPreferences)
            ...
            boolean alreadyLinked = existingFiles.stream()
                .anyMatch(existingFile -> existingFile.getLink().equals(link));
            if (alreadyLinked) {
     ...
     * <li>Rename file to configured pattern (and skip renaming if file already
     * exists)</li>
    ...
    public void coveOrMoveFilesSteps(BibEntry entry, List<Path> files, boolean shouldMove) {
        ...
        for (Path file : files) {
            ...
            LinkedFileHandler linkedFileHandler = new LinkedFileHandler(linkedFile, entry,
                    bibDatabaseContextSupplier.get(), filePreferences);
            try {

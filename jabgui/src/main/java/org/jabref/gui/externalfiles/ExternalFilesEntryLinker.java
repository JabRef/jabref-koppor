...
public class ExternalFilesEntryLinker {
    ...
    public ExternalFilesEntryLinker(
        ...
        this.bibDatabaseContextSupplier = () ->
            stateManager.getActiveDatabase().orElse(new BibDatabaseContext());
        this.notificationService = notificationService;
    }

    public void linkFilesToEntry(BibEntry entry, List<Path> files) {
        List<LinkedFile> existingFiles = entry.getFiles();
        List<LinkedFile> linkedFiles = files
            .stream()
            .flatMap(file -> {
                ...
                Path relativePath = FileUtil.relativize(
                    file,
                    bibDatabaseContextSupplier.get(),
                    filePreferences
                );
                ...
                boolean alreadyLinked = existingFiles
                    .stream()
                    .anyMatch(existingFile ->
                        existingFile.getLink().equals(link)
                    );
                if (alreadyLinked) {
                    notificationService.notify(
                        Localization.lang("File '%0' already linked", link)
                    );
                    return Stream.empty();
                } else {
     ...
     * </ul>
     */
    public void coveOrMoveFilesSteps(
        BibEntry entry,
        List<Path> files,
        boolean shouldMove
    ) {
        ...
        for (Path file : files) {
            ...
            try {
                linkedFileHandler.copyOrMoveToDefaultDirectory(
                    shouldMove,
                    true
                );

...
public class GlobalSearchResultDialog extends BaseDialog<Void> {
    ...
    private void initialize() {
        GlobalSearchResultDialogViewModel viewModel =
            new GlobalSearchResultDialogViewModel(
        ...
        resultsTable.setOnMouseClicked(event -> {
            if (event.getClickCount() == 2) {
                BibEntryTableViewModel selectedEntry = resultsTable
                    .getSelectionModel()
                    .getSelectedItem();
                ...
                libraryTabContainer
                    ...
                    .filter(tab ->
                        tab
                            ...
                            .equals(selectedEntry.getBibDatabaseContext())
                    )
                    .findFirst()
                ...
                stateManager
                    ...
                    .get()
                    .ifPresent(tab ->
                        tab.clearAndSelect(selectedEntry.getEntry())
                    );
        ...
        keepOnTopSubscription = EasyBind.subscribe(
            viewModel.keepOnTop(),
            value -> {
        ...
        stage.setOnShown(event -> {
            stage.setHeight(
                preferences.getSearchPreferences().getSearchWindowHeight()
            );
            stage.setWidth(
                preferences.getSearchPreferences().getSearchWindowWidth()
            );
            container.setDividerPositions(
                preferences
                    .getSearchPreferences()
                    .getSearchWindowDividerPosition()
            );

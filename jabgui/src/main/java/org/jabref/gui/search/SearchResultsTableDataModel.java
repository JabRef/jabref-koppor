...
public class SearchResultsTableDataModel {
    ...
    public SearchResultsTableDataModel(
        ...
        stateManager
            .getOpenDatabases()
            .addListener(
                (ListChangeListener<BibDatabaseContext>) change ->
                    populateEntriesViewModel()
            );
            ...
            if (query.isPresent()) {
                ...
                for (BibEntryTableViewModel entry : entriesViewModel) {
                    entry
                        ...
                        .set(
                            searchResults.hasFulltextResults(entry.getEntry())
                        );
                    entry
                        .isVisibleBySearch()
                        .set(searchResults.isMatched(entry.getEntry()));
                }
            } else {
                for (BibEntryTableViewModel entry : entriesViewModel) {

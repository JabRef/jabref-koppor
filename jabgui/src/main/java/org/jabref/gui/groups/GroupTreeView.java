...
public class GroupTreeView extends BorderPane {
        ...
        groupTree.setOnKeyPressed(event -> {
            if (
                event
                    ...
                    .toString()
                    .equals(
                        KeyBinding.GROUP_SUBGROUP_RENAME.getDefaultKeyBinding()
                    )
            ) {
                TreeItem<GroupNodeViewModel> selectedItem = groupTree
                    .getSelectionModel()
                    .getSelectedItem();
    ...
    private void initialize() {
        ...
        groupTree
            .rootProperty()
            .bind(
                EasyBind.map(viewModel.rootGroupProperty(), group -> {
                    if (group == null) {
                        return null;
                    } else {
            ...
            .bind(
                Bindings.createStringBinding(
                    () -> {
                        ...
                        return "-fx-font-size: %fem;".formatted(
                            reducedFontSize
                        );
                    },
    ...
    private void handleOnDragDropped(
        ...
        if (dragboard.hasFiles()) {
            ...
            stateManager.setSelectedGroups(
                database,
                List.of(row.getItem().getGroupNode())
            );
            importHandler
    ...
    private void initScrolling() {
        ...
        scrollableAreaHeight = Math.min(
            heightOfOneNode * 3.0,
            groupTree.getHeight() / 3.0
        );
    ...
    private ContextMenu createContextMenuForGroup(GroupNodeViewModel group) {
        ...
        if (group.isAllEntriesGroup()) {
            contextMenu
                ...
                .add(
                    factory.createMenuItem(
                        StandardActions.GROUP_SUGGESTED_GROUPS_ADD,
                        new ContextAction(
                            StandardActions.GROUP_SUGGESTED_GROUPS_ADD,
                            group
                        )
        ...
        contextMenu
            ...
            .addAll(
                ...
                factory.createMenuItem(
                    StandardActions.GROUP_SUBGROUP_RENAME,
                    new ContextAction(
                        StandardActions.GROUP_SUBGROUP_RENAME,
                        group
                    )
    ...
    private void setupClearButtonField(CustomTextField customTextField) {
        try {
            Method m = TextFields.class.getDeclaredMethod(
    ...
    private class ContextAction extends SimpleCommand {
            ...
            this.executable.bind(
                BindingsHelper.constantOf(
                    switch (command) {
                        case
                            GROUP_EDIT,
                            GROUP_SUBGROUP_RENAME -> group.isEditable();
                        case
                            GROUP_REMOVE,
        ...
        public void execute() {
            switch (command) {
                ...
                case GROUP_EDIT, GROUP_SUBGROUP_RENAME -> viewModel.editGroup(
                    group
                );
                case GROUP_GENERATE_EMBEDDINGS -> viewModel.generateEmbeddings(
                    group
                );
                case GROUP_GENERATE_SUMMARIES -> viewModel.generateSummaries(
                    group
                );
                case GROUP_CHAT -> viewModel.chatWithGroup(group);
                case GROUP_SUGGESTED_GROUPS_ADD -> viewModel.addSuggestedGroups(
                    group
                );

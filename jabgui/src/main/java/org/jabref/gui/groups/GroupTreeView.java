...
public class GroupTreeView extends BorderPane {
    ...
    private static final PseudoClass PSEUDOCLASS_SUBELEMENT = PseudoClass.getPseudoClass("sub"); // >
                                                                                                 // 1
                                                                                                 // deep
    ...
    /**
     * Note: This panel is deliberately not created in fxml, since parsing equivalent fxml
     * takes about 500 msecs
     */
    public GroupTreeView(TaskExecutor taskExecutor, StateManager stateManager, AdaptVisibleTabs adaptVisibleTabs,
            GuiPreferences preferences, DialogService dialogService, AiService aiService, UndoManager undoManager,
    ...
    private void initialize() {
        ...
        // Set-up bindings
        Platform
            .runLater(() -> BindingsHelper.bindContentBidirectional(groupTree.getSelectionModel().getSelectedItems(),
                    viewModel.selectedGroupsProperty(), newSelectedGroups -> {
                        groupTree.getSelectionModel().clearSelection();
                        newSelectedGroups.forEach(this::selectNode);
                    }, this::updateSelection));

        // We try to prevent publishing changes in the search field directly to the search
        ...
        groupTree.rootProperty().bind(EasyBind.map(viewModel.rootGroupProperty(), group -> {
            if (group == null) {
                ...
                return new RecursiveTreeItem<>(group, GroupNodeViewModel::getChildren,
                        GroupNodeViewModel::expandedProperty, viewModel.filterPredicateProperty());
            }
        }));

        // Icon and group name
        new ViewModelTreeTableCellFactory<GroupNodeViewModel>().withText(GroupNodeViewModel::getDisplayName)
        ...
        new ViewModelTreeTableCellFactory<GroupNodeViewModel>().withGraphic(this::createNumberCell)
            .install(numberColumn);

        // Arrow indicating expanded status
        new ViewModelTreeTableCellFactory<GroupNodeViewModel>().withGraphic(this::getArrowCell)
        ...
        new ViewModelTreeTableRowFactory<GroupNodeViewModel>().withContextMenu(this::createContextMenuForGroup)
            ...
            .withCustomInitializer(row -> {
                ...
                // column
                // Simply setting to null is not enough since it would be replaced by the
                // default node on every change
            ...
            .withPseudoClass(PSEUDOCLASS_ROOTELEMENT,
                    row -> Bindings.createBooleanBinding(() -> (row != null) && (groupTree.getRoot() != null)
                            && (row.getItem() == groupTree.getRoot().getValue()), row.treeItemProperty()))
            .withPseudoClass(PSEUDOCLASS_SUBELEMENT,
    ...
    private StackPane createNumberCell(GroupNodeViewModel group) {
        ...
        Text text = new Text();
        EasyBind.subscribe(preferences.getGroupsPreferences().displayGroupCountProperty(), shouldDisplayGroupCount -> {
    ...
    private void handleOnDragExited(TreeTableRow<GroupNodeViewModel> row, GroupNodeViewModel fieldViewModel,
            DragEvent dragEvent) {
        ControlHelper.removeDroppingPseudoClasses(row);
    }

    private void handleOnDragDetected(TreeTableRow<GroupNodeViewModel> row, GroupNodeViewModel groupViewModel,
            MouseEvent event) {
        List<String> groupsToMove = new ArrayList<>();
        for (TreeItem<GroupNodeViewModel> selectedItem : row.getTreeTableView()
            .getSelectionModel()
            .getSelectedItems()) {
    ...
    private void handleOnDragDropped(TreeTableRow<GroupNodeViewModel> row, GroupNodeViewModel originalItem,
            DragEvent event) {
        ...
        if (dragboard.hasFiles()) {
            ...
            this.importHandler = new ImportHandler(database, preferences, fileUpdateMonitor, undoManager, stateManager,
            ...
            importHandler
                .importFilesInBackground(files, database, preferences.getFilePreferences(), event.getTransferMode())
    ...
    private void handleOnDragOver(TreeTableRow<GroupNodeViewModel> row, GroupNodeViewModel originalItem,
            DragEvent event) {
        Dragboard dragboard = event.getDragboard();
    ...
    private void setupDragScrolling() {
        ...
        groupTree.setOnDragOver(event -> {
            ...
            // / 10.0 is because of the 100 milliseconds above. (it is 20px per second,
            // 10.0 * 100.0 ms = 1 second)
    ...
    private void initScrolling() {
        ...
        // https://github.com/JabRef/jabref/issues/9754#issuecomment-1766864908)
        // (1.0 / groupTree.getHeight()) is the factor to convert from px to fraction of
        // total height
    ...
    private ContextMenu createContextMenuForGroup(GroupNodeViewModel group) {
        ...
        if (group.hasSubgroups() && group.canAddGroupsIn() && !group.isRoot()) {
            ...
            removeGroup = factory.createMenuItem(StandardActions.GROUP_REMOVE,
                    new GroupTreeView.ContextAction(StandardActions.GROUP_REMOVE, group));
        }

        if (preferences.getAiPreferences().getEnableAi()) {
            contextMenu.getItems()
                .add(factory.createMenuItem(StandardActions.GROUP_CHAT,
                        new ContextAction(StandardActions.GROUP_CHAT, group)));
        }

        contextMenu.getItems()
            .addAll(factory.createMenuItem(StandardActions.GROUP_EDIT,
                    new ContextAction(StandardActions.GROUP_EDIT, group)),
                    factory.createMenuItem(StandardActions.GROUP_GENERATE_EMBEDDINGS,
                            new ContextAction(StandardActions.GROUP_GENERATE_EMBEDDINGS, group)),
                    factory.createMenuItem(StandardActions.GROUP_GENERATE_SUMMARIES,
                            new ContextAction(StandardActions.GROUP_GENERATE_SUMMARIES, group)),
                    removeGroup, new SeparatorMenuItem());

        if (group.isAllEntriesGroup()) {
            contextMenu.getItems()
                .add(factory.createMenuItem(StandardActions.GROUP_SUGGESTED_GROUPS_ADD,
                        new ContextAction(StandardActions.GROUP_SUGGESTED_GROUPS_ADD, group)));
        }

        contextMenu.getItems()
            .addAll(factory.createMenuItem(StandardActions.GROUP_SUBGROUP_ADD,
                    new ContextAction(StandardActions.GROUP_SUBGROUP_ADD, group)),
                    factory.createMenuItem(StandardActions.GROUP_SUBGROUP_RENAME,
                            new ContextAction(StandardActions.GROUP_SUBGROUP_RENAME, group)),
                    factory.createMenuItem(StandardActions.GROUP_SUBGROUP_REMOVE,
                    ...
                    factory.createMenuItem(StandardActions.GROUP_ENTRIES_ADD,
                            new ContextAction(StandardActions.GROUP_ENTRIES_ADD, group)),
                    factory.createMenuItem(StandardActions.GROUP_ENTRIES_REMOVE,
    ...
    private ObservableBooleanValue groupsDisabledProperty() {
        return Bindings.createBooleanBinding(() -> stateManager.getOpenDatabases().isEmpty(),
    ...
    private class ContextAction extends SimpleCommand {
        ...
        public ContextAction(StandardActions command, GroupNodeViewModel group) {
            ...
            this.executable.bind(BindingsHelper.constantOf(switch (command) {
                case GROUP_EDIT, GROUP_SUBGROUP_RENAME -> group.isEditable();
                ...
                case GROUP_SUBGROUP_REMOVE ->
                    group.isEditable() && group.hasSubgroups() && group.canRemove() || group.isRoot();
                case GROUP_SUBGROUP_SORT ->
                    group.isEditable() && group.hasSubgroups() && group.canAddEntriesIn() || group.isRoot();
        ...
        public void execute() {
            switch (command) {
                ...
                case GROUP_EDIT, GROUP_SUBGROUP_RENAME -> viewModel.editGroup(group);
                ...
                case GROUP_SUGGESTED_GROUPS_ADD -> viewModel.addSuggestedGroups(group);

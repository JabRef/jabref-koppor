...
public class GroupTreeViewModel extends AbstractViewModel {
    ...
    private void onActiveDatabaseChanged(
        ...
        if (newDatabase.isPresent()) {
            ...
            if (stateManager.getSelectedGroups(newDatabase.get()).isEmpty()) {
                stateManager.setSelectedGroups(
                    newDatabase.get(),
                    List.of(newRoot.getGroupNode())
                );
            }
            selectedGroups.setAll(
                stateManager
                    .getSelectedGroups(newDatabase.get())
                    .stream()
                    .map(selectedGroup ->
                        new GroupNodeViewModel(
                            newDatabase.get(),
    ...
    private void addGroupImportEntries(GroupNodeViewModel parent) {
        ...
        String grpName = preferences
            .getLibraryPreferences()
            .getAddImportedEntriesGroupName();
        AbstractGroup importEntriesGroup = new SmartGroup(
            grpName,
            GroupHierarchyType.INDEPENDENT,
            ','
        );
        boolean isGrpExist = parent
            .getGroupNode()
            .getChildren()
            .stream()
    ...
    public void addSuggestedGroups(GroupNodeViewModel parent) {
        currentDatabase.ifPresent(database -> {
            ...
            SearchGroup withoutGroupsGroup =
            ...
            dialogService.notify(
                Localization.lang(
                    "Created %0 suggested groups.",
                    String.valueOf(newSuggestedSubgroups.size())
                )
            );
        });
    }
    ...
    public void chatWithGroup(GroupNodeViewModel group) {
        ...
        StringProperty groupNameProperty = group
            .getGroupNode()
            .getGroup()
            .nameProperty();
        ...
        groupNameProperty.addListener((obs, oldValue, newValue) ->
            nameProperty.setValue(
                Localization.lang("Group %0", groupNameProperty.get())
            )
        );

        ObservableList<ChatMessage> chatHistory = aiService
            .getChatHistoryService()
            .getChatHistoryForGroup(
                currentDatabase.get(),
                group.getGroupNode()
            );
        ObservableList<BibEntry> bibEntries = FXCollections.observableArrayList(
            group
                .getGroupNode()
                .findMatches(currentDatabase.get().getDatabase())
        );
    ...
    private void openAiChat(
        ...
        Optional<AiChatWindow> existingWindow = stateManager
            .getAiChatWindows()
            .stream()
    ...
    public void generateSummaries(GroupNodeViewModel groupNode) {
        ...
        dialogService.notify(
            Localization.lang(
                "Summarization started for group \"%0\".",
                group.getName()
            )
        );
    }

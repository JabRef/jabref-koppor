...
public class GitPullViewModel extends AbstractViewModel {
    ...
    public MergeResult pull()
        ...
        Optional<BibDatabaseContext> databaseContextOpt =
            gitStatusViewModel.getDatabaseContext();
        ...
        Path bibFilePath = localBibDatabaseContext
            .getDatabasePath()
            .orElseThrow(() ->
                new JabRefException(
                    Localization.lang(
                        "Cannot pull: Please save the library to a file first."
                    )
                )
            );

        MergeResult result = syncService.fetchAndMerge(
            localBibDatabaseContext,
            bibFilePath
        );

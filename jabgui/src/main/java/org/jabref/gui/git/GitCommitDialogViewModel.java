...
public class GitCommitDialogViewModel extends AbstractViewModel {
    ...
    public GitCommitDialogViewModel(
        ...
        this.commitMessageValidator = new FunctionBasedValidator<>(
            ...
            ValidationMessage.error(
                Localization.lang("Commit message cannot be empty")
            )
        );
    }
    ...
    private void doCommit()
        ...
        if (!status.tracking()) {
            throw new JabRefException(
                Localization.lang(
                    "Commit aborted: The file is not under Git version control."
                )
            );
        }
        if (status.conflict()) {
            throw new JabRefException(
                Localization.lang(
                    "Commit aborted: Local repository has unresolved merge conflicts."
                )
            );
        }

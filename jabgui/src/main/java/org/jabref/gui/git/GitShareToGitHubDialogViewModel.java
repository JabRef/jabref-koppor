...
public class GitShareToGitHubDialogViewModel extends AbstractViewModel {
    ...
    // TODO: This should be a library preference -> the library is connected to
    // repository; not all JabRef libraries to the same one
    // Reason: One could have https://github.com/JabRef/JabRef-exmple-libraries as one
    // repo and https://github.com/myexampleuser/demolibs as onther repository
    // Both share the same secrets, but are different URLs.
    // Also think of having two .bib files in the same folder - they will have the same
    // repository URL -- should make no issues, but let's see...
    ...
    public GitShareToGitHubDialogViewModel(GitPreferences gitPreferences, StateManager stateManager,
        ...
        repositoryUrlValidator = new FunctionBasedValidator<>(repositoryUrlProperty, githubHttpsUrlValidator(),
                ValidationMessage.error(Localization.lang("Please enter a valid HTTPS GitHub repository URL")));
        githubUsernameValidator = new FunctionBasedValidator<>(usernameProperty, notEmptyValidator(),
                ValidationMessage.error(Localization.lang("GitHub username is required")));
        githubPatValidator = new FunctionBasedValidator<>(patProperty, notEmptyValidator(),
    ...
    public void shareToGitHub(Runnable close) {
        // We store the settings because "Share" implies that the settings should be used
        // as typed
        // We also have the option to not store the settings permanently: This is
        // implemented in JabRefCliPreferences at the listeners.
        this.storeSettings();
        BackgroundTask.wrap(() -> {
            ...
            .onFailure(e -> dialogService.showErrorDialogAndWait(Localization.lang("GitHub share failed"),
                    e.getMessage(), e))
    ...
    private void doShareToGitHub() throws JabRefException, IOException, GitAPIException {
        ...
        if (bibFilePathOpt.isEmpty()) {
            throw new JabRefException(
                    Localization.lang("No library file path. Please save the library to a file first."));
        }

        // We don't get a new preference object (and re-use the existing one instead),
        // because of ADR-0016

        // TODO: Read remove from the git configuration - and only prompt for a repository
        // if there is none
        ...
        if (status.syncStatus() == SyncStatus.BEHIND) {
            throw new JabRefException(
                    Localization.lang("Remote repository is not empty. Please pull changes before pushing."));
        }

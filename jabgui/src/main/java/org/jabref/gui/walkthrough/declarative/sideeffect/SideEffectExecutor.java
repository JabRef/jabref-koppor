...
public class SideEffectExecutor {
    ...
    private boolean execute(
        ...
        try {
            if (forward) {
                ...
                if (!conditionMet) {
                    LOGGER.warn(
                        "Expected condition not met for side effect: {}",
                        sideEffect.description()
                    );
                    notifyUser(
                        Localization.lang("Side effect timeout"),
                        Localization.lang(
                            "The condition for '%0' was not met within the timeout period.",
                            sideEffect.description()
                        )
                    );
                    return false;
            ...
            LOGGER.error(
                "Error executing {} effect: {}",
                forward ? "forward" : "backward",
                sideEffect.description(),
                e
            );
            notifyUser(
                Localization.lang("Walkthrough side effect error"),
                Localization.lang(
    ...
    private boolean waitForCondition(
        ...
        if (condition.evaluate()) {
            LOGGER.debug(
                "Expected condition already met for: {}",
                sideEffect.description()
            );
            return true;
        }
        ...
        startTimeout(sideEffect.timeoutMs(), () -> {
            LOGGER.debug(
                "Timeout reached for side effect: {}",
                sideEffect.description()
            );
        ...
        setupDependencyMonitoring(sideEffect.dependencies(), () -> {
            if (condition.evaluate()) {
                LOGGER.debug(
                    "Expected condition met via dependency change for: {}",
                    sideEffect.description()
                );
        ...
        try {
            return conditionFuture.get(
                sideEffect.timeoutMs() + 100,
    ...
    private void setupDependencyMonitoring(
        @NonNull List<Observable> dependencies,

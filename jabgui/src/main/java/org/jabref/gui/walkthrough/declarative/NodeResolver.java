...
public interface NodeResolver {
    ...
    static NodeResolver buttonWithGraphic(IconTheme.JabRefIcons glyph) {
        return scene ->
            Streams
                ...
                .concat(
                scene.getRoot().lookupAll(".button").stream(),
                ...
                .filter(node -> {
                    ...
                    return (
                        ((graphic instanceof JabRefIconView jabRefIconView)
                            && jabRefIconView.getGlyph() == glyph)
                        || ((graphic instanceof FontIcon fontIcon)
                            && fontIcon.getIconCode() == glyph.getIkon())
                    );
                })
                .findFirst();
    }
    ...
    static NodeResolver buttonType(@NonNull ButtonType buttonType) {
        return scene ->
            predicate(DialogPane.class::isInstance)
                .resolve(scene)
                .map(node ->
    ...
    static NodeResolver selectorWithText(
        @NonNull String selector,
        @NonNull Predicate<String> textMatcher
    ) {
        return scene ->
            scene
                .getRoot()
    ...
    static NodeResolver menuItem(@NonNull String key) {
        return scene -> {
            ...
            return menu
                ...
                .filter(item ->
                    Optional.ofNullable(item.getText())
    ...
    private static Node findNode(
        @NonNull Node root,
        @NonNull Predicate<Node> predicate
    ) {

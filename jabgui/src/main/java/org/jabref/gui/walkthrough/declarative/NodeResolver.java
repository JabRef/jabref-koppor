...
public interface NodeResolver {
    ...
    static NodeResolver predicate(@NonNull Predicate<Node> predicate) {
        return scene ->
            Optional.ofNullable(findNode(scene.getRoot(), predicate));
    }
            ...
            return menu
                ...
                .flatMap(item ->
                    Stream.iterate(
                        ...
                        .filter(node ->
                            node.getStyleClass().contains("menu-item")
                        )
                        .findFirst()
                        .stream()
                )
    ...
    @Nullable
    private static Node findNodeByAction(
        @NonNull Scene scene,
        @NonNull StandardActions action
    ) {

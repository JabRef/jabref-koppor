...
public class WalkthroughScroller {
    ...
    private void scrollNodeIntoView(
        ...
        @NonNull List<Node> scrollableParents
    ) {
    ...
    private void scrollNodeIntoViewForParent(
        @NonNull Node targetNode,
        @NonNull Node scrollableParent
    ) {
        try {
            if (
            ...
            switch (scrollableParent) {
                case ScrollPane scrollPane -> {
                    Bounds targetBounds = targetNode.localToScene(
                        targetNode.getBoundsInLocal()
                    );
                    scrollIntoScrollPane(scrollPane, targetBounds);
                }
                case ListView<?> listView -> scrollIntoListView(
                    targetNode,
                    listView
                );
                case TableView<?> tableView -> scrollIntoTableView(
                    targetNode,
                    tableView
                );
                case TreeView<?> treeView -> scrollIntoTreeView(
                    targetNode,
                    treeView
                );
                default -> LOGGER.warn(
                    "Unsupported scrollable type: {}",
                    scrollableParent.getClass().getSimpleName()
                );
            }
        } catch (RuntimeException e) {
            LOGGER.warn(
                "Failed to scroll node into view for parent {}",
                scrollableParent.getClass().getSimpleName(),
                e
            );
        }
    ...
    private void scrollIntoScrollPane(
        @NonNull ScrollPane scrollPane,
        @NonNull Bounds targetBounds
    ) {
    ...
    private void scrollIntoListView(
        @NonNull Node targetNode,
        @NonNull ListView<?> listView
    ) {
        int index = findItemIndex(
            targetNode,
            listView,
    ...
    private void scrollIntoTableView(
        @NonNull Node targetNode,
    ...
    private void scrollIntoTreeView(
        @NonNull Node targetNode,
        @NonNull TreeView<?> treeView
    ) {
        int index = findItemIndex(
            targetNode,
            treeView,
    ...
    private int findItemIndex(
        ...
        Bounds targetBounds = container.sceneToLocal(
            targetNode.localToScene(targetNode.getBoundsInLocal())
        );
        ...
        if (itemHeight > 0) {
            ...
            return (int) Math.max(
                0,
                Math.min(itemCount - 1, targetCenterY / itemHeight)
            );
        }
        return -1;
    }

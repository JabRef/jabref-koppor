    ...
    public void attach(
        ...
        windowShowingListener = (_, wasShowing, isShowing) -> {
            if (wasShowing && !isShowing) {
                ...
                reversion = new DelayedExecution(
                    REVERSION_DELAY,
                    this::findAndUndo
                );
                reversion.start();
        ...
        for (int i = from - 1; i >= 0; i--) {
            switch (walkthrough.getStepAtIndex(i)) {
                case VisibleComponent component -> {
                    Optional<Window> window = component
                        .windowResolver()
                        .flatMap(WindowResolver::resolve);
                    if (
                        window.isEmpty()
                        && component.windowResolver().isPresent()
                    ) {
                        continue;
                    }
                    ...
                    if (
                        ...
                        && (component.nodeResolver().isEmpty()
                            || component
                                .nodeResolver()
                                .get()

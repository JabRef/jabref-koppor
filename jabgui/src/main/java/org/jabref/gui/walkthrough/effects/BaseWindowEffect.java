...
public abstract sealed class BaseWindowEffect
    permits Spotlight, FullScreenDarken, Ping {
    ...
    protected BaseWindowEffect(@NonNull Pane pane) {
        this.pane = pane;
        this.debouncedUpdater = WalkthroughUtils.debounced(_ ->
            this.updateLayout()
        );
    }
    ...
    protected void setupListeners(@NonNull Node node) {
        ...
        subscriptions.add(
            EasyBind.listen(node.boundsInLocalProperty(), debouncedUpdater)
        );
        subscriptions.add(
            EasyBind.listen(new TreeShowingProperty(node), debouncedUpdater)
        );

        Scene scene = node.getScene();
        if (scene != null) {
            subscriptions.add(
                EasyBind.listen(scene.widthProperty(), debouncedUpdater)
            );
            subscriptions.add(
                EasyBind.listen(scene.heightProperty(), debouncedUpdater)
            );
            setupWindowListeners(scene.getWindow());
        }

        subscriptions.add(
            EasyBind.listen(node.sceneProperty(), (_, _, newScene) -> {
                if (newScene != null) {
                    subscriptions.add(
                        EasyBind.listen(
    ...
    protected void setupPaneListeners() {
        subscriptions.add(
            EasyBind.listen(pane.widthProperty(), debouncedUpdater)
        );
        subscriptions.add(
            EasyBind.listen(pane.heightProperty(), debouncedUpdater)
        );
        subscriptions.add(
            EasyBind.listen(pane.layoutBoundsProperty(), debouncedUpdater)
        );
    }

    private void setupWindowListeners(@NonNull Window window) {
        subscriptions.add(
            EasyBind.listen(window.xProperty(), debouncedUpdater)
        );
        subscriptions.add(
            EasyBind.listen(window.yProperty(), debouncedUpdater)
        );
        subscriptions.add(
            EasyBind.listen(window.widthProperty(), debouncedUpdater)
        );
        subscriptions.add(
            EasyBind.listen(window.heightProperty(), debouncedUpdater)
        );
    }
}

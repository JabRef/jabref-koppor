...
public class FetchAndMergeEntry {
    ...
    // All identifiers listed here should also appear at {@link
    // org.jabref.logic.importer.CompositeIdFetcher#performSearchById}
    public static List<Field> SUPPORTED_FIELDS = Arrays.asList(StandardField.DOI, StandardField.EPRINT,
            StandardField.ISBN);
    ...
    public FetchAndMergeEntry(BibDatabaseContext bibDatabaseContext, TaskExecutor taskExecutor,
            GuiPreferences preferences, DialogService dialogService, UndoManager undoManager) {
    ...
    public void fetchAndMerge(BibEntry entry, List<Field> fields) {
        for (Field field : fields) {
            ...
            if (fieldContent.isPresent()) {
                Optional<IdBasedFetcher> fetcher = WebFetchers.getIdBasedFetcherForField(field,
                        preferences.getImportFormatPreferences());
                fetcher.ifPresent(idBasedFetcher -> BackgroundTask
                    .wrap(() -> idBasedFetcher.performSearchById(fieldContent.get()))
                    .onSuccess(fetchedEntry -> {
                        ...
                        if (fetchedEntry.isPresent()) {
                            ...
                            dialogService.notify(Localization.lang("Cannot get info based on given %0: %1", type,
                                    fieldContent.get()));
                        }
                    })
                    .onFailure(exception -> {
                        ...
                        if (exception instanceof FetcherClientException) {
                            dialogService.showInformationDialogAndWait(
                                    Localization.lang("Fetching information using %0", idBasedFetcher.getName()),
        ...
        if (mergedEntry.isPresent()) {
            ...
            for (Field field : jointFields) {
                ...
                if (originalString.isEmpty() || !originalString.equals(mergedString)) {
                    originalEntry.setField(field, mergedString.get()); // mergedString
                                                                       // always present
            ...
            // Remove fields which are not in the merged entry, unless they are internal
            // fields
            for (Field field : originalFields) {
                if (!jointFields.contains(field) && !FieldFactory.isInternalField(field)) {
                    Optional<String> originalString = originalEntry.getField(field);
                    originalEntry.clearField(field);
                    ce.addEdit(new UndoableFieldChange(originalEntry, field, originalString.get(), null)); // originalString
                                                                                                           // always
                                                                                                           // present
                    edited = true;
                }
    ...
    public void fetchAndMerge(BibEntry entry, EntryBasedFetcher fetcher) {
        BackgroundTask.wrap(() -> fetcher.performSearch(entry).stream().findFirst()).onSuccess(fetchedEntry -> {
            ...
            dialogService.showErrorDialogAndWait(Localization.lang("Error while fetching from %0", fetcher.getName()),
                    exception);
        }).executeWith(taskExecutor);
    }

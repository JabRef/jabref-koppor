...
public class CopyMoreAction extends SimpleCommand {
    ...
    public void execute() {
        ...
        switch (action) {
            case COPY_CITATION_KEY -> copyKey();
            case COPY_AS_CITE_COMMAND -> copyCiteKey();
            case COPY_CITATION_KEY_AND_TITLE -> copyKeyAndTitle();
            case COPY_CITATION_KEY_AND_LINK -> copyKeyAndLink();
            case COPY_DOI, COPY_DOI_URL -> copyDoi();
            case COPY_FIELD_AUTHOR -> copyField(
                StandardField.AUTHOR,
                Localization.lang("Author")
            );
            case COPY_FIELD_TITLE -> copyField(
                StandardField.TITLE,
                Localization.lang("Title")
            );
            case COPY_FIELD_JOURNAL -> copyField(
                StandardField.JOURNAL,
                Localization.lang("Journal")
            );
            case COPY_FIELD_DATE -> copyField(
                StandardField.DATE,
                Localization.lang("Date")
            );
            case COPY_FIELD_KEYWORDS -> copyField(
                StandardField.KEYWORDS,
                Localization.lang("Keywords")
            );
            case COPY_FIELD_ABSTRACT -> copyField(
    ...
    private void copyCiteKey() {
        doCopyKey(keys -> {
            CitationCommandString citeCommand = preferences
    ...
    private void copyKeyAndLink() {
        ...
        for (BibEntry entry : entriesWithKey) {
            ...
            if (LOGGER.isDebugEnabled() && key.isEmpty()) {
                LOGGER.debug(
                    "entry {} had no citation key, but it should have had one",
                    entry
                );
            }
            ...
            fallbackString.append(
                url.isEmpty() ? key : "%s - %s".formatted(key, url)
            );
    ...
    private void copyField(StandardField field, String fieldDisplayName) {
        ...
        final String copiedContent = fieldValues
            .stream()
        ...
        if (fieldValues.size() == selectedBibEntries.size()) {
            ...
            dialogService.notify(
                Localization.lang(
                    ...
                    Integer.toString(
                        selectedBibEntries.size() - fieldValues.size()
                    ),

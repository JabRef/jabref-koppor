...
public class CopyTo extends SimpleCommand {
    ...
    public void execute() {
        ...
        List<BibEntry> selectedEntries = stateManager
            .getSelectedEntries()
            .stream()
            .toList();

        boolean includeCrossReferences =
            copyToPreferences.getShouldIncludeCrossReferences();
        boolean showDialogBox =
            copyToPreferences.getShouldAskForIncludingCrossReferences();
        ...
        for (BibEntry bibEntry : selectedEntries) {
            if (bibEntry.hasField(StandardField.CROSSREF) && showDialogBox) {
                ...
                copyToPreferences.setShouldIncludeCrossReferences(
    ...
    public void copyEntriesWithCrossRef(
        ...
        copyEntriesWithFeedback(
            ...
            Localization.lang(
                "Copied %0 entry(s) to %1, including cross-references"
            ),
            Localization.lang(
    ...
    public void copyEntriesWithoutCrossRef(
        List<BibEntry> selectedEntries,
        BibDatabaseContext targetDatabaseContext
    ) {
        copyEntriesWithFeedback(
            ...
            targetDatabaseContext,
            Localization.lang(
                "Copied %0 entry(s) to %1 without cross-references"
            ),
            Localization.lang(
    ...
    private void copyEntriesWithFeedback(
        ...
        tracker.setOnFinish(() -> {
            ...
            String targetName = targetDatabaseContext
                .getDatabasePath()
            ...
            if (importedCount == entriesToAdd.size()) {
                ...
                dialogService.notify(
                    Localization.lang("No entry was copied to %0", targetName)
                );
            } else {
                dialogService.notify(
                    Localization.lang(
    ...
    public Optional<BibEntry> getCrossRefEntry(
        BibEntry bibEntryToCheck,
        BibDatabaseContext sourceDatabaseContext
    ) {
        return sourceDatabaseContext
            .getEntries()
            .stream()
            .filter(entry ->
                bibEntryToCheck
                    .getField(StandardField.CROSSREF)
                    .equals(entry.getCitationKey())
            )
            .findFirst();
    }
    ...
    private boolean askForCrossReferencedEntries() {
        if (copyToPreferences.getShouldAskForIncludingCrossReferences()) {
            return dialogService.showConfirmationDialogWithOptOutAndWait(
                Localization.lang(
                    "Include or exclude cross-referenced entries"
                ),
                Localization.lang(
                    "Would you like to include cross-reference entries in the current operation?"
                ),
                Localization.lang("Include"),
                Localization.lang("Exclude"),
                Localization.lang("Do not ask again"),
                optOut ->
                    copyToPreferences.setShouldAskForIncludingCrossReferences(
                        !optOut
                    )
            );
        } else {

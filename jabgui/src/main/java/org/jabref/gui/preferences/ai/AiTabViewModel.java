...
public class AiTabViewModel implements PreferenceTabViewModel {
    ...
    private final BooleanProperty disableAutoGenerateEmbeddings =
        new SimpleBooleanProperty();
    private final BooleanProperty autoGenerateSummaries =
        new SimpleBooleanProperty();
    private final BooleanProperty disableAutoGenerateSummaries =
        new SimpleBooleanProperty();
    ...
    private final ObjectProperty<AiProvider> selectedAiProvider =
    ...
    private final BooleanProperty disableApiBaseUrl = new SimpleBooleanProperty(
        true
    ); // {@link HuggingFaceChatModel} and {@link GoogleAiGeminiChatModel} doesn't support setting API base URL
    ...
    private final OptionalObjectProperty<AiTemplate> selectedTemplate =
    ...
    private final BooleanProperty disableBasicSettings =
        new SimpleBooleanProperty(true);
    private final BooleanProperty disableExpertSettings =
        new SimpleBooleanProperty(true);
    ...
    public AiTabViewModel(CliPreferences preferences) {
        ...
        this.enableAi.addListener((_, _, newValue) -> {
            ...
            disableExpertSettings.set(
                !newValue || !customizeExpertSettings.get()
            );
        });
        ...
        this.selectedAiProvider.addListener((_, oldValue, newValue) -> {
            List<String> models = AiDefaultPreferences.getAvailableModels(
                newValue
            );
        ...
        this.temperatureTypeValidator = new FunctionBasedValidator<>(
            ...
            ValidationMessage.error(
                Localization.lang("Temperature must be a number")
            )
        );
        ...
        this.ragMaxResultsCountValidator = new FunctionBasedValidator<>(
            ...
            ValidationMessage.error(
                Localization.lang(
                    "RAG max results count must be greater than 0"
                )
            )
        );
    ...
    public void setValues() {
        openAiApiKey.setValue(
            aiPreferences.getApiKeyForAiProvider(AiProvider.OPEN_AI)
        );
        mistralAiApiKey.setValue(
            aiPreferences.getApiKeyForAiProvider(AiProvider.MISTRAL_AI)
        );
        geminiAiApiKey.setValue(
            aiPreferences.getApiKeyForAiProvider(AiProvider.GEMINI)
        );
        huggingFaceApiKey.setValue(
            aiPreferences.getApiKeyForAiProvider(AiProvider.HUGGING_FACE)
        );
        gpt4AllApiKey.setValue(
            aiPreferences.getApiKeyForAiProvider(AiProvider.GPT4ALL)
        );
        ...
        huggingFaceApiBaseUrl.setValue(
            aiPreferences.getHuggingFaceApiBaseUrl()
        );
        ...
        autoGenerateEmbeddings.setValue(
            aiPreferences.getAutoGenerateEmbeddings()
        );

        selectedAiProvider.setValue(aiPreferences.getAiProvider());

        customizeExpertSettings.setValue(
            aiPreferences.getCustomizeExpertSettings()
        );
        ...
        Arrays.stream(AiTemplate.values()).forEach(template ->
            templateSources
                .get(template)
                .set(aiPreferences.getTemplate(template))
        );

        temperature.setValue(
            LocalizedNumbers.doubleToString(aiPreferences.getTemperature())
        );
        contextWindowSize.setValue(aiPreferences.getContextWindowSize());
        documentSplitterChunkSize.setValue(
            aiPreferences.getDocumentSplitterChunkSize()
        );
        documentSplitterOverlapSize.setValue(
            aiPreferences.getDocumentSplitterOverlapSize()
        );
        ragMaxResultsCount.setValue(aiPreferences.getRagMaxResultsCount());
        ragMinScore.setValue(
            LocalizedNumbers.doubleToString(aiPreferences.getRagMinScore())
        );
    }
    ...
    public void storeSettings() {
        ...
        aiPreferences.setGpt4AllChatModel(
            gpt4AllChatModel.get() == null ? "" : gpt4AllChatModel.get()
        );

        aiPreferences.storeAiApiKeyInKeyring(
            AiProvider.OPEN_AI,
            openAiApiKey.get() == null ? "" : openAiApiKey.get()
        );
        aiPreferences.storeAiApiKeyInKeyring(
        ...
        aiPreferences.setGpt4AllApiBaseUrl(
            gpt4AllApiBaseUrl.get() == null ? "" : gpt4AllApiBaseUrl.get()
        );

        Arrays.stream(AiTemplate.values()).forEach(template ->
            aiPreferences.setTemplate(
                template,
                templateSources.get(template).get()
            )
        );
        ...
        aiPreferences.setDocumentSplitterChunkSize(
            documentSplitterChunkSize.get()
        );
        aiPreferences.setDocumentSplitterOverlapSize(
            documentSplitterOverlapSize.get()
        );
    ...
    public void resetExpertSettings() {
        ...
        contextWindowSize.set(
            AiDefaultPreferences.getContextWindowSize(
                selectedAiProvider.get(),
                currentChatModel.get()
            )
        );

        temperature.set(
            LocalizedNumbers.doubleToString(AiDefaultPreferences.TEMPERATURE)
        );
        documentSplitterChunkSize.set(
            AiDefaultPreferences.DOCUMENT_SPLITTER_CHUNK_SIZE
        );
        documentSplitterOverlapSize.set(
            AiDefaultPreferences.DOCUMENT_SPLITTER_OVERLAP
        );
        ragMaxResultsCount.set(AiDefaultPreferences.RAG_MAX_RESULTS_COUNT);
        ragMinScore.set(
            LocalizedNumbers.doubleToString(AiDefaultPreferences.RAG_MIN_SCORE)
        );
    }
    ...
    public void resetTemplates() {
        Arrays.stream(AiTemplate.values()).forEach(template ->
            templateSources
                .get(template)
                .set(AiDefaultPreferences.TEMPLATES.get(template))
        );
    }
    ...
    public void resetCurrentTemplate() {
        selectedTemplateProperty()
            .get()
            .ifPresent(template -> {
                String defaultTemplate = AiDefaultPreferences.TEMPLATES.get(
                    template
                );

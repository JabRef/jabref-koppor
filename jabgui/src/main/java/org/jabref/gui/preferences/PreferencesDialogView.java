...
/**
 * Preferences dialog. Contains a TabbedPane, and tabs will be defined in separate
 * classes. Tabs MUST implement the PreferencesTab interface, since this dialog will call
 * the storeSettings() method of all tabs when the user presses ok.
 */
public class PreferencesDialogView extends BaseDialog<PreferencesDialogViewModel> {
    ...
    public PreferencesDialogView(Class<? extends PreferencesTab> preferencesTabToSelectClass) {
        ...
        // Stop the default button from firing when the user hits enter within the search
        // box
    ...
    private void initialize() {
        ...
        if (this.preferencesTabToSelectClass != null) {
            Optional<PreferencesTab> tabToSelectIfExist = preferenceTabList.getItems()
                ...
                .findFirst();
            tabToSelectIfExist
                .ifPresent(preferencesTab -> preferenceTabList.getSelectionModel().select(preferencesTab));
        }
        else {
            preferenceTabList.getSelectionModel().selectFirst();
        }

        new ViewModelListCellFactory<PreferencesTab>().withText(PreferencesTab::getTabName).install(preferenceTabList);

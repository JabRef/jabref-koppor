...
public class KeyBindingsTab
    ...
    private void initialize() {
        viewModel = new KeyBindingsTabViewModel(
        ...
        keyBindingsTable
            .rootProperty()
            .bind(
                EasyBind.map(viewModel.rootKeyBindingProperty(), keybinding ->
        ...
        new ViewModelTreeTableCellFactory<KeyBindingViewModel>()
            ...
            .withOnMouseClickedEvent(
                keyBinding -> evt -> keyBinding.resetToDefault()
            )
            .install(resetColumn);
        new ViewModelTreeTableCellFactory<KeyBindingViewModel>()
            .withGraphic(keyBinding ->
        ...
        searchBox
            ...
            .addListener((observable, previousText, searchTerm) -> {
                ...
                setCategoriesExpanded(
                    !searchTerm.isEmpty() || previousText.isEmpty()
                );
            });
    ...
    private void setCategoriesExpanded(boolean expanded) {
        for (TreeItem<KeyBindingViewModel> child : keyBindingsTable
            .getRoot()
            .getChildren()) {

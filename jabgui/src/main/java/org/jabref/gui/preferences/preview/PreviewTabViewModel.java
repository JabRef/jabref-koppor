...
public class PreviewTabViewModel implements PreferenceTabViewModel {
    ...
    public PreviewTabViewModel(DialogService dialogService, PreviewPreferences previewPreferences,
        ...
        chosenListValidator = new FunctionBasedValidator<>(chosenListProperty,
                input -> !chosenListProperty.getValue().isEmpty(),
                ValidationMessage.error("%s > %s %n %n %s".formatted(Localization.lang("Entry preview"),
                        Localization.lang("Selected"), Localization.lang("Selected Layouts can not be empty"))));
    }
    ...
    public void setValues() {
        ...
        BackgroundTask.wrap(CSLStyleLoader::getStyles)
            .onSuccess(styles -> styles.stream()
                ...
                .filter(style -> chosenListProperty.getValue()
    ...
    private PreviewLayout findLayoutByName(String name) {
        return availableListProperty.getValue()
            .stream()
            .filter(layout -> layout.getName().equals(name))
            .findAny()
            .orElse(chosenListProperty.getValue()
                .stream()
                .filter(layout -> layout.getName().equals(name))
                .findAny()
                .orElse(null));
    ...
    public void storeSettings() {
        ...
        if (!chosenSelectionModelProperty.getValue().getSelectedItems().isEmpty()) {
            previewPreferences.setLayoutCyclePosition(chosenListProperty.getValue()
                .indexOf(chosenSelectionModelProperty.getValue().getSelectedItems().getFirst()));
        }
    }
    ...
    /**
     * XML-Syntax-Highlighting for RichTextFX-Codearea created by (c) Carlos Martins
     * (github: <a href="https://github.com/cmartins">@cemartins</a>)
     * <p>
     * License:
     * <a href="https://github.com/FXMisc/RichTextFX/blob/master/LICENSE">BSD-2-Clause</a>
     * <p>
     * See also <a href=
     * "https://github.com/FXMisc/RichTextFX/blob/master/richtextfx-demos/README.md#xml-editor">https://github.com/FXMisc/RichTextFX/blob/master/richtextfx-demos/README.md#xml-editor</a>
     * @param text to parse and highlight
     * @return highlighted span for codeArea
     */
    public StyleSpans<Collection<String>> computeHighlighting(String text) {
        final Pattern XML_TAG = Pattern
                ...
                if (matcher.group("ELEMENT") != null) {
                    ...
                    if (!attributesText.isEmpty()) {
                        ...
                        while (attributesMatcher.find()) {
                            ...
                            spansBuilder.add(Set.of("attribute"), attributesMatcher.end(GROUP_ATTRIBUTE_NAME)
                                    - attributesMatcher.start(GROUP_ATTRIBUTE_NAME));
                            spansBuilder.add(Set.of("tagmark"), attributesMatcher.end(GROUP_EQUAL_SYMBOL)
                                    - attributesMatcher.end(GROUP_ATTRIBUTE_NAME));
                            spansBuilder.add(Set.of("avalue"), attributesMatcher.end(GROUP_ATTRIBUTE_VALUE)
    ...
    public void dragDetected(ListProperty<PreviewLayout> sourceList,
            ObjectProperty<MultipleSelectionModel<PreviewLayout>> sourceSelectionModel,
            List<PreviewLayout> selectedLayouts, Dragboard dragboard) {
    ...
    /**
     * This is called, when the user drops some PreviewLayouts either in the
     * availableListView or in the empty space of chosenListView
    ...
    public void setAvailableFilter(String searchTerm) {
        this.filteredAvailableLayouts
            .setPredicate(preview -> searchTerm.isEmpty() || preview.containsCaseIndependent(searchTerm));
    }

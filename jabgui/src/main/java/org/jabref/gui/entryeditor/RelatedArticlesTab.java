...
public class RelatedArticlesTab extends EntryEditorTab {
    ...
    public RelatedArticlesTab(BuildInfo buildInfo, GuiPreferences preferences, DialogService dialogService,
    ...
    /**
     * Gets a StackPane of related article information to be displayed in the Related
     * Articles tab
     * @param entry The currently selected BibEntry on the JabRef UI.
     * @return A StackPane with related article information to be displayed in the Related
     * Articles tab.
     */
    private StackPane getRelatedArticlesPane(BibEntry entry) {
        ...
        MrDLibFetcher fetcher = new MrDLibFetcher(preferences.getWorkspacePreferences().getLanguage().name(),
                buildInfo.version, preferences.getMrDlibPreferences());
    ...
    /**
     * Creates a VBox of the related article information to be used in the StackPane
     * displayed in the Related Articles tab
     * @param list List of BibEntries of related articles
     * @return VBox of related article descriptions to be displayed in the Related
     * Articles tab
     */
        ...
        for (BibEntry entry : list) {
            ...
            titleLink.setOnAction(event -> {
                if (entry.getField(StandardField.URL).isPresent()) {
                    try {
                        NativeDesktop.openBrowser(entry.getField(StandardField.URL).get(),
     ...
     * @param entry Currently selected BibEntry. (required to allow reloading of pane if
     * accepted)
    ...
    private ScrollPane getPrivacyDialog(BibEntry entry) {
        ...
        Text line1 = new Text(Localization.lang(
        ...
        Text line2 = new Text(Localization.lang(
        ...
        CheckBox cbVersion = new CheckBox(Localization
            .lang("JabRef Version (Required to ensure backwards compatibility with Mr. DLib's Web Service)"));
        cbVersion.setSelected(true);
        cbVersion.setDisable(true);
        CheckBox cbLanguage = new CheckBox(Localization.lang(
                "JabRef Language (Provides for better recommendations by giving an indication of user's preferred language.)"));
        CheckBox cbOS = new CheckBox(Localization.lang(
                "Operating System (Provides for better recommendations by giving an indication of user's system set-up.)"));
        CheckBox cbTimezone = new CheckBox(Localization.lang(
        ...
        button.setOnAction(event -> {
            ...
            dialogService.showWarningDialogAndWait(Localization.lang("Restart"),

...
public class LatexCitationsTabViewModel extends AbstractViewModel {
    ...
    private static final IOFileFilter FILE_FILTER = FileFilterUtils.or(
    ...
    public void handleMouseClick(
        MouseEvent event,
        CitationsDisplay citationsDisplay
    ) {
        Citation selectedItem = citationsDisplay
            .getSelectionModel()
            .getSelectedItem();
        ...
        if (
            ...
            application.jumpToLine(
                selectedItem.path(),
                selectedItem.line(),
                selectedItem.colStart()
            );
        }
    }
    ...
    public void bindToEntry(LibraryTab libraryTab, BibEntry entry) {
        ...
        if (citationKey.isPresent()) {
            citationList.setAll(
                latexFiles.getCitationsByKey(citationKey.get())
            );
    ...
    private void checkAndUpdateDirectory() {
        Path newDirectory = currentDatabaseContext
            .getMetaData()
    ...
    private class CitationsAlterationListener
        ...
        public void onFileCreate(File file) {
            ...
            if (citationKey.isPresent()) {
                Collection<Citation> citations = result.getCitationsByKey(
                    citationKey.get()
                );
                UiTaskExecutor.runInJavaFXThread(() ->
                    citationList.addAll(citations)
                );
            }
        ...
        public void onFileDelete(File file) {
            ...
            if (citationKey.isPresent()) {
                ...
                UiTaskExecutor.runInJavaFXThread(() ->
                    citationList.removeAll(citations)
                );
                updateStatus();
            }
        }

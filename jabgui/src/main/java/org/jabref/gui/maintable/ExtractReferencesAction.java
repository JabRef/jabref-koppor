...
public class ExtractReferencesAction extends SimpleCommand {
    ...
    public ExtractReferencesAction(DialogService dialogService, StateManager stateManager, CliPreferences preferences) {
        this(dialogService, stateManager, preferences, null, null);
    }

    /**
     * Can be used to bind the action on a context menu in the linked file view (future
     * work)
     * @param entry the entry to handle (can be null)
     * @param linkedFile the linked file (can be null)
     */
    private ExtractReferencesAction(@NonNull DialogService dialogService, @NonNull StateManager stateManager,
            @NonNull CliPreferences preferences, @Nullable BibEntry entry, @Nullable LinkedFile linkedFile) {
    ...
    private void extractReferences() {
        stateManager.getActiveDatabase().ifPresent(databaseContext -> {
            ...
            if (online) {
                Optional<Callable<ParserResult>> parserResultCallableOnline = getParserResultCallableOnline(
    ...
    private @NonNull Callable<ParserResult> getParserResultCallableOffline(BibDatabaseContext databaseContext,
            List<BibEntry> selectedEntries) {
        return () -> {
            BibEntry currentEntry = selectedEntries.getFirst();
            List<Path> fileList = FileUtil.getListOfLinkedFiles(selectedEntries,
                    databaseContext.getFileDirectories(preferences.getFilePreferences()));

            // We need to have ParserResult handled at the importer, because it imports
            // the meta data (library type, encoding, ...)
            ...
            while (selectedEntriesIterator.hasNext()) {
                ...
                fileList = FileUtil.getListOfLinkedFiles(List.of(currentEntry),
                        databaseContext.getFileDirectories(preferences.getFilePreferences()));
    ...
    private void extractReferences(Iterator<Path> fileListIterator, ParserResult result, BibEntry currentEntry) {
        while (fileListIterator.hasNext()) {
            result.getDatabase()
                .insertEntries(
                        bibliographyFromPdfImporter.importDatabase(fileListIterator.next()).getDatabase().getEntries());
        }
    ...
    /**
     * Creates the field content for the "cites" field. The field contains the citation
     * keys of the imported entries.
     *
     * TODO: Move this part to logic somehow
     * @param currentEntry used to create citation keys if the importer did not provide
     * one from the imported entry
     */
    private static String getCites(List<BibEntry> entries, BibEntry currentEntry) {
        ...
        for (BibEntry importedEntry : entries) {
            ...
            if (citationKey.isPresent()) {
                ...
                // the number of the current entry (extracted from the reference;
                // fallback: current number of the entry (count variable))
                ...
                // We use the number of the comment field (because there is no other way
                // to get the number reliable)
    ...
    private Optional<Callable<ParserResult>> getParserResultCallableOnline(BibDatabaseContext databaseContext,
            List<BibEntry> selectedEntries) {
        List<Path> fileList = FileUtil.getListOfLinkedFiles(selectedEntries,
                databaseContext.getFileDirectories(preferences.getFilePreferences()));
        ...
        return Optional.of(() -> new ParserResult(new GrobidService(this.preferences.getGrobidPreferences())
            .processReferences(fileList, preferences.getImportFormatPreferences())));
    }

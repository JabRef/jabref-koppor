...
public class MainTableHeaderContextMenu extends ContextMenu {
    ...
    private void constructItems() {
        ...
        > column : mainTable.getColumns()) {
            ...
            if (isACommonColumn(searchCol)) {
                commonColumns.removeIf(tableCol ->
                    ((MainTableColumn<?>) tableCol).getModel().equals(
                        searchCol.getModel()
                    )
                );
            }
        ...
        if (!commonColumns.isEmpty()) {
            ...
            > tableColumn : commonColumns) {
                RightClickMenuItem itemToAdd = createMenuItem(
                    tableColumn,
                    false
                );
                this.getItems().add(itemToAdd);
        ...
        MenuItem showMoreItem = actionfactory.createMenuItem(
            StandardActions.SHOW_PREFS.withText(
                Localization.lang("More options...")
            ),
    ...
    private int obtainIndexOfColumn(MainTableColumn<?> searchColumn) {
        ...
        for (int i = 0; i < columns.size(); i++) {
            ...
            MainTableColumnModel model = ((MainTableColumn<
                    ?
                >) column).getModel();
    ...
    private void addColumn(MainTableColumn<?> tableColumn, int index) {
        if (
            (index <= OUT_OF_BOUNDS) || (index >= mainTable.getColumns().size())
        ) {

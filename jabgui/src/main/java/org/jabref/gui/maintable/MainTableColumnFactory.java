...
public class MainTableColumnFactory {
    ...
    public MainTableColumnFactory(
        ...
        ThemeManager themeManager = Injector.instantiateModelOrService(
            ThemeManager.class
        );
        this.tooltip = new MainTableTooltip(
            dialogService,
            preferences,
            themeManager,
            taskExecutor
    ...
    public List<TableColumn<BibEntryTableViewModel, ?>> createColumns() {
        ...
        columnPreferences
            .getColumns()
            .forEach(column -> columns.add(createColumn(column)));
        return columns;
    }
    ...
    private TableColumn<BibEntryTableViewModel, ?> createGroupIconColumn(
        ...
        TableColumn<BibEntryTableViewModel, List<AbstractGroup>> column =
            new MainTableColumn<>(columnModel);
        Node headerGraphic =
            IconTheme.JabRefIcons.DEFAULT_GROUP_ICON_COLUMN.getGraphicNode();
        Tooltip.install(
            headerGraphic,
            new Tooltip(MainTableColumnModel.Type.GROUP_ICONS.getDisplayName())
        );
        column.setGraphic(headerGraphic);
        column.getStyleClass().add(STYLE_ICON_COLUMN);
    ...
    private Node createGroupIconRegion(
        ...
        List<JabRefIcon> groupIcons = matchedGroups
            .stream()
            .filter(abstractGroup -> abstractGroup.getIconName().isPresent())
            .flatMap(group ->
                IconTheme.findIcon(
                    group.getIconName().get(),
                    group.getColor().orElse(IconTheme.getDefaultGroupColor())
                ).stream()
            )
            .toList();

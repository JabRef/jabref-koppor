...
public class DatabaseChangeDetailsViewFactory {
    ...
    public DatabaseChangeDetailsViewFactory(BibDatabaseContext databaseContext, DialogService dialogService,
            ThemeManager themeManager, GuiPreferences preferences, BibEntryTypesManager entryTypesManager,
            PreviewViewer previewViewer, TaskExecutor taskExecutor) {
    ...
    public DatabaseChangeDetailsView create(DatabaseChange databaseChange) {
        return switch (databaseChange) {
            case EntryChange entryChange ->
                new EntryChangeDetailsView(entryChange.getOldEntry(), entryChange.getNewEntry(), databaseContext,
                        dialogService, themeManager, preferences, entryTypesManager, previewViewer, taskExecutor);
            case EntryAdd entryAdd -> new EntryWithPreviewAndSourceDetailsView(entryAdd.getAddedEntry(),
                    databaseContext, preferences, entryTypesManager, previewViewer);
            case EntryDelete entryDelete -> new EntryWithPreviewAndSourceDetailsView(entryDelete.getDeletedEntry(),
                    databaseContext, preferences, entryTypesManager, previewViewer);
            ...
            case MetadataChange metadataChange -> new MetadataChangeDetailsView(metadataChange,

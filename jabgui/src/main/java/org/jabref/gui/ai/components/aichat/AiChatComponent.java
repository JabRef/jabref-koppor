...
public class AiChatComponent extends VBox {
    ...
    // Example Questions
    private static final String EXAMPLE_QUESTION_1 = Localization.lang(
        "What is the goal of the paper?"
    );
    private static final String EXAMPLE_QUESTION_2 = Localization.lang(
        "Which methods were used in the research?"
    );
    private static final String EXAMPLE_QUESTION_3 = Localization.lang(
    ...
    public AiChatComponent(
        ...
        aiService
            ...
            .ingest(
                name,
                ListUtil.getLinkedFiles(entries).toList(),
                bibDatabaseContext
            );
    ...
    private void initializeChatPrompt() {
        notificationsButton.setOnAction(event ->
            new PopOver(new NotificationsComponent(notifications)).show(
                notificationsButton
            )
        );
        ...
        if (!notifications.isEmpty()) {
            UiTaskExecutor.runInJavaFXThread(() ->
                notificationsButton.setGraphic(
                    IconTheme.JabRefIcons.WARNING.withColor(
                        Color.YELLOW
                    ).getGraphicNode()
                )
            );
    ...
    private List<Notification> updateNotificationsForEntry(BibEntry entry) {
        ...
        entry
            ...
            .stream()
            .map(file ->
                aiService.getIngestionService().ingest(file, bibDatabaseContext)
            )
            .forEach(ingestionStatus -> {
                switch (ingestionStatus.getState()) {
                    case PROCESSING -> notifications.add(
                        new Notification(
                            Localization.lang(
                            ...
                            Localization.lang(
                                "After the file is ingested, you will be able to chat with it."
                            )
                        )
                    );
                    case ERROR -> {
    ...
    private void onSendMessage(String userPrompt) {
        ...
        BackgroundTask<AiMessage> task = BackgroundTask.wrap(() ->
            aiChatLogic.execute(userMessage)
        )
            .showToUser(true)
            .onSuccess(aiMessage -> {
                setLoading(false);

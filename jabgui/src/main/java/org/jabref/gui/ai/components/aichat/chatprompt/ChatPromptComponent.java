...
public class ChatPromptComponent extends HBox {
    ...
    private final ObjectProperty<Consumer<String>> sendCallback =
        new SimpleObjectProperty<>();
    private final ObjectProperty<Consumer<String>> retryCallback =
        new SimpleObjectProperty<>();
    private final ObjectProperty<Runnable> cancelCallback =
        new SimpleObjectProperty<>();

    private final ListProperty<String> history = new SimpleListProperty<>(
    ...
    private final BooleanProperty showingHistoryMessage =
    ...
    private void initialize() {
        userPromptTextArea.setOnKeyPressed(keyEvent -> {
            if (keyEvent.getCode() == KeyCode.DOWN) {
                ...
                if (
                    currentUserMessageScroll.get() != NEW_NON_EXISTENT_MESSAGE
                ) {
                    showingHistoryMessage.set(true);
                    currentUserMessageScroll.set(
                        currentUserMessageScroll.get() - 1
                    );
                ...
                if (
                    ...
                    if (
                        ...
                        history
                            ...
                            .findFirst()
                            .ifPresent(message ->
                                userPromptTextArea.setText(message)
                            );
                    }
                } else {

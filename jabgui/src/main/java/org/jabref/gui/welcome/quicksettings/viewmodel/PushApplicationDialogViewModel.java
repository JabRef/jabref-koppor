...
public class PushApplicationDialogViewModel extends AbstractViewModel {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(
        PushApplicationDialogViewModel.class
    );

    private final ListProperty<GuiPushToApplication> applicationsProperty =
        new SimpleListProperty<>(FXCollections.observableArrayList());
    private final ObjectProperty<
        GuiPushToApplication
    > selectedApplicationProperty = new SimpleObjectProperty<>();
    ...
    private final Map<PushToApplication, String> detectedApplicationPaths =
        new ConcurrentHashMap<>();
    private Future<?> detectionFuture;

    public PushApplicationDialogViewModel(
        ...
        this.pushToApplicationPreferences =
    ...
    private void initializeApplications() {
        List<GuiPushToApplication> allApplications =
            GuiPushToApplications.getAllGUIApplications(
                dialogService,
        ...
        if (
            !pushToApplicationPreferences.getActiveApplicationName().isEmpty()
        ) {
            allApplications
                .stream()
                .filter(app ->
    ...
    private void detectApplications() {
        detectionFuture = BackgroundTask.wrap(() ->
            PushToApplicationDetector.detectApplicationPaths(
                applicationsProperty
            )
        )
            .onSuccess(detectedPaths -> {
                detectedApplicationPaths.putAll(detectedPaths);
                List<GuiPushToApplication> sortedApplications = new ArrayList<>(
                    detectedPaths.keySet()
                );
                applicationsProperty
                    .stream()
                ...
                LOGGER.info(
                    "Application detection completed. Found {} applications",
                    detectedPaths.size()
                );
            })
            .onFailure(throwable ->
    ...
    private void setupPathUpdates() {
        selectedApplicationProperty.addListener((_, _, selectedApp) -> {
            ...
            String existingPath = pushToApplicationPreferences
                .getCommandPaths()
                .get(selectedApp.getDisplayName());
            pathProperty.set(
                PushToApplicationDetector.isValidAbsolutePath(existingPath)
                    ? existingPath
                    : Objects.requireNonNullElse(
                        detectedApplicationPaths.get(selectedApp),
                        ""
                    )
            );
        });
    ...
    public void browseForApplication() {
        FileDialogConfiguration fileConfig =
    ...
    public boolean isValidConfiguration() {
        ...
        return (
            !pathText.isEmpty() && path.isAbsolute() && path.toFile().exists()
        );
    }
    ...
    public void saveSettings() {
        ...
        pushToApplicationPreferences.setActiveApplicationName(

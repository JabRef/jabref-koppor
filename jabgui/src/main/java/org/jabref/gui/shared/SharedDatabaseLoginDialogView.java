...
public class SharedDatabaseLoginDialogView extends BaseDialog<Void> {
    ...
    public SharedDatabaseLoginDialogView(LibraryTabContainer tabContainer) {
        ...
        ControlHelper.setAction(connectButton, this.getDialogPane(), event ->
            openDatabase()
        );
        Button btnConnect = (Button) this.getDialogPane().lookupButton(
            connectButton
        );
        ...
        btnConnect
            .textProperty()
            .bind(
                EasyBind.map(viewModel.loadingProperty(), loading ->
                    loading
    ...
    private void initialize() {
        ...
        expertMode
            .selectedProperty()
            .bindBidirectional(viewModel.expertModeProperty());

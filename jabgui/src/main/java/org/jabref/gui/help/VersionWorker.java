...
/**
 * This worker checks if there is a new version of JabRef available. If there is it will
 * display a dialog to the user offering him multiple options to proceed (see changelog,
 * go to the download page, ignore this version, and remind later).
 *
 * If the versions check is executed manually and this is the latest version it will also
...
public class VersionWorker {
    ...
    public VersionWorker(Version installedVersion, DialogService dialogService, TaskExecutor taskExecutor,
    ...
    /**
     * Returns a newer version excluding any non-stable versions, except if the installed
     * one is unstable too. If no newer version was found, then an empty optional is
     * returned.
     */
    ...
    /**
     * Prints the connection problem to the status bar and shows a dialog if it was
     * executed manually
     */
    ...
    /**
     * Prints up-to-date to the status bar (and shows a dialog it was executed manually)
     * if there is now new version. Shows a "New Version" Dialog to the user if there is.
     */
    private void showUpdateInfo(Optional<Version> newerVersion, boolean manualExecution) {
        // no new version could be found, only respect the ignored version on automated
        // version checks
        if (newerVersion.isEmpty()
            ...
            if (dialogService
                .showCustomDialogAndWait(new NewVersionDialog(installedVersion, newerVersion.get(), dialogService,

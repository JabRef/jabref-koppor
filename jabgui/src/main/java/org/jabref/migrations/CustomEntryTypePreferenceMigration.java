...
class CustomEntryTypePreferenceMigration {
    ...
    static void upgradeStoredBibEntryTypes(BibDatabaseMode defaultBibDatabaseMode, JabRefCliPreferences preferences,
    ...
    /**
     * Retrieves all deprecated information about the entry type in preferences, with the
     * tag given by number.
     * <p>
    ...
    private static Optional<BibEntryType> getBibEntryType(int number, JabRefCliPreferences preferences) {
        ...
        BibEntryTypeBuilder entryTypeBuilder = new BibEntryTypeBuilder().withType(EntryTypeFactory.parse(name))
            .withRequiredFields(
                    req.stream().map(FieldFactory::parseOrFields).collect(Collectors.toCollection(LinkedHashSet::new)));
        if (priOpt.isEmpty()) {
            entryTypeBuilder = entryTypeBuilder.withImportantFields(
                    opt.stream().map(FieldFactory::parseField).collect(Collectors.toCollection(LinkedHashSet::new)));
            ...
            entryTypeBuilder = entryTypeBuilder.withImportantFields(
                    priOpt.stream().map(FieldFactory::parseField).collect(Collectors.toCollection(LinkedHashSet::new)))
                .withDetailFields(secondary.stream()

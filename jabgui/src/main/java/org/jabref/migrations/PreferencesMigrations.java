...
public class PreferencesMigrations {
    ...
    /**
     * Perform checks and changes for users with a preference set from an older JabRef
     * version.
     */
    public static void runMigrations(JabRefGuiPreferences preferences) {
        ...
        // changeColumnVariableNamesFor51 needs to be run before
        // upgradeColumnPre50Preferences to ensure
        // backward compatibility, as it copies the old values to new variable names and
        // keeps th old sored with the old
        // variable names. However, the variables from 5.0 need to be copied to the new
        // variable name too.
    ...
    /**
     * Upgrade the sort order preferences for the current version The old preference is
     * kept in case an old version of JabRef is used with these preferences, but it is
     * only used when the new preference does not exist
     */
     ...
     */
    private static void upgradeStoredBibEntryTypes(JabRefCliPreferences prefs, Preferences mainPrefsNode,
    ...
    private static void upgradeLabelPatternToCitationKeyPattern(JabRefCliPreferences prefs, Preferences mainPrefsNode) {
        ...
        final String V3_3_LOGIC_LABELPATTERN = "logic/labelpattern"; // version 3.3 - 3.5,
                                                                     // mind the case
        final String V3_0_LOGIC_LABELPATTERN = "logic/labelPattern"; // node used for
                                                                     // version 3.0 - 3.2
    ...
    /**
     * Migrate Import File Name and Directory name Patterns from versions <=4.0 to new
     * BracketedPatterns
     */
    private static void migrateFileImportPattern(String oldStylePattern, String newStylePattern,
        ...
        if (oldStylePattern.equals(preferenceFileNamePattern)) {
            ...
            LOGGER.info("migrated old style {} value \"{}\" to new value \"{}\" in the preference file",
                    JabRefCliPreferences.IMPORT_FILENAMEPATTERN, oldStylePattern, newStylePattern);
            ...
            if (prefs.hasKey(JabRefCliPreferences.IMPORT_FILENAMEPATTERN)) {
                ...
                if (oldStylePattern.equals(fileNamePattern)) {
                    ...
                    LOGGER.info("migrated old style {} value \"{}\" to new value \"{}\" in the running application",
                            JabRefCliPreferences.IMPORT_FILENAMEPATTERN, oldStylePattern, newStylePattern);
                }
            }
        }
    ...
    static void upgradeImportFileAndDirePatterns(JabRefCliPreferences prefs, Preferences mainPrefsNode) {
        ...
        // Directory preferences are not yet migrated, since it is not quote clear how to
    ...
    private static void migrateTypedKeyPrefs(JabRefCliPreferences prefs, Preferences oldPatternPrefs)
        ...
        GlobalCitationKeyPatterns keyPattern = GlobalCitationKeyPatterns
            .fromPattern(prefs.get(JabRefCliPreferences.DEFAULT_CITATION_KEY_PATTERN));
     ...
     * <ul>
     * <li>Since v5.0-alpha the custom preview layout shows the 'comment' field instead of
     * the 'review' field
     * (<a href="https://github.com/JabRef/jabref/pull/4100">#4100</a>).</li>
     * <li>Since v5.1 a marker enables markdown in comments
     * (<a href="https://github.com/JabRef/jabref/pull/6232">#6232</a>).</li>
     * <li>Since v5.2 'bibtexkey' is rebranded as citationkey
     * (<a href="https://github.com/JabRef/jabref/pull/6875">#6875</a>).</li>
     * </ul>
     */
    protected static void upgradePreviewStyle(JabRefGuiPreferences prefs) {
        ...
        String migratedStyle = currentPreviewStyle
            .replace("\\begin{review}<BR><BR><b>Review: </b> \\format[HTMLChars]{\\review} \\end{review}",
            ...
            .replace("\\format[Markdown,HTMLChars]{\\comment}",
                    "\\format[Markdown,HTMLChars(keepCurlyBraces)]{\\comment}")
            .replace("<b><i>\\bibtextype</i><a name=\"\\bibtexkey\">\\begin{bibtexkey} (\\bibtexkey)</a>",
    ...
    /**
     * The former preferences default of columns was a simple list of strings
     * ("author;title;year;..."). Since 5.0 the preferences store the type of the column
     * too, so that the formerly hardwired columns like the graphic groups column or the
     * other icon columns can be reordered in the main table and behave like any other
     * field column
     * ("groups;linked_id;field:author;special:readstatus;extrafile:pdf;...").
     * <p>
     * Simple strings are by default parsed as a FieldColumn, so there is nothing to do
     * there, but the formerly hard wired columns need to be added.
     * <p>
     * In 5.1 variable names in JabRefPreferences have changed to offer backward
     * compatibility with pre 5.0 releases Pre 5.1: columnNames, columnWidths,
     * columnSortTypes, columnSortOrder Since 5.1: mainTableColumnNames,
     * mainTableColumnWidths, mainTableColumnSortTypes, mainTableColumnSortOrder
     */
    static void upgradeColumnPreferences(JabRefCliPreferences preferences) {
        ...
        // "field:"
        String normalFieldTypeString = MainTableColumnModel.Type.NORMALFIELD.getName()
    ...
    /**
     * In 5.0 the format of column names have changed. That made newer versions of JabRef
     * preferences incompatible with earlier versions of JabRef. As some complains came
     * up, we decided to change the variable names and to clear the variable contents if
     * they are unreadable, so former versions of JabRef would automatically create
     * preferences they can deal with.
     */
    static void restoreVariablesForBackwardCompatibility(JabRefCliPreferences preferences) {
        // 5.0 preference name "columnNames". The new one is {@link
    ...
    /**
     * In version 6.0 the formatting of the CleanUps preferences changed. Instead of using
     * several keys that have have a variable name a single preference key is introduced
     * containing just the active cleanup jobs. Also instead of a combined field for the
     * field formatters and the enabled status of all of them, they are split for easier
     * parsing.
     * <p>
     * <h3>Changes:</h3>
     * <table>
     ...
     * </tr>
     * <tr>
     * <td colspan="2">CLEANUP - old format:</td>
     * </tr>
     * <tr>
     * <td>CleanUpCLEAN_UP_DOI</td>
     * <td>enabled</td>
     * </tr>
     * <tr>
     * <td>CleanUpRENAME_PDF</td>
     * <td>disabled</td>
     * </tr>
     * <tr>
     * <td>CleanUpMOVE_PDF</td>
     * <td>enabled<br>
     * <tr>
     * <td colspan="2">...</td>
     * </tr>
     ...
     * </tr>
     * <tr>
     * <td colspan="2">CLEANUP_JOBS - new format:</td>
     * </tr>
     * <tr>
     * <td>CleanUpJobs</td>
     * <td>CLEAN_UP_DOI;RENAME_PDF;MOVE_PDF</td>
     * </tr>
     * <tr>
     * <td>&nbsp;</td>
     * </tr>
     * <tr>
     * <td colspan="2">CLEANUP_FORMATTERS - old format:</td>
     * </tr>
     * <tr>
     * <td>CleanUpFormatters</td>
     ...
     * </tr>
     * <tr>
     * <td colspan="2">CLEANUP_FORMATTERS - new format:</td>
     * </tr>
     * <tr>
     * <td>CleanUpFormattersEnabled</td>
     * <td>TRUE</td>
     * </tr>
     * <tr>
     * <td>CleanUpFormatters</td>
     * <td>field[formatter,formatter...]\nfield[...]\nfield[...]...</td>
     * </tr>
     * </table>
     */
    private static void upgradeCleanups(JabRefCliPreferences prefs) {
        ...
        for (CleanupPreferences.CleanupStep action : EnumSet.allOf(CleanupPreferences.CleanupStep.class)) {
            ...
            if (job.isPresent() && Boolean.parseBoolean(job.get())) {
                ...
                // prefs.deleteKey(V5_8_CLEANUP + action.name()); // for backward
                // compatibility in comments
            }
        }
        ...
        List<String> formatterCleanups = List
            .of(StringUtil.unifyLineBreaks(prefs.get(V5_8_CLEANUP_FIELD_FORMATTERS), "\n").split("\n"));
        if (formatterCleanups.size() >= 2 && (FieldFormatterCleanups.ENABLED.equals(formatterCleanups.getFirst())
                || FieldFormatterCleanups.DISABLED.equals(formatterCleanups.getFirst()))) {
            prefs.putBoolean(V6_0_CLEANUP_FIELD_FORMATTERS_ENABLED,
                    FieldFormatterCleanups.ENABLED.equals(formatterCleanups.getFirst()) ? Boolean.TRUE : Boolean.FALSE);

            prefs.put(V6_0_CLEANUP_FIELD_FORMATTERS,
            ...
            try (final Keyring keyring = Keyring.create()) {
                for (int i = 0; i < names.size(); i++) {
                    keyring.setPassword("org.jabref.customapikeys", names.get(i),
                            new Password(keys.get(i), preferences.getInternalPreferences().getUserAndHost()).encrypt());
                }
    ...
    /**
     * The tab "Comments" is hard coded using {@link CommentsTab} since v5.10 (and thus
     * hard-wired in {@link org.jabref.gui.entryeditor.EntryEditor#createTabs()}. Thus,
     * the configuration ih the preferences is obsolete
     */

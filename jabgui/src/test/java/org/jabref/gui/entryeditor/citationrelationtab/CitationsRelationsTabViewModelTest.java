...
class CitationsRelationsTabViewModelTest {
    ...
    void setUp() {
        ...
        when(preferences.getImportFormatPreferences()).thenReturn(
        ...
        when(importerPreferences.shouldGenerateNewKeyOnImport()).thenReturn(
            false
        );
        when(preferences.getImporterPreferences()).thenReturn(
            importerPreferences
        );

        FieldPreferences fieldPreferences = mock(FieldPreferences.class);
        when(fieldPreferences.getNonWrappableFields()).thenReturn(
            FXCollections.observableArrayList()
        );
        when(preferences.getFieldPreferences()).thenReturn(fieldPreferences);

        when(preferences.getFilePreferences()).thenReturn(
            mock(FilePreferences.class)
        );
        when(preferences.getOwnerPreferences()).thenReturn(
            mock(OwnerPreferences.class, Answers.RETURNS_DEEP_STUBS)
        );
        when(preferences.getTimestampPreferences()).thenReturn(
            mock(TimestampPreferences.class, Answers.RETURNS_DEEP_STUBS)
        );
        ...
        when(citationKeyPatternPreferences.getKeyPatterns()).thenReturn(
            patterns
        );
        when(preferences.getCitationKeyPatternPreferences()).thenReturn(
            citationKeyPatternPreferences
        );
        when(duplicateCheck.isDuplicate(any(), any(), any())).thenReturn(false);

        StateManager stateManager = mock(
            StateManager.class,
            Answers.RETURNS_DEEP_STUBS
        );
        bibDatabaseContext = new BibDatabaseContext(new BibDatabase());
        bibDatabaseContext.setMode(BibDatabaseMode.BIBTEX);
        when(stateManager.getActiveDatabase()).thenReturn(
    ...
    void existingEntryCitesOtherPaperWithCitationKeys() {
        ...
        assertEquals(
            Optional.of(
                "FirstAuthorCitationKey2022,SecondAuthorCitationKey20221"
            ),
    ...
    void importedEntriesWithExistingCitationKeysCiteExistingEntry() {
        ...
        BibEntry firstEntryInLibrary = bibDatabaseContext
            .getDatabase()
            .getEntryByCitationKey(firstEntryToImport.getCitationKey().get())
            .get();
        BibEntry secondEntryInLibrary = bibDatabaseContext
            .getDatabase()
            .getEntryByCitationKey(secondEntryToImport.getCitationKey().get())
            .get();

        assertEquals(
            Optional.of("Test2023"),
            firstEntryInLibrary.getField(StandardField.CITES)
        );
        assertEquals(
            List.of(existingEntry, firstEntryInLibrary, secondEntryInLibrary),
            bibDatabaseContext.getEntries()
        );
    }
    ...
    void existingEntryCitesOtherPaperWithCitationKeysAndExistingCiteField() {
        ...
        assertEquals(
            Optional.of(
                "Asdf1222,FirstAuthorCitationKey2022,SecondAuthorCitationKey20221"
            ),
            existingEntry.getField(StandardField.CITES)
        );
        assertEquals(

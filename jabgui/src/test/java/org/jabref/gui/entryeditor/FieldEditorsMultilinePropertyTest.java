...
public class FieldEditorsMultilinePropertyTest {
    ...
    private static final Pattern FIELD_PROPERTY_PATTERN = Pattern
        .compile("fieldProperties\\.contains\\s*\\(\\s*FieldProperty\\.(\\w+)\\s*\\)");
     ...
     * If the structure of FieldEditors.java is changed, this test might fail. This test
     * performs the following steps: 1. Use Java parser to parse FieldEditors.java and
     * check all if statements in the getForField method. 2. Match the conditions of if
     * statements to extract the field properties. 3. Match the created FieldEditor class
     * name with field properties extracted from step 2. This creates a map where: - The
     * key is the file path of the FieldEditor class (for example: ....UrlEditor.java) -
     * The value is the list of properties of the FieldEditor class (for example:
     * [FieldProperty.EXTERNAL]) 4. For every class in the map, when its properties
     * contain MULTILINE_TEXT, check whether it: a) Holds a TextInputControl field b) Has
     * an EditorTextArea object creation
     */
    @Test
    public void fieldEditorsMatchMultilineProperty() throws Exception {
        ...
        for (Map.Entry<Path, List<FieldProperty>> entry : result.entrySet()) {
            // Now we have the file path and its properties, going to analyze the target
            // Editor class
            Path filePath = entry.getKey();
            List<FieldProperty> properties = entry.getValue();
            CompilationUnit cu = PARSER.parse(filePath)
                .getResult()
                .orElseThrow(() -> new NullPointerException("Failed to parse " + filePath
                        + ", java parser returned null CompilationUnit" + ", please check if the file exists"));
            ...
            if (properties.contains(FieldProperty.MULTILINE_TEXT)) {
                // If the editor has MULTILINE_TEXT property, we are going to check if the
                // class holds a `TextInputControl` field
    ...
    /**
     * Parse FieldEditors.java to get all field editors and their properties in function
     * getForField
    ...
    private static Map<Path, List<FieldProperty>> getEditorsWithPropertiesInFieldEditors() throws Exception {
        ...
        CompilationUnit cu = PARSER.parse(Path.of(filePath))
            .getResult()
            .orElseThrow(() -> new NullPointerException("Failed to parse FieldEditors.java"));

        // Locate getForField method in FieldEditors.java
        MethodDeclaration getForFieldCall = cu.findAll(MethodDeclaration.class)
            .stream()
                ...
                .ifPresent(creationExpr -> {
                    ...
                    cu.findAll(ImportDeclaration.class)
                        ...
                        .ifPresentOrElse(importDeclaration -> {
                            ...
                            Path classFilePath = Path
                                .of("src/main/java/org/jabref/gui/fieldeditors/" + createdClassName + ".java");
                            result.put(classFilePath, properties);
                        });
                });
        });
    ...
    private static boolean implementedFieldEditorFX(CompilationUnit cu) {
        return cu.findAll(ClassOrInterfaceDeclaration.class)
            .stream()
            .anyMatch(classDecl -> classDecl.getImplementedTypes()
                .stream()
    ...
    private static boolean holdTextInputControlField(CompilationUnit cu) {
        // Since the class implements FieldEditorFX, we are going to check the first
        // parameter when call
            ...
            .ifPresent(methodCallExpr -> {
                if (!methodCallExpr.getArguments().isEmpty()) {
                    ...
                    cu.findAll(FieldDeclaration.class)
                        .stream()
                        .filter(fieldDeclaration -> fieldDeclaration.getVariables()
                            .stream()

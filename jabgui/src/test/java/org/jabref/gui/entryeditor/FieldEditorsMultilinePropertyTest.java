...
public class FieldEditorsMultilinePropertyTest {
    ...
    private static final Pattern FIELD_PROPERTY_PATTERN = Pattern.compile(
        "fieldProperties\\.contains\\s*\\(\\s*FieldProperty\\.(\\w+)\\s*\\)"
    );
    private static final Pattern STANDARD_FIELD_PATTERN = Pattern.compile(
        "==\\s*StandardField\\.(\\w+)"
    );
    ...
    public void fieldEditorsMatchMultilineProperty() throws Exception {
        Map<Path, List<FieldProperty>> result =
            getEditorsWithPropertiesInFieldEditors();
    ...
    private static Map<
        Path,
        List<FieldProperty>
    > getEditorsWithPropertiesInFieldEditors() throws Exception {
        ...
        MethodDeclaration getForFieldCall = cu
            .findAll(MethodDeclaration.class)
            .stream()
            .filter(methodDeclaration ->
                "getForField".equals(methodDeclaration.getNameAsString())
            )
            .findFirst()
            .orElseThrow(() ->
                new Exception(
                    "Failed to find getForField method in FieldEditors.java"
                )
            );
        ...
        getForFieldCall
            ...
            .forEach(ifStmt -> {
                ...
                Matcher propertyMatcher = FIELD_PROPERTY_PATTERN.matcher(
                    condition
                );
                while (propertyMatcher.find()) {
                    String propertyName = propertyMatcher.group(1);
                    FieldProperty property = FieldProperty.valueOf(
                        propertyName
                    );
                    properties.add(property);
                }
                // Match `== StandardField.XXX`
                Matcher standardFieldMatcher = STANDARD_FIELD_PATTERN.matcher(
                    condition
                );
                if (standardFieldMatcher.find()) {
                    String fieldName = standardFieldMatcher.group(1);
                    StandardField standardField = StandardField.valueOf(
                        fieldName
                    );
                    properties.addAll(standardField.getProperties());
                }
                // Match `== InternalField.XXX`
                Matcher internalFieldMatcher = INTERNAL_FIELD_PATTERN.matcher(
                    condition
                );
                if (internalFieldMatcher.find()) {
                    String fieldName = internalFieldMatcher.group(1);
                    InternalField internalField = InternalField.valueOf(
                        fieldName
                    );
                ...
                ifStmt
                    ...
                    .ifPresent(creationExpr -> {
                        String createdClassName = creationExpr
                            .getTypeAsString()
                            .replace("<>", "");
                        cu
        ...
        return cu
            ...
            .anyMatch(classDecl ->
                classDecl
                    ...
                    .stream()
                    .anyMatch(type ->
                        Objects.equals("FieldEditorFX", type.getNameAsString())
                    )
            );
    }
     ...
     */
    private static boolean hasEditorTextAreaCreationExisted(
        CompilationUnit cu
    ) {
        return cu
            ...
            .stream()
            .anyMatch(creation ->
                Objects.equals("EditorTextArea", creation.getType().toString())
            );
    }
    ...
    private static boolean holdTextInputControlField(CompilationUnit cu) {
        ...
        cu
            ...
            .filter(methodCallExpr ->
                "establishBinding".equals(methodCallExpr.getNameAsString())
            )
            .findFirst()
            .ifPresent(methodCallExpr -> {
                if (!methodCallExpr.getArguments().isEmpty()) {
                    String firstArgument = methodCallExpr
                        .getArgument(0)
                        .toString();
                    cu
                        ...
                        .filter(fieldDeclaration ->
                            fieldDeclaration
                                .getVariables()
                                .stream()
                                .anyMatch(variableDeclarator ->
                                    variableDeclarator
                                        .getNameAsString()
                                        .equals(firstArgument)
                                )
                        )
                        .findFirst()
                        .ifPresent(fieldDeclaration -> {
                            String classType = fieldDeclaration
                                .getElementType()
                                .asString();
    ...
    private static boolean holdEditorTextField(
        ...
        compilationUnit
            ...
            .ifPresent(establishBindingCall -> {
                String firstArg = establishBindingCall
                    .getArgument(0)
                    .toString();
                compilationUnit
                    .findAll(FieldDeclaration.class)
                    .stream()
                    .filter(field ->
                        field.getVariable(0).getNameAsString().equals(firstArg)
                    )
                    .findFirst()
                    .ifPresent(fieldDeclaration -> {
                        String fieldType = fieldDeclaration
                            .getElementType()
                            .asString();

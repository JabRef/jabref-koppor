...
class DonationProviderTest {
    ...
    public void calculateDaysUntilNextPopup() {
        ...
        int lastShownEpochDay = (int) LocalDate.now()
            .minusDays(400)
            .toEpochDay();
        int daysUntilNextPopup = donationProvider.calculateDaysUntilNextPopup(
            lastShownEpochDay
        );
        assertEquals(0, daysUntilNextPopup);

        lastShownEpochDay = (int) LocalDate.now().toEpochDay();
        daysUntilNextPopup = donationProvider.calculateDaysUntilNextPopup(
            lastShownEpochDay
        );

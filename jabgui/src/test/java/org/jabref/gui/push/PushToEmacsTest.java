...
class PushToEmacsTest {
    ...
    void setup() {
        ...
        when(pushToApplicationPreferences.getCommandPaths()).thenReturn(
            new SimpleMapProperty<>(emacsConfigObservableMap)
        );

        when(pushToApplicationPreferences.getEmacsArguments()).thenReturn(
            "-n -e"
        );

        when(
            pushToApplicationPreferences.getCiteCommand().toString()
        ).thenReturn("\\cite{key1,key2}");

        pushToEmacs = new GuiPushToEmacs(
            dialogService,

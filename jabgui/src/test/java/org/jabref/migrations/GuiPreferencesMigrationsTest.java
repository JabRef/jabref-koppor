...
class GuiPreferencesMigrationsTest {
    ...
    void oldStyleBibtexkeyPattern0() {
        when(
            preferences.get(JabRefCliPreferences.IMPORT_FILENAMEPATTERN)
        ).thenReturn(oldStylePatterns[0]);
        when(
            mainPrefsNode.get(JabRefCliPreferences.IMPORT_FILENAMEPATTERN, null)
        ).thenReturn(oldStylePatterns[0]);
        when(
            preferences.hasKey(JabRefCliPreferences.IMPORT_FILENAMEPATTERN)
        ).thenReturn(true);
        ...
        verify(preferences).put(
            JabRefCliPreferences.IMPORT_FILENAMEPATTERN,
            newStylePatterns[0]
        );
        verify(mainPrefsNode).put(
            JabRefCliPreferences.IMPORT_FILENAMEPATTERN,
            newStylePatterns[0]
        );
    }
    ...
    void arbitraryBibtexkeyPattern() {
        ...
        verify(preferences, never()).put(
            JabRefCliPreferences.IMPORT_FILENAMEPATTERN,
            arbitraryPattern
        );
        verify(mainPrefsNode, never()).put(
            JabRefCliPreferences.IMPORT_FILENAMEPATTERN,
            arbitraryPattern
        );
    }
    ...
    void previewStyleReviewToComment() {
        ...
        when(preferences.get(JabRefGuiPreferences.PREVIEW_STYLE)).thenReturn(
            oldPreviewStyle
        );
        ...
        verify(preferences).put(
            JabRefGuiPreferences.PREVIEW_STYLE,
            newPreviewStyle
        );
    }
    ...
    void upgradeColumnPreferencesAlreadyMigrated() {
        ...
        when(
            preferences.getStringList(JabRefGuiPreferences.COLUMN_NAMES)
        ).thenReturn(columnNames);
        when(
            preferences.getStringList(JabRefGuiPreferences.COLUMN_WIDTHS)
    ...
    void upgradeColumnPreferencesFromWithoutTypes() {
        ...
        verify(preferences).putStringList(
            JabRefGuiPreferences.COLUMN_NAMES,
            updatedNames
        );
        verify(preferences).putStringList(
    ...
    void changeColumnPreferencesVariableNamesFor51() {
        ...
        when(preferences.getStringList("columnWidths")).thenReturn(
            columnWidths
        );
        when(preferences.getStringList("mainTableColumnSortTypes")).thenReturn(
            columnNames
        );
        when(preferences.getStringList("mainTableColumnSortOrder")).thenReturn(
            columnWidths
        );
        ...
        when(
            preferences.getStringList(JabRefGuiPreferences.COLUMN_SORT_TYPES)
        ).thenReturn(List.of());
        when(
            preferences.getStringList(JabRefGuiPreferences.COLUMN_SORT_ORDER)
        ).thenReturn(List.of());
    ...
    void restoreColumnVariablesForBackwardCompatibility() {
        ...
        when(preferences.get(JabRefGuiPreferences.MAIN_FONT_SIZE)).thenReturn(
            "11.2"
        );

        PreferencesMigrations.restoreVariablesForBackwardCompatibility(
            preferences
        );
    ...
    void moveApiKeysToKeyRing() throws PasswordAccessException {
        ...
        when(
            preferences.getStringList(V5_9_FETCHER_CUSTOM_KEY_NAMES)
        ).thenReturn(List.of("FetcherA", "FetcherB", "FetcherC"));
        when(preferences.getStringList(V5_9_FETCHER_CUSTOM_KEYS)).thenReturn(
            List.of("KeyA", "KeyB", "KeyC")
        );
        when(preferences.getInternalPreferences().getUserAndHost()).thenReturn(
            "user-host"
        );
    ...
    void resolveBibTexStringsFields() {
        ...
        String expectedValue =
            "author;booktitle;editor;editora;editorb;editorc;institution;issuetitle;journal;journalsubtitle;journaltitle;mainsubtitle;month;publisher;shortauthor;shorteditor;subtitle;titleaddon;monthfiled";
        when(
            preferences.get(JabRefCliPreferences.RESOLVE_STRINGS_FOR_FIELDS)
        ).thenReturn(oldPrefsValue);

        PreferencesMigrations.upgradeResolveBibTeXStringsFields(preferences);
        verify(preferences).put(
            JabRefCliPreferences.RESOLVE_STRINGS_FOR_FIELDS,
            expectedValue
        );
    }
}

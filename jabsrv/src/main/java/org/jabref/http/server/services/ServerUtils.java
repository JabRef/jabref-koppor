...
public class ServerUtils {
    ...
    private static java.nio.file.Path getLibraryPath(
        String id,
    ...
    public static @NonNull Path getLibraryPath(
        String id,
        FilesToServe filesToServe,
        SrvStateManager srvStateManager
    ) {
    ...
    /// @throws NotFoundException if no file with the given id is found in either filesToServe or contextsToServe
    public static @NonNull BibDatabaseContext getBibDatabaseContext(
        String id,
        FilesToServe filesToServe,
        SrvStateManager srvStateManager,
        ...
        if ("demo".equals(id)) {
            try (
                InputStream chocolateBibInputStream =
                    BibDatabase.class.getResourceAsStream("/Chocolate.bib")
            ) {
                BufferedReader reader = new BufferedReader(
                    new InputStreamReader(
                        chocolateBibInputStream,
        ...
        if (filesToServe.isEmpty()) {
            return srvStateManager
                ...
                .findFirst()
                .orElseThrow(() ->
                    new NotFoundException("No library with id " + id + " found")
                );
        }

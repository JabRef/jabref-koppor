...
public class CitationsResource {
    ...
    /// + JabRef-side [CitationCacheService#getByTextHash] hits mean repeat
    ...
    private LookupResponse doLookup(String citationText,
        ...
        // Text-hash cache first: lets a re-scan of the same PDF page (or two
        // distinct PDFs that cite the same paper) reuse the prior parse and
        // skip the LLM call entirely.
        String textHash = CitationCacheService.sha256Hex(citationText);
        Optional<CitationCacheService.CachedCitation> cachedByText = citationCacheService.getByTextHash(textHash);
        BibEntry parsed;
        if (cachedByText.isPresent()) {
            ...
            citationCacheService.putByTextHash(textHash, parsed, citationText);
        }

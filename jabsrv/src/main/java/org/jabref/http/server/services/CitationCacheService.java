...
/// 2. **Text-hash cache** — keyed by SHA-256 of the raw citation text. Lets a
///    repeat `/lookup` (or batched `/lookup:batch`) skip the LLM / fetcher
///    parse entirely when the same citation string has been parsed recently
///    (e.g. SumatraPDF re-scans a visible PDF page after the user jumps back
///    to it). Longer TTL than the token cache because the parsed BibEntry
///    stays valid as long as the citation text does.
@Singleton
public class CitationCacheService {
    ...
    /// Stashes a parsed citation under its text-hash so a later `/lookup` for
    ...
    public void putByTextHash(String textHash, BibEntry parsed, String rawCitation) {
        textCache.put(textHash, new CachedCitation(parsed, rawCitation, Instant.now()));
    }

    /// Returns the cached parse for the given text-hash, or empty on miss.
    public Optional<CachedCitation> getByTextHash(String textHash) {
        return Optional.ofNullable(textCache.getIfPresent(textHash));
    }

    /// SHA-256 of the UTF-8 bytes of the citation text, lower-case hex. Used
    /// as the text-cache key. Stable across processes / restarts so a reader
    /// hitting the same PDF twice in a row gets cache hits even after a
    /// JabRef restart that lost the in-memory cache (clients re-derive the
    /// same hash from the same citation text).
    public static String sha256Hex(String citationText) {
        try {
            MessageDigest md = MessageDigest.getInstance("SHA-256");
            byte[] hash = md.digest(citationText.getBytes(StandardCharsets.UTF_8));

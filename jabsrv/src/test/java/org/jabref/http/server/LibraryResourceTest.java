...
class LibraryResourceTest extends ServerTest {
    ...
    void getJson() {
        assertEquals("""
                ...
                """, target("/libraries/" + TestBibFile.GENERAL_SERVER_TEST.id).request(JabrefMediaType.BIBTEX)
            .get(String.class));
    }
    ...
    void getClsItemJson() {
        assertEquals(
                ...
                target("/libraries/" + TestBibFile.GENERAL_SERVER_TEST.id).request(JabrefMediaType.JSON_CSL_ITEM)
                    .get(String.class));
    }

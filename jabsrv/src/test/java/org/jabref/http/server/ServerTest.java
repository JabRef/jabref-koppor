...
public abstract class ServerTest extends JerseyTest {
    ...
    public void setUp() throws Exception {
        super.setUp();
        FILES_TO_SERVE.setFilesToServe(
            List.of(TestBibFile.GENERAL_SERVER_TEST.path)
        );
    }

    protected void addFilesToServeToResourceConfig(
    ...
    protected void addFormatterServiceToResourceConfig(
        ResourceConfig resourceConfig
    ) {
        resourceConfig.register(
            new AbstractBinder() {
    ...
    private static void initializePreferencesService() {
        ...
        when(preferences.getFieldPreferences()).thenReturn(
        ...
        when(importFormatPreferences.fieldPreferences()).thenReturn(
            fieldContentFormatterPreferences
        );

        LastFilesOpenedPreferences lastFilesOpenedPreferences = mock(
            LastFilesOpenedPreferences.class
        );
        when(preferences.getLastFilesOpenedPreferences()).thenReturn(
    ...
    protected void addGlobalExceptionMapperToResourceConfig(
        ResourceConfig resourceConfig
    ) {

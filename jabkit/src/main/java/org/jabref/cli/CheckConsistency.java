...
@Command(
    ...
    description = "Check consistency of the library."
)
class CheckConsistency implements Callable<Integer> {
    ...
    public Integer call() {
        ...
        if (!sharedOptions.porcelain) {
            System.out.println(
                Localization.lang("Checking consistency of '%0'.", inputFile)
            );
            System.out.flush();
        }
        ...
        BibliographyConsistencyCheck.Result result = consistencyCheck.check(
            databaseContext,
            (count, total) -> {
                if (!sharedOptions.porcelain) {
                    System.out.println(
                        Localization.lang(
                            "Checking consistency for entry type %0 of %1",
                            count + 1,
                            total
                        )
                    );
    ...
    private int writeCheckResult(
        ...
        if (!sharedOptions.porcelain) {
            System.out.println(
                Localization.lang("Consistency check completed")
            );
        }
        return 0;
    }

...
public class LspDiagnosticHandler {
    ...
    public LspDiagnosticHandler(
        LspClientHandler clientHandler,
        CliPreferences cliPreferences,
        JournalAbbreviationRepository abbreviationRepository
    ) {
        this.clientHandler = clientHandler;
        this.jabRefCliPreferences = cliPreferences;
        this.lspIntegrityCheck = new LspIntegrityCheck(
            cliPreferences,
    ...
    public void computeAndPublishDiagnostics(
    ...
    private List<Diagnostic> computeDiagnostics(String content, String uri) {
        ...
        try {
            bibDatabaseContext = BibDatabaseContext.of(
                content,
                jabRefCliPreferences.getImportFormatPreferences()
            );
        } catch (JabRefException e) {
            Diagnostic parseDiagnostic = LspDiagnosticBuilder.create(
                Localization.lang(
        ...
        if (clientHandler.getSettings().isIntegrityCheck()) {
            integrityDiagnosticsCache.put(
                uri,
                lspIntegrityCheck.check(bibDatabaseContext, content)
            );
            LOGGER.debug("Cached integrity diagnostics for {}", uri);
        }

        if (clientHandler.getSettings().isConsistencyCheck()) {
            consistencyDiagnosticsCache.put(
                uri,
                lspConsistencyCheck.check(bibDatabaseContext, content)
            );
    ...
    private List<Diagnostic> getFinalDiagnosticsList(String uri) {
        ...
        return Stream.concat(
            settings.isIntegrityCheck()
                ? integrityDiagnosticsCache
                    .getOrDefault(uri, List.of())
                    .stream()
                : Stream.empty(),
            settings.isConsistencyCheck()
                ? consistencyDiagnosticsCache
                    .getOrDefault(uri, List.of())
                    .stream()
                : Stream.empty()
        ).toList();

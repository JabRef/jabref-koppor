...
public class LspConsistencyCheck {
    ...
    public List<Diagnostic> check(
        ...
        List<Field> allReportedFields = result
        ...
        result
            .entryTypeToResultMap()
            .forEach((entryType, entryTypeResult) -> {
                Optional<BibEntryType> bibEntryType =
                    new BibEntryTypesManager().enrich(
                        entryType,
                ...
                entryTypeResult
                    ...
                    .forEach(entry ->
                        requiredFields.forEach(requiredField -> {
                            if (
                                entry.getFieldOrAlias(requiredField).isEmpty()
                            ) {
                                LspDiagnosticBuilder diagnosticBuilder =
                                    LspDiagnosticBuilder.create(
                                        Localization.lang(
                                            "Required field \"%0\" is empty.",
                                            requiredField.getName()
                                        )
                                    );
                                diagnosticBuilder.setSeverity(
                ...
                optionalFields.forEach(optionalField ->
                    entryTypeResult
                        ...
                        .forEach(entry -> {
                            if (
                                entry.getFieldOrAlias(optionalField).isEmpty()
                            ) {
                                LspDiagnosticBuilder diagnosticBuilder =
                                    LspDiagnosticBuilder.create(
                                        Localization.lang(
                                            "Optional field \"%0\" is empty.",
                                            optionalField.getName()
                                        )
                                    );
                                diagnosticBuilder.setContent(content);
                                diagnosticBuilder.setEntry(entry);

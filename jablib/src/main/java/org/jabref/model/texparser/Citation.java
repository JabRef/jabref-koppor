...
public record Citation(Path path, int line, int colStart, int colEnd, String lineText) {
    ...
    /**
     * Get a fixed-width string that contains a cite and the text that surrounds it along
     * the same line.
     */
    public String getContext() {
        ...
        int start = Math.max(0,
                center + CONTEXT_WIDTH / 2 < lineLength ? center - CONTEXT_WIDTH / 2 : lineLength - CONTEXT_WIDTH);
        int end = Math.min(lineLength, start + CONTEXT_WIDTH);

        // Add three dots when the string does not contain all the line.
        return "%s%s%s".formatted(start > 0 ? "..." : "", lineText.substring(start, end).trim(),
                end < lineLength ? "..." : "");
    }

    @Override
    public String toString() {
        return "Citation{path=%s, line=%s, colStart=%s, colEnd=%s, lineText='%s'}".formatted(this.path, this.line,
                this.colStart, this.colEnd, this.lineText);
    }
}

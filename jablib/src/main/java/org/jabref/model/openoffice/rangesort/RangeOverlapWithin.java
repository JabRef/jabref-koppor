...
public class RangeOverlapWithin {
     ...
     * <p>
     * For overlapping and touching, only report consecutive ranges and only with a single
     * sample of otherwise identical ranges.
     * @param rangeHolders represent the ranges to be checked.
     * <p>
     * Note: for each rangeHolder, rangeHolder.getRange() is called multiple times. To
     * avoid repeated work, they should keep a copy of the range instead of getting it
     * each time from the document.
     * @param reportAtMost Limit the number of records returned to atMost. Zero
     * {@code reportAtMost} means no limit.
     * @param includeTouching Should the result contain ranges sharing only a boundary?
     */
    public static <V extends RangeHolder> List<RangeOverlap<V>> findOverlappingRanges(XTextDocument doc,
            List<V> rangeHolders, boolean includeTouching, int reportAtMost) {
     ...
     * For overlapping and touching, only report consecutive ranges and only with a single
     * sample of otherwise identical ranges.
     * @param atMost Limit the number of records returned to atMost. Zero {@code atMost}
     * means no limit.
     * @param includeTouching Should the result contain ranges sharing only a boundary?
     */
    public static <V extends RangeHolder> List<RangeOverlap<V>> findOverlappingRanges(
        ...
        for (List<V> partition : input.getPartitions()) {
            ...
            XTextRangeCompare cmp = UnoCast.cast(XTextRangeCompare.class, partition.getFirst().getRange().getText())
                .get();
            ...
            for (int i = 0; i < (partition.size() - 1); i++) {
                ...
                if (cmpResult == 0) {
                    ...
                    // aValues.add(bHolder);
                    // collect those equal
                    while (i < (partition.size() - 1) && UnoTextRange.compareStartsThenEndsUnsafe(cmp, aRange,
                ...
                if (cmpResult > 0 || (includeTouching && (cmpResult == 0))) {
                    ...
                    result.add(new RangeOverlap<>(cmpResult == 0 ? RangeOverlapKind.TOUCH : RangeOverlapKind.OVERLAP,

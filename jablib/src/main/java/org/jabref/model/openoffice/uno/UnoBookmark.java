...
public class UnoBookmark {
    ...
    /**
     * Insert a bookmark with the given name at the cursor provided, or with another name
     * if the one we asked for is already in use.
     * <p>
     * In LibreOffice the another name is in "{name}{number}" format.
     * @return The XNamed interface of the bookmark. result.getName() should be checked by
     * the caller, because its name may differ from the one requested.
     */

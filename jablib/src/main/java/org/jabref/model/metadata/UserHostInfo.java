...
/**
 * Record to represent user and host information. This is used to identify the user when
 * retrieving file directories.
 */
@NullMarked
public record UserHostInfo(String user, String host) {
    /**
     * Creates a new UserHostInfo from a user-host string. The user-host string is
     * expected to be in the format "user-host". If the string does not contain a hyphen,
     * the entire string is considered as the user and the host is empty.
    ...
    /**
     * Returns the user-host string representation. If the host is empty, only the user is
     * returned. Otherwise, the format is "user-host".

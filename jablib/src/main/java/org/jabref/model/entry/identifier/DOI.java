...
public class DOI implements Identifier {
    ...
    public static final URI AGENCY_RESOLVER = URLUtil.createUri(
        "https://doi.org/doiRA"
    );
    ...
    private static final String FIND_DOI_EXP =
        ...
        // divider
        + "(?:[^\\s,]+[^,;(\\.\\s)])"
        // suffix alphanumeric without " "/"," and not ending on "."/","/";"
        + ")"; // end group \1
    ...
    private static final String SHORT_DOI_EXP_PREFIX =
        ...
        // can begin with...
        + "\\s*(?:https?://)?(?:www\\.)?"
        // optional url parts "http(s)://"+"www."
        + "[a-zA-Z\\.]*doi[a-zA-Z\\.]*"
        //  eg "dx.doi." or "doi.acm." or "doi." if with url, must include "doi", otherwise too ambiguous
        + "\\.[a-zA-Z]{2,10}/)?"; // ".org" or ".de" or ".academy"
    private static final String SHORT_DOI_EXP =
        "(?:"
        // begin "any one of these"
        + "(?:[\\s/]?(?:(?:urn:)|(?:doi:)|(?:urn:doi:)))"
        // "doi:10/12ab" or " urn:10/12ab" or "/urn:doi:/10/12ab" ...
        + "|(?:\\s?/?)"
        // or "/10/12ab" or " /10/12ab" or "10/12ab" or " 10/12ab"
        + ")"
    ...
    private static final String FIND_SHORT_DOI_EXP =
        ...
        // "doi:10/12ab" or " urn:10/12ab" or "/urn:doi:/10/12ab" or "doi.org/doi:10/12ab"...
        + ")"
        // end "any one of these"
        + "("
        // begin group \1

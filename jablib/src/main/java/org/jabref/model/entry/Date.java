...
public class Date {
    ...
    static {
        List<String> formatStrings = Arrays.asList("uuuu-MM-dd'T'HH:mm[:ss][xxx][xx][X]", // covers
                                                                                          // 2018-10-03T07:24:14+03:00
                ...
                "d MMMM u / d MMMM u", "u'-'", // covers 2015-
                "u'?'", // covers 2023?
        ...
        SIMPLE_DATE_FORMATS = formatStrings.stream()
            ...
            .reduce(new DateTimeFormatterBuilder(), DateTimeFormatterBuilder::appendOptional,
                    (builder, formatterBuilder) -> builder.append(formatterBuilder.toFormatter()))
            .toFormatter(Locale.US);

        /*
         * There is also {@link org.jabref.model.entry.Date#parse(java.lang.String)}. The
         * regex of that method cannot be used as we parse single dates here and that
         * method parses: i) date ranges ii) two dates separated by '/' Additionally,
         * parse method requires the reviewed String to hold only a date.
         */
        DATE_REGEX = "\\d{4}-\\d{1,2}-\\d{1,2}" + // covers YYYY-MM-DD, YYYY-M-DD,
                                                  // YYYY-MM-D, YYYY-M-D
                "|\\d{4}\\.\\d{1,2}\\.\\d{1,2}|" + // covers YYYY.MM.DD, YYYY.M.DD,
                                                   // YYYY.MM.D, YYYY.M.D
    ...
    public static Optional<Date> parse(String dateString) {
        ...
        if (dateString.matches("\\d{4}/\\d{4}|" + // uuuu/uuuu
        ...
        else if (dateString.matches("\\d{4} / \\d{4}|" + // uuuu / uuuu
                "\\d{4}-\\d{2} / \\d{4}-\\d{2}|" + // uuuu-mm / uuuu-mm
                "\\d{4}-\\d{2}-\\d{2} / \\d{4}-\\d{2}-\\d{2}|" + // uuuu-mm-dd /
                                                                 // uuuu-mm-dd
                "(?i)(January|February|March|April|May|June|July|August|September|October|November|December)"
                + "( |\\-)(\\d{1,4}) / (January|February|March|April|May|June|July|August|September|October|November"
                + "|December)( |\\-)(\\d{1,4})(?i-)|" + // January 2015/January 2015
        ...
        else if (dateString.matches("\\d{1,4} BC/\\d{1,4} AD|" + // 30 BC/5 AD and 0030
                                                                 // BC/0005 AD
                "\\d{1,4} BC/\\d{1,4} BC|" + // 30 BC/10 BC and 0030 BC/0010 BC
                "\\d{1,4} AD/\\d{1,4} AD|" + // 5 AD/10 AD and 0005 AD/0010 AD
                "\\d{1,4}-\\d{1,2} BC/\\d{1,4}-\\d{1,2} AD|" + // 5 AD/10 AD and 0005
                                                               // AD/0010 AD
                "\\d{1,4}-\\d{1,2} BC/\\d{1,4}-\\d{1,2} BC|" + // 5 AD/10 AD and 0005
                                                               // AD/0010 AD
            ...
            try {
        ...
        else if (dateString.matches("\\d{1,4} BC / \\d{1,4} AD|" + // 30 BC / 5 AD and
                                                                   // 0030 BC / 0005 AD
                "\\d{1,4} BC / \\d{1,4} BC|" + // 30 BC / 10 BC and 0030 BC / 0010 BC
                "\\d{1,4} AD / \\d{1,4} AD|" + // 5 AD / 10 AD and 0005 AD / 0010 AD
                "\\d{1,4}-\\d{1,2} BC / \\d{1,4}-\\d{1,2} AD|" + // 5 AD/10 AD and 0005
                ...
                "\\d{1,4}-\\d{1,2} BC / \\d{1,4}-\\d{1,2} BC|" + // 5 AD/10 AD and 0005
                                                                 // AD/0010 AD
                "\\d{1,4}-\\d{1,2} AD / \\d{1,4}-\\d{1,2} AD" // 5 AD/10 AD and 0005
                                                              // AD/0010 AD
        )) {
            try {
                String[] strDates = dateString.split(" / ");
        ...
        if (dateString.matches("\\d{1,4} BC|" + // covers 1 BC
        ...
        if (dateString.matches("^(\\d{1,4})-(\\d{1,2})$" // covers 2025-21
        )) {
            try {
     ...
     * @param dateString the string which contain era indicator to extract the date
     * information
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(getYear(), date1.getYear()) && Objects.equals(getMonth(), date1.getMonth())
                && Objects.equals(getSeason(), date1.getSeason()) && Objects.equals(getDay(), date1.getDay())
    ...
    public String toString() {
        ...
        if (season != null) {
            ...
            return "Date{" + "date=" + formattedDate + ", " + "season=" + season.getName() + '}';
        }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(getYear(), getMonth(), getSeason(), getDay(), get(ChronoField.HOUR_OF_DAY),
                get(ChronoField.MINUTE_OF_HOUR), get(ChronoField.OFFSET_SECONDS));
    }

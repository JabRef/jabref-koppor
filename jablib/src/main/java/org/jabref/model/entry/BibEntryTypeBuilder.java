...
public class BibEntryTypeBuilder {
    ...
    public BibEntryTypeBuilder withImportantFields(
        ...
        this.optionalFields = Streams.concat(
    ...
    public BibEntryTypeBuilder withImportantFields(Field... newFields) {
        return withImportantFields(
            Arrays.stream(newFields).collect(
                Collectors.toCollection(LinkedHashSet::new)
            )
        );
    }

    public BibEntryTypeBuilder withDetailFields(
        SequencedCollection<Field> newFields
    ) {
        List<Field> containedFields = containedInSeenFields(newFields);
        if (!containedFields.isEmpty()) {
            LOGGER.warn(
    ...
    public BibEntryTypeBuilder addRequiredFields(
        SequencedSet<OrFields> requiredFields
    ) {
        Set<Field> fieldsToAdd = requiredFields
            .stream()
    ...
    public BibEntryType build() {
        ...
        Stream<BibField> requiredAsImportant = requiredFields
            ...
            .flatMap(Set::stream)
            .map(field -> new BibField(field, FieldPriority.IMPORTANT));
        SequencedSet<BibField> allFields = Stream.concat(
            optionalFields.stream(),
            requiredAsImportant
        ).collect(Collectors.toCollection(LinkedHashSet::new));

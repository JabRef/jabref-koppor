...
/**
 * Represents a node in a chain. We view a chain as a vertical hierarchy and thus refer to
 * the previous node as parent and the next node is a child.
 * <p>
 * In usual implementations, nodes function as wrappers around a data object. Thus
 * normally they have a value property which allows access to the value stored in the
 * node. In contrast to this approach, the ChainNode&lt;T> class is designed to be used as
 * a base class which provides the tree traversing functionality via inheritance.
 * <p>
 * Example usage: private class BasicChainNode extends ChainNode&lt;BasicChainNode> {
 * public BasicChainNode() { super(BasicChainNode.class); } }
 *
 * @param <T> the type of the class
 */
@SuppressWarnings("unchecked") // We use some explicit casts of the form "(T) this". The
                               // constructor ensures that this cast is valid.
public abstract class ChainNode<T extends ChainNode<T>> {
     ...
     * "T.class". We need this parameter since it is hard to get this information by other
     * means.
     */
     ...
     * <p>
     * This method does not set this node as the child of the new parent nor does it
     * remove this node from the old parent. You should probably call
     * {@link #moveTo(ChainNode)} to change the chain.
     ...
     * The given node is not allowed to already be in a tree (i.e. it has to have no
     * parent).
    ...
    /**
     * Removes this node from its parent and makes it a child of the specified node. In
     * this way the whole subchain based at this node is moved to the given node.
    ...
    /**
     * Adds the given node at the end of the chain. E.g., "A > B > C" + "D" -> "A > B > C
     * > D".
     */

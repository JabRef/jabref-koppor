    ...
    private void extractOtherFields() {
        ...
        for (Map.Entry<COSName, COSBase> o : dict.entrySet()) {
            ...
            if (FIELD_MAPPING.containsKey(o.getKey())) {
                ...
                key.startsWith(XmpUtilShared.BIBTEX_DI_FIELD_NAME_PREFIX)
            ) {
                String value = dict.getString(key);

                String fieldName = key.substring(
                    XmpUtilShared.BIBTEX_DI_FIELD_NAME_PREFIX.length()
                );
                Field field = FieldFactory.parseField(fieldName);
                switch (field) {
                    case InternalField.TYPE_HEADER -> bibEntry.setType(
                        EntryTypeFactory.parse(value)
                    );

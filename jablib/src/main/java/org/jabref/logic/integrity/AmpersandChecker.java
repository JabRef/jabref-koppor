...
/**
 * Checks if the BibEntry contains unescaped ampersands. This is done in nonverbatim
 * fields. Similar to {@link HTMLCharacterChecker}
 *
 * The {@link LatexIntegrityChecker} is not able to check unescaped ampersands. Therefore,
 * this separate checker is required.
 */
public class AmpersandChecker implements EntryChecker {
    ...
    public List<IntegrityMessage> check(BibEntry entry) {
        return entry.getFieldMap()
            ...
            // We use "flatMap" instead of filtering later, because we assume there won't
            // be that much error messages - and construction of Stream.empty() is faster
            // than construction of a new Tuple2 (including lifting long to Long)
            .flatMap(AmpersandChecker::getUnescapedAmpersandsWithCount)
            .map(pair -> new IntegrityMessage(Localization.lang("Found %0 unescaped '&'", pair.getValue()), entry,
                    pair.getKey()))
            .toList();
    }

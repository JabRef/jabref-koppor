...
public class TexBibEntriesResolver {
    ...
    public LatexBibEntriesResolverResult resolve(
        ...
        List<BibDatabase> bibDatabases = latexParserResults
            .getBibFiles()
            .stream()
            .map(path -> {
                try {
        ...
        List<BibEntry> newEntries = findNewEntries(
            bibDatabases,
            latexParserResults.getCitations().keySet()
        );
    ...
    private List<BibEntry> findNewEntries(
        ...
        return bibDatabases
            ...
            .flatMap(database ->
                citations
                    .stream()
                    .flatMap(citation ->
                        database.getEntriesByCitationKey(citation).stream()
                    )

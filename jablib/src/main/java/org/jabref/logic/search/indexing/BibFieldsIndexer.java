...
public class BibFieldsIndexer {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(
        BibFieldsIndexer.class
    );

    private static final Pattern GROUPS_SEPARATOR_REGEX = Pattern.compile(
        "\s*,\s*"
    );
    private static final Set<Field> DATE_FIELDS = Set.of(
        StandardField.DATE,
        StandardField.YEAR,
        StandardField.MONTH,
        StandardField.DAY
    ...
    public BibFieldsIndexer(
        ...
        this.libraryName = databaseContext
            .getDatabasePath()
        ...
        this.schemaMainTableReference =
            PostgreConstants.getMainTableSchemaReference(mainTable);
        this.schemaSplitValuesTableReference =
            PostgreConstants.getSplitTableSchemaReference(mainTable);
    ...
    private void setup() {
        try {
            ...
            LOGGER.error(
                "Could not create tables for library: {}",
                libraryName,
                e
            );
        }
        try {
            ...
            LOGGER.error(
                "Could not create indexes for library: {}",
                libraryName,
                e
            );
        }
    ...
    public void addToIndex(List<BibEntry> entries, BackgroundTask<?> task) {
        ...
        if (count > 1) {
            ...
            task.setTitle(
                Localization.lang("Indexing bib fields for %0", libraryName)
            );
        }
        ...
        for (int i = 0; i < count; i++) {
            ...
            task.updateMessage(
                Localization.lang(
                    "%0 of %1 entries added to the index.",
                    i,
                    count
                )
            );
    ...
    private void addToIndex(BibEntry bibEntry) {
        ...
        try (
            ...
            PreparedStatement preparedStatementSplitValues =
                connection.prepareStatement(insertIntoSplitTable)
        ) {
            String entryId = bibEntry.getId();
            LOGGER.atTrace()
                .setMessage("Adding entry {}")
                .addArgument(() -> bibEntry.getKeyAuthorTitleYear())
                .log();
            for (Map.Entry<Field, String> fieldPair : bibEntry
                .getFieldMap()
                .entrySet()) {
                ...
                if (!DATE_FIELDS.contains(field)) {
                    ...
                    String resolvedFieldLatexFree = bibEntry
                        .getResolvedFieldOrAliasLatexFree(
                            field,
            ...
            for (Field dateField : DATE_FIELDS) {
                Optional<String> resolvedDateValue =
                    bibEntry.getResolvedFieldOrAlias(
                        dateField,
                        this.databaseContext.getDatabase()
                    );
                resolvedDateValue.ifPresent(dateValue ->
    ...
    public void removeFromIndex(
        ...
        if (entries.size() > 1) {
            ...
            task.setTitle(
                Localization.lang(
                    "Removing entries from index for %0",
                    libraryName
                )
            );
        }
        ...
        for (BibEntry entry : entries) {
            ...
            task.updateMessage(
                Localization.lang(
                    "%0 of %1 entries removed from the index.",
                    i,
                    entries.size()
                )
            );
        ...
        if (DATE_FIELDS.contains(field)) {
            try (
                PreparedStatement preparedStatement =
                    connection.prepareStatement(insertDateFieldQuery)
            ) {
                for (Field dateField : DATE_FIELDS) {
                    Optional<String> resolvedDateValue =
            ...
            try (
                ...
                Optional<String> resolvedFieldLatexFree =
                    entry.getResolvedFieldOrAliasLatexFree(
                        field,
                        this.databaseContext.getDatabase()
                    );
    ...
    private void removeField(BibEntry entry, Field field) {
        try {
            ...
            LOGGER.debug(
                "Field {} removed from entry {} in index",
                field.getName(),
                entry.getId()
            );
        } catch (SQLException e) {
    ...
    private void closeIndex() {
        try {
            LOGGER.debug(
                "Closing connection to Postgres server for library: {}",
                libraryName
            );
            connection
                .createStatement()
            ...
            LOGGER.error(
                "Could not drop table for library: {}",
                libraryName,
                e
            );
        }
    ...
    private void addEntryLinks(
        ...
        bibEntry
            .getEntryLinkList(field, databaseContext.getDatabase())
            .stream()
            .distinct()
            .forEach(link ->
                addBatch(
    ...
    private static void addKeywords(
        ...
        KeywordList keywordList = KeywordList.parse(
            keywordsString,
            keywordSeparator
        );
        keywordList
            .stream()
            .flatMap(keyword -> keyword.flatten().stream())
            .forEach(keyword -> {
    ...
    private static void addBatch(
        ...
        addBatch(
            ...
            field,
            value,
            LatexToUnicodeAdapter.format(value)
        );
    }
    ...
    private static void addBatch(
        ...
        try {
            ...
            LOGGER.error(
                "Could not add field {} having value {} of entry {} to the index.",
                field.getName(),
                value,
                entryId,
                e

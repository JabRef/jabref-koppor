...
public final class LinkedFilesSearcher {
    ...
    public LinkedFilesSearcher(
        ...
        this.parser = new MultiFieldQueryParser(
            LinkedFilesConstants.PDF_FIELDS.toArray(new String[0]),
    ...
    public SearchResults search(SearchQuery searchQuery) {
        ...
        LOGGER.debug(
            "Searching in linked files with query: {}",
            luceneQuery.get()
        );
        try {
            IndexSearcher linkedFilesIndexSearcher = acquireIndexSearcher(
                searcherManager
            );
            SearchResults searchResults = search(
                linkedFilesIndexSearcher,
                luceneQuery.get()
            );
    ...
    private SearchResults search(IndexSearcher indexSearcher, Query searchQuery)
        throws IOException {
    ...
    private SearchResults getSearchResults(
        ...
        Highlighter highlighter = new Highlighter(
            ...
            new QueryScorer(searchQuery)
        );

        for (ScoreDoc scoreDoc : topDocs.scoreDocs) {
            Document document = storedFields.document(scoreDoc.doc);
            String fileLink = getFieldContents(
                document,
                LinkedFilesConstants.PATH
            );
                ...
                if (entriesWithFile != null && !entriesWithFile.isEmpty()) {
                    SearchResult searchResult = new SearchResult(
                        ...
                        Integer.parseInt(
                            getFieldContents(
                                document,
                                LinkedFilesConstants.PAGE_NUMBER
                            )
                        ),
                        highlighter
                    );
        ...
        LOGGER.debug(
            "Getting linked files results took {} ms",
    ...
    private Map<String, List<String>> getLinkedFilesMap() {
        ...
        for (BibEntry bibEntry : databaseContext.getEntries()) {
            for (LinkedFile linkedFile : bibEntry.getFiles()) {
                linkedFilesMap
                    .computeIfAbsent(linkedFile.getLink(), k ->
                        new ArrayList<>()
                    )
                    .add(bibEntry.getId());
            }
    ...
    private static String getFieldContents(
        Document document,
    ...
    private static IndexSearcher acquireIndexSearcher(

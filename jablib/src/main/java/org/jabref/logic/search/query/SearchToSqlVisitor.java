...
public class SearchToSqlVisitor extends SearchBaseVisitor<SqlQueryNode> {
    ...
    public SqlQueryNode visitImplicitAndExpression(SearchParser.ImplicitAndExpressionContext ctx) {
        ...
        if (children.size() == 1) {
            ...
            String cte = """
                    ...
                    """.formatted(cteCounter,
                    children.stream()
                        .map(node -> "    SELECT %s FROM %s".formatted(ENTRY_ID, node.cte()))
    ...
    public SqlQueryNode visitNegatedExpression(SearchParser.NegatedExpressionContext ctx) {
        ...
        String cte = """
                ...
                """.formatted(cteCounter, MAIN_TABLE, ENTRY_ID, mainTableName, MAIN_TABLE, MAIN_TABLE, ENTRY_ID,
                ENTRY_ID, subNode.cte());
    ...
    private SqlQueryNode getFieldQueryNode(String field, String term, EnumSet<SearchFlags> searchFlags) {
        ...
        if (ENTRY_ID.toString().equals(field)) {
            ...
            if (searchFlags.contains(EXACT_MATCH)) {
                return searchFlags.contains(NEGATION) ? buildExactNegationAnyFieldQuery(sqlOperator, term)
            ...
            if (searchFlags.contains(EXACT_MATCH)) {
                return searchFlags.contains(NEGATION) ? buildExactNegationFieldQuery(field, sqlOperator, term)
    ...
    private SqlQueryNode buildContainsAnyFieldQuery(String operator, String prefixSuffix, String term) {
        String cte = """
                ...
                """.formatted(cteCounter, MAIN_TABLE, ENTRY_ID, mainTableName, MAIN_TABLE, MAIN_TABLE, FIELD_NAME,
                GROUPS_FIELD, // https://github.com/JabRef/jabref/issues/7996
    ...
    private SqlQueryNode buildContainsNegationAnyFieldQuery(String operator, String prefixSuffix, String term) {
        String cte = """
                ...
                """.formatted(cteCounter, MAIN_TABLE, ENTRY_ID, mainTableName, MAIN_TABLE, MAIN_TABLE, ENTRY_ID,
                INNER_TABLE, ENTRY_ID, mainTableName, INNER_TABLE, INNER_TABLE, FIELD_NAME, GROUPS_FIELD, // https://github.com/JabRef/jabref/issues/7996
    ...
    private SqlQueryNode buildExactAnyFieldQuery(String operator, String term) {
        String cte = """
                ...
                """.formatted(cteCounter, MAIN_TABLE, ENTRY_ID, mainTableName, MAIN_TABLE, splitValuesTableName,
                SPLIT_TABLE, MAIN_TABLE, ENTRY_ID, SPLIT_TABLE, ENTRY_ID, MAIN_TABLE, FIELD_NAME, SPLIT_TABLE,
                FIELD_NAME, MAIN_TABLE, FIELD_NAME, GROUPS_FIELD, // https://github.com/JabRef/jabref/issues/7996
                MAIN_TABLE, FIELD_VALUE_LITERAL, operator, MAIN_TABLE, FIELD_VALUE_TRANSFORMED, operator, SPLIT_TABLE,
                FIELD_VALUE_LITERAL, operator, SPLIT_TABLE, FIELD_VALUE_TRANSFORMED, operator);
    ...
    private SqlQueryNode buildExactNegationAnyFieldQuery(String operator, String term) {
        String cte = """
                ...
                """.formatted(cteCounter, MAIN_TABLE, ENTRY_ID, mainTableName, MAIN_TABLE, MAIN_TABLE, ENTRY_ID,
                INNER_TABLE, ENTRY_ID, mainTableName, INNER_TABLE, splitValuesTableName, SPLIT_TABLE, INNER_TABLE,
                FIELD_NAME, SPLIT_TABLE, FIELD_NAME, INNER_TABLE, ENTRY_ID, SPLIT_TABLE, ENTRY_ID, INNER_TABLE,
                FIELD_NAME, GROUPS_FIELD, // https://github.com/JabRef/jabref/issues/7996
                INNER_TABLE, FIELD_VALUE_LITERAL, operator, INNER_TABLE, FIELD_VALUE_TRANSFORMED, operator, SPLIT_TABLE,
                FIELD_VALUE_LITERAL, operator, SPLIT_TABLE, FIELD_VALUE_TRANSFORMED, operator);
    ...
    private SqlQueryNode buildContainsFieldQuery(String field, String operator, String prefixSuffix, String term) {
        String cte = """
                ...
                """.formatted(cteCounter, MAIN_TABLE, ENTRY_ID, mainTableName, MAIN_TABLE, MAIN_TABLE, FIELD_NAME,
                field, MAIN_TABLE, FIELD_VALUE_LITERAL, operator, MAIN_TABLE, FIELD_VALUE_TRANSFORMED, operator);
    ...
    private SqlQueryNode buildContainsNegationFieldQuery(String field, String operator, String prefixSuffix,
            String term) {
        String cte = """
                cte%d AS (
                    SELECT %s.%s
                ...
                """.formatted(cteCounter, MAIN_TABLE, ENTRY_ID, mainTableName, MAIN_TABLE, MAIN_TABLE, ENTRY_ID,
                INNER_TABLE, ENTRY_ID, mainTableName, INNER_TABLE, INNER_TABLE, FIELD_NAME, field, INNER_TABLE,
                FIELD_VALUE_LITERAL, operator, INNER_TABLE, FIELD_VALUE_TRANSFORMED, operator);
    ...
    private SqlQueryNode buildExactFieldQuery(String field, String operator, String term) {
        String cte = """
                ...
                """.formatted(cteCounter, MAIN_TABLE, ENTRY_ID, mainTableName, MAIN_TABLE, splitValuesTableName,
                SPLIT_TABLE, MAIN_TABLE, ENTRY_ID, SPLIT_TABLE, ENTRY_ID, MAIN_TABLE, FIELD_NAME, SPLIT_TABLE,
                FIELD_NAME, MAIN_TABLE, FIELD_NAME, field, MAIN_TABLE, FIELD_VALUE_LITERAL, operator, MAIN_TABLE,
    ...
    private SqlQueryNode buildExactNegationFieldQuery(String field, String operator, String term) {
        String cte = """
                ...
                """.formatted(cteCounter, MAIN_TABLE, ENTRY_ID, mainTableName, MAIN_TABLE, MAIN_TABLE, ENTRY_ID,
                INNER_TABLE, ENTRY_ID, mainTableName, INNER_TABLE, splitValuesTableName, SPLIT_TABLE, INNER_TABLE,
                ENTRY_ID, SPLIT_TABLE, ENTRY_ID, INNER_TABLE, FIELD_NAME, SPLIT_TABLE, FIELD_NAME, INNER_TABLE,
                FIELD_NAME, field, INNER_TABLE, FIELD_VALUE_LITERAL, operator, INNER_TABLE, FIELD_VALUE_TRANSFORMED,
                operator, SPLIT_TABLE, FIELD_NAME, field, SPLIT_TABLE, FIELD_VALUE_LITERAL, operator, SPLIT_TABLE,
                FIELD_VALUE_TRANSFORMED, operator);
    ...
    private static void setFlags(EnumSet<SearchFlags> flags, SearchFlags matchType, boolean caseSensitive,
            boolean negation) {
        flags.add(matchType);
    ...
    private static String getSqlOperator(EnumSet<SearchFlags> searchFlags) {
        return searchFlags.contains(REGULAR_EXPRESSION) ? (searchFlags.contains(CASE_SENSITIVE) ? "~" : "~*")

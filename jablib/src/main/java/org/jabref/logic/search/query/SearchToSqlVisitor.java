...
public class SearchToSqlVisitor extends SearchBaseVisitor<SqlQueryNode> {
    ...
    public SearchToSqlVisitor(
        String table,
        EnumSet<SearchFlags> searchBarFlags
    ) {
        this.searchBarFlags = searchBarFlags;
        this.mainTableName = PostgreConstants.getMainTableSchemaReference(
            table
        );
        this.splitValuesTableName =
            PostgreConstants.getSplitTableSchemaReference(table);
    }
    ...
    @Override
    public SqlQueryNode visitImplicitAndExpression(
        SearchParser.ImplicitAndExpressionContext ctx
    ) {
        List<SqlQueryNode> children = ctx
            .expression()
            .stream()
            .map(this::visit)
            .toList();
        ...
        if (children.size() == 1) {
            ...
            List<String> params = children
                .stream()
                .flatMap(node -> node.params().stream())
                .toList();
    ...
    @Override
    public SqlQueryNode visitParenExpression(
    ...
    @Override
    public SqlQueryNode visitNegatedExpression(
    ...
    @Override
    public SqlQueryNode visitBinaryExpression(
    ...
    @Override
    public SqlQueryNode visitComparisonExpression(
    ...
    public SqlQueryNode visitComparison(SearchParser.ComparisonContext ctx) {
        ...
        if (
            operator == SearchParser.EQUAL || operator == SearchParser.CONTAINS
        ) {
            ...
            operator == SearchParser.EEQUAL || operator == SearchParser.MATCHES
        ) {
    ...
    private SqlQueryNode getFieldQueryNode(
        String field,
        String term,
        ...
        if (ENTRY_ID.toString().equals(field)) {
            ...
            if (searchFlags.contains(EXACT_MATCH)) {
                ...
                return searchFlags.contains(NEGATION)
                    ? buildContainsNegationAnyFieldQuery(
                        sqlOperator,
                        prefixSuffix,
                        term
                    )
                    : buildContainsAnyFieldQuery(
                        sqlOperator,
                        prefixSuffix,
                        term
                    );
            ...
            if (searchFlags.contains(EXACT_MATCH)) {
                ...
                return searchFlags.contains(NEGATION)
                    ? buildContainsNegationFieldQuery(
                        field,
                        sqlOperator,
                        prefixSuffix,
                        term
                    )
                    : buildContainsFieldQuery(
                        field,
                        sqlOperator,
                        prefixSuffix,
                        term
    ...
    private SqlQueryNode buildContainsAnyFieldQuery(
        String operator,
        String prefixSuffix,
        String term
    ) {
        ...
        List<String> params = Collections.nCopies(
            2,
    ...
    private SqlQueryNode buildContainsNegationAnyFieldQuery(
        String operator,
        String prefixSuffix,
        String term
    ) {
    ...
    private SqlQueryNode buildExactNegationAnyFieldQuery(
        String operator,
        String term
    ) {
        String cte = """
    ...
    private SqlQueryNode buildContainsFieldQuery(
        String field,
        String operator,
        String prefixSuffix,
        String term
    ...
    private SqlQueryNode buildContainsNegationFieldQuery(
        String field,
        String operator,
        String prefixSuffix,
        String term
    ...
    private SqlQueryNode buildExactFieldQuery(
        String field,
        String operator,
        String term
    ) {
    ...
    private SqlQueryNode buildExactNegationFieldQuery(
        String field,
        String operator,
        String term
    ) {

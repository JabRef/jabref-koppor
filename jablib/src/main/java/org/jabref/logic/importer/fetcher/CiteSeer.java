...
public class CiteSeer implements SearchBasedFetcher, FulltextFetcher {
    ...
    @Override
    public List<BibEntry> performSearch(BaseQueryNode queryNode)
        ...
        try {
            ...
            Optional<JSONArray> jsonResponse = Optional.ofNullable(
                requestResponse
            )
    ...
    private JSONElement getPayloadJSON(BaseQueryNode searchQueryList)
        throws ParseException {
        transformer = new CiteSeerQueryTransformer();
        String transformedQuery = transformer
            .transformSearchQuery(searchQueryList)
            .orElse("");

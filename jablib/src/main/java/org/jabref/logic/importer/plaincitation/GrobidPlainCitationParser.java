...
public class GrobidPlainCitationParser implements PlainCitationParser {
    ...
    public GrobidPlainCitationParser(GrobidPreferences grobidPreferences,
    ...
    /**
     * Passes request to grobid server, using consolidateCitations option to improve
     * result. Takes a while, since the server has to look up the entry.
    ...
    public Optional<BibEntry> parsePlainCitation(String text) throws FetcherException {
        try {
            return grobidService.processCitation(text, importFormatPreferences,

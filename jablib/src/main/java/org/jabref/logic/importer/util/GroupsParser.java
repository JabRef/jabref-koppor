...
public class GroupsParser {
    ...
    private static AbstractGroup automaticKeywordGroupFromString(
        ...
        Character delimiter = StringUtil.unquote(
            tok.nextToken(),
            MetadataSerializationConfiguration.GROUP_QUOTE_CHAR
        ).charAt(0);
    ...
    private static KeywordGroup keywordGroupFromString(
        ...
        if (regExp) {
            newGroup = new RegexKeywordGroup(
                name,
                context,
                field,
                expression,
    ...
    private static SmartGroup smartGroupFromString(
        String input,
        Character keywordSeparator
    ) throws ParseException {
        if (
            !input.startsWith(MetadataSerializationConfiguration.SMART_GROUP_ID)
        ) {
            throw new IllegalArgumentException(
                "SmartGroup cannot be created from \"" + input + "\"."
            );
        }
        QuotedStringTokenizer tok = new QuotedStringTokenizer(
            input.substring(
                MetadataSerializationConfiguration.SMART_GROUP_ID.length()
            ),
            MetadataSerializationConfiguration.GROUP_UNIT_SEPARATOR,
            MetadataSerializationConfiguration.GROUP_QUOTE_CHAR
        );
        ...
        try {
            ...
            SmartGroup newGroup = new SmartGroup(
                name,

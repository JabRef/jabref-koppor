...
/**
 * Provides a convenient interface for entry-based fetcher, which follow the usual
 * three-step procedure: 1. Open a URL based on the entry 2. Parse the response to get a
 * list of {@link BibEntry} 3. Post-process fetched entries
 */
public interface EntryBasedParserFetcher extends EntryBasedFetcher, ParserFetcher {
    ...
    default List<BibEntry> performSearch(BibEntry entry) throws FetcherException {
        ...
        try (InputStream stream = new BufferedInputStream(urlForEntry.openStream())) {
            ...
            // TODO: Catch HTTP Response 401 errors and report that user has no rights to
            // access resource
            // Same TODO as in org.jabref.logic.net.URLDownload.openConnection. Code
            // should be reused.

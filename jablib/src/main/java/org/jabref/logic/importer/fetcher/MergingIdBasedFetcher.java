...
public class MergingIdBasedFetcher {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(
        MergingIdBasedFetcher.class
    );
    private static final List<StandardField> SUPPORTED_FIELDS = List.of(
        StandardField.DOI,
        StandardField.ISBN,
        StandardField.EPRINT
    );
    private static final NonSpaceWhitespaceRemover REMOVE_WHITESPACE_FORMATTER =
        new NonSpaceWhitespaceRemover();
    private final ImportFormatPreferences importFormatPreferences;

    public MergingIdBasedFetcher(
    ...
    private Optional<FetcherResult> fetchWithField(
        BibEntry entry,
        Field field
    ) {
        return entry
            .getField(field)
            .flatMap(identifier ->
                fetchWithIdentifier(field, identifier, entry)
            );
    }

    private Optional<FetcherResult> fetchWithIdentifier(
        Field field,
        String identifier,
        BibEntry entryFromLibrary
    ) {
        return WebFetchers.getIdBasedFetcherForField(
            field,
            importFormatPreferences
        ).flatMap(fetcher ->
            executeFetch(fetcher, field, identifier, entryFromLibrary)
        );
    }

    private Optional<FetcherResult> executeFetch(
        ...
        try {
            ...
            return fetcher
                ...
                .map(fetchedEntry ->
                    mergeBibEntries(entryFromLibrary, fetchedEntry)
                );
        } catch (FetcherException e) {
            LOGGER.error(
                "Fetch failed for {} with identifier: {}",
                field,
                identifier,
                e
            );
    ...
    private FetcherResult mergeBibEntries(
        BibEntry entryFromLibrary,
        ...
        Set<Field> updatedFields = updateFieldsFromSource(
            fetchedEntry,
            mergedEntry
        );
    ...
    private Set<Field> updateFieldsFromSource(
    ...
    private boolean shouldUpdateField(
        Field field,
        BibEntry sourceEntry,
        BibEntry targetEntry
    ) {
        String sourceValue = sourceEntry
            .getField(field)
            .map(REMOVE_WHITESPACE_FORMATTER::format)
            .orElse("");
        String targetValue = targetEntry
            .getField(field)
            .map(REMOVE_WHITESPACE_FORMATTER::format)
            .orElse("");

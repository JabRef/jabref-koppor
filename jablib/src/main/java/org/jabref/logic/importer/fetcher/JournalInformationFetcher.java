...
public class JournalInformationFetcher implements WebFetcher {
    ...
    public Optional<JournalInformation> getJournalInformation(
        ...
        if (httpResponse.getBody() != null) {
            ...
            journalInformationOptional = Optional.of(
        ...
        try {
            ...
            if (responseJsonObject.has("data")) {
                ...
                if (data.has("journal") && data.get("journal") != null) {
                    ...
                    coverageStartYear = journalData.optString(
                        "coverageStartYear",
                        ""
                    );
                    coverageEndYear = journalData.optString(
                        "coverageEndYear",
                        ""
                    );
                    ...
                    categories = getConcatenatedString(
                        journalData,
                        "categories"
                    );
                    ...
                    if (citationInfo != null) {
                        docsThisYear = parseCitationInfo(
                            citationInfo,
                            "docsThisYear"
                        );
                        docsPrevious3Years = parseCitationInfo(
                            citationInfo,
                            "docsPrevious3Years"
                        );
                        citableDocsPrevious3Years = parseCitationInfo(
                            citationInfo,
                            "citableDocsPrevious3Years"
                        );
                        citesOutgoing = parseCitationInfo(
                            citationInfo,
                            "citesOutgoing"
                        );
                        citesOutgoingPerDoc = parseCitationInfo(
                            citationInfo,
                            "citesOutgoingPerDoc"
                        );
                        citesIncomingByRecentlyPublished = parseCitationInfo(
                            citationInfo,
    ...
    private static String getConcatenatedString(
        ...
        if (jsonArray != null) {
            return QUOTES_BRACKET_PATTERN.matcher(
                jsonArray.join(", ")
            ).replaceAll("");
        } else {
            return "";

...
public class LlmPlainCitationParser implements PlainCitationParser {
    ...
    public Optional<BibEntry> parsePlainCitation(String text)
        ...
        try {
            return BibtexParser.singleFromString(
                getBibtexStringFromLlm(text),
                importFormatPreferences
            );
        } catch (ParseException e) {
            throw new FetcherException(
                "Could not parse BibTeX returned from LLM",
                e
            );
        }
    }
    ...
    private String getBibtexStringFromLlm(String searchQuery) {
        String systemMessage =
            aiTemplatesService.makeCitationParsingSystemMessage();
        String userMessage = aiTemplatesService.makeCitationParsingUserMessage(
            searchQuery
        );

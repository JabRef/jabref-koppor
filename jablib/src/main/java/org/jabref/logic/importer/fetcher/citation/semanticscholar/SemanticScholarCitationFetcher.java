...
public class SemanticScholarCitationFetcher
    implements CitationFetcher, CustomizableKeyFetcher {

    public static final String FETCHER_NAME =
        "Semantic Scholar Citations Fetcher";

    private static final Logger LOGGER = LoggerFactory.getLogger(
        SemanticScholarCitationFetcher.class
    );
    ...
    public SemanticScholarCitationFetcher(
    ...
    public List<BibEntry> searchCitedBy(BibEntry entry)
        ...
        CitationsResponse citationsResponse = GSON.fromJson(
            urlDownload.asString(),
            CitationsResponse.class
        );
        ...
        return citationsResponse
            ...
            .stream()
            .filter(
                citationDataItem -> citationDataItem.getCitingPaper() != null
            )
    ...
    @Override
    public @NonNull List<BibEntry> searchCiting(@NonNull BibEntry entry)
        throws FetcherException {
        if (entry.getDOI().isEmpty()) {
            return List.of();
        }
        ...
        ReferencesResponse referencesResponse = GSON.fromJson(
            response,
        ...
        if (referencesResponse.getData() == null) {
            ...
            Optional.ofNullable(responseObject.optJSONObject("citingPaperInfo"))
                .flatMap(citingPaperInfo ->
                    Optional.ofNullable(
                        citingPaperInfo.optJSONObject("openAccessPdf")
                    )
                )
                .flatMap(openAccessPdf ->
                    Optional.ofNullable(openAccessPdf.optString("disclaimer"))
                )
                .ifPresent(
                    Unchecked.consumer(disclaimer -> {
                        LOGGER.debug(
                            "Received a disclaimer from Semantic Scholar: {}",
                            disclaimer
                        );
                        if (disclaimer.contains("references")) {
                            throw new FetcherException(
                                Localization.lang(
                                    "Restricted access to references: %0",
                                    disclaimer
                                )
                            );
                        }
        ...
        return referencesResponse
            ...
            .stream()
            .filter(
                citationDataItem -> citationDataItem.getCitedPaper() != null
            )

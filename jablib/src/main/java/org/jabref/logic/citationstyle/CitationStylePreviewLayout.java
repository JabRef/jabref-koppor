...
public final class CitationStylePreviewLayout implements PreviewLayout {
    ...
    public String generatePreview(BibEntry entry, BibDatabaseContext databaseContext) {
        if (!citationStyle.hasBibliography()) {
            ...
            return CitationStyleGenerator.generateCitation(List.of(entry), citationStyle.getSource(),
                    CitationStyleOutputFormat.HTML, databaseContext, bibEntryTypesManager);
        }
        return CitationStyleGenerator
            .generateBibliography(List.of(entry), citationStyle.getSource(), CitationStyleOutputFormat.HTML,

...
/**
 * Keeps all essential data for establishing a new connection to a DBMS using
 * {@link DBMSConnection}.
 */
public class DBMSConnectionProperties implements DatabaseConnectionProperties {
    ...
    /**
     * Gets all required data from {@link SharedDatabasePreferences} and sets them if
     * present.
     */
    public DBMSConnectionProperties(SharedDatabasePreferences prefs) {
        ...
        if (prefs.getUser().isPresent()) {
            ...
            if (prefs.getPassword().isPresent()) {
                try {
                    this.password = new Password(prefs.getPassword().get().toCharArray(), prefs.getUser().get())
                        .decrypt();
                }
        ...
        if (prefs.getPassword().isEmpty()) {
            // Some DBMS require a non-null value as a password (in case of using an empty
            // string).
            this.password = "";
        }
    }

    DBMSConnectionProperties(DBMSType type, String host, int port, String database, String user, String password,
            boolean useSSL, boolean allowPublicKeyRetrieval, String serverTimezone, String keyStore, String jdbcUrl,
            boolean expertMode) {
        this.type = type;
        this.host = host;
        this.port = port;
    ...
    public boolean equals(Object obj) {
        ...
        return Objects.equals(type, properties.getType()) && this.host.equalsIgnoreCase(properties.getHost())
                && Objects.equals(port, properties.getPort()) && Objects.equals(database, properties.getDatabase())
                && Objects.equals(user, properties.getUser()) && Objects.equals(useSSL, properties.isUseSSL())
    ...
    @Override
    public int hashCode() {
        return Objects.hash(type, host, port, database, user, useSSL, allowPublicKeyRetrieval, serverTimezone, keyStore,
                jdbcUrl, expertMode);
    }

    @Override
    public boolean isValid() {
        return type != null && host != null && port > 0 && database != null && user != null && password != null;
    }

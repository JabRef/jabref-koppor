...
public class DBMSConnection implements DatabaseConnection {
    ...
    public DBMSConnection(DBMSConnectionProperties connectionProperties)
        ...
        try {
            ...
            // we use the side effect of getAvailableDBMSTypes() - it loads all available
            // drivers
            DBMSConnection.getAvailableDBMSTypes();

            if (connectionProperties.isUseExpertMode()) {
                this.connection = DriverManager.getConnection(connectionProperties.getJdbcUrl(),
                        connectionProperties.asProperties());
            }
            else {
                this.connection = DriverManager.getConnection(connectionProperties.getUrl(),
                        connectionProperties.asProperties());
            }
        }
    ...
    public static Set<DBMSType> getAvailableDBMSTypes() {
        ...
        for (DBMSType dbms : DBMSType.values()) {
            try {
                ...
                // In case that the driver is not available do not perform tests for this
                // system.

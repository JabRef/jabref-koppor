...
public class CSLUpdateBibliography {
    ...
    public Optional<XTextRange> getBibliographyRange(XTextDocument doc)
        ...
        Optional<XTextRange> range = UnoTextSection.getAnchor(
            doc,
            BIBLIOGRAPHY_SECTION_NAME
        );
    ...
    public void rebuildCSLBibliography(
        ...
        if (sectionRange.isEmpty()) {
            LOGGER.debug(
                "Bibliography section not found. Creating new section."
            );
    ...
    private void clearCSLBibTextSectionContent(XTextDocument doc)
        ...
        if (sectionRange.isPresent()) {
            ...
            LOGGER.warn(
                "Failed to clear CSL bibliography section: section not found"
            );
        }
    }
    ...
    private void populateCSLBibTextSection(
        ...
        if (sectionRange.isEmpty()) {
            LOGGER.error(
                "Bibliography section not found when trying to populate"
            );
        ...
        cslCitationOOAdapter.insertBibliography(
            cursor,
            citationStyle,
            entries,
            bibDatabaseContext,

...
/**
 * Contains utility constants and methods for processing of CSL citations as generated by
 * methods of <a href="https://github.com/michel-kraemer/citeproc-java">citeproc-java</a>
 * ({@link org.jabref.logic.citationstyle.CitationStyleGenerator}).
 * <p>
 * These methods are used in {@link CSLCitationOOAdapter} which inserts CSL citation text
 * into an OO document.
 * </p>
 */
public final class CSLFormatUtils {
    ...
    /**
     * The formatting names specified to and understood by LibreOffice internally via code
     * are often different from their displayed names in the LibreOffice application. For
     * example, "Hanging Indent" as visible in LO styles list should be specified as
     * "Hanging indent" (note casing) internally. Similarly, "Body Text, Indented" is
     * specified as "Text body indent". This is also why we don't let users manually input
     * the name of any pre-set format. The following two lists below are taken from
     * https://github.com/LibreOffice/core/blob/0891df6b21fd95ec7c9614509d92829c0f17c353/sw/qa/python/check_styles.py#L132
     */
    public static final List<String> BIBLIOGRAPHY_TITLE_FORMATS = List.of("Bibliography Heading", "Title", "Text body",
            "Subtitle", "Heading 1", "Heading 2", "Heading 3", "Heading 4");

    public static final List<String> BIBLIOGRAPHY_BODY_FORMATS = List.of("Bibliography 1", "Text body",
            "Text body indent", "First line indent", "Hanging indent", "List", "List Indent", "Marginalia");
    ...
    /**
     * Transforms provided HTML into a format that can be fully parsed and inserted into
     * an OO document. Context: The HTML produced by
     * {@link org.jabref.logic.citationstyle.CitationStyleGenerator#generateBibliography(List, String, CitationStyleOutputFormat, BibDatabaseContext, BibEntryTypesManager)
     * generateBibliography} or
     * {@link org.jabref.logic.citationstyle.CitationStyleGenerator#generateCitation(List, String, CitationStyleOutputFormat, BibDatabaseContext, BibEntryTypesManager)
     * generateCitation} is not directly (completely) parsable by
     * {@link OOTextIntoOO#write(XTextDocument, XTextCursor, OOText) write}. For more
     * details, read the documentation for the {@link OOTextIntoOO} class. <a href=
     * "https://devdocs.jabref.org/code-howtos/openoffice/code-reorganization.html">Additional
     * Information</a>.
    ...
    public static String transformHTML(String html) {
        ...
        html = html.replaceAll("<div class=\"csl-left-margin\">(.*?)</div><div class=\"csl-right-inline\">(.*?)</div>",
                "$1 $2");
        ...
        // Remove leading paragraph tags (preserving any whitespaces after them for
        // indentation)
        ...
        // In bibliography entries, citeproc-java adds 4 leading spaces for numeric
        // styles, and 2 for non-numeric
        html = html.trim();
    ...
    /**
     * Alphanumeric citations are not natively supported by citeproc-java (see
     * {@link org.jabref.logic.citationstyle.CitationStyleGenerator#generateCitation(List, String, CitationStyleOutputFormat, BibDatabaseContext, BibEntryTypesManager)
     * generateCitation}). Thus, we manually format a citation to produce its alphanumeric
     * form.
     * @param entries the list of entries for which the alphanumeric citation is to be
     * generated.
    ...
    public static String generateAlphanumericCitation(List<BibEntry> entries, BibDatabaseContext bibDatabaseContext) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            Optional<String> author = entry.getResolvedFieldOrAlias(StandardField.AUTHOR,
            ...
            if (author.isPresent() && year.isPresent()) {
                ...
                String shortYear = year.get().length() >= 2 ? year.get().substring(year.get().length() - 2)
                        : year.get();
    ...
    /**
     * Method to update citation number of a bibliographic entry (to be inserted in the
     * list of references). By default, citeproc-java
     * ({@link org.jabref.logic.citationstyle.CitationStyleGenerator#generateBibliography(List, String, CitationStyleOutputFormat, BibDatabaseContext, BibEntryTypesManager)
     * generateBibliography}) always starts the numbering of a list of references with
     * "1". If a citation doesn't correspond to the first cited entry, the number should
     * be changed to the appropriate current citation number. The numbers should be
     * globally unique. If an entry has been cited before, the older citation number
     * corresponding to it should be reused. The number can be enclosed in different
     * formats, such as "1", "1.", "1)", "(1)" or "[1]".
     * <p>
    ...
    /**
     * Extracts year from a citation having single or multiple entries, for the purpose of
     * using in in-text citations.
     * @param formattedCitation the citation cleaned up and formatted using
     * {@link CSLFormatUtils#transformHTML transformHTML}.
     */

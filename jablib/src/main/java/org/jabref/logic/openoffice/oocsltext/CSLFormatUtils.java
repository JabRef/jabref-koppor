...
public final class CSLFormatUtils {
    ...
    private static final String DEFAULT_HANGING_INDENT_BIBLIOGRAPHY_BODY_FORMAT =
        "Hanging indent";

    private static final Pattern YEAR_IN_CITATION_PATTERN = Pattern.compile(
        "(.)(.*), (\\d{4}.*)"
    );
    ...
    public static String generateAlphanumericCitation(
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            Optional<String> author = entry.getResolvedFieldOrAlias(
                StandardField.AUTHOR,
                bibDatabaseContext.getDatabase()
            );
            Optional<String> year = entry.getResolvedFieldOrAlias(
                StandardField.YEAR,
                bibDatabaseContext.getDatabase()
            );
    ...
    public static String generateAlphanumericInTextCitation(
        BibEntry entry,
        BibDatabaseContext bibDatabaseContext
    ) {
        String inTextCitation = generateAlphanumericCitation(
            List.of(entry),
            bibDatabaseContext
        );

        String authorName = entry
            ...
            .map(list ->
                BracketedPattern.joinAuthorsOnLastName(list, 1, "", " et al.")
            )
            .orElse("");
     ...
     */
    public static String updateSingleBibliographyNumber(
        String citation,
        int currentNumber
    ) {
        ...
        while (matcher.find()) {
            if (!numberReplaced) {
                String prefix = matcher.group(1) != null
                    ? matcher.group(1)
                    : "";
                ...
                String replacement =
                    prefix + currentNumber + suffix + dot + space;
     ...
     */
    public static String generateAuthorPrefix(

 ...
 * <p>
 * To check that our protected ranges do not overlap, we collect these ranges. To check
 * for overlaps between these, we need the {@code range} itself. To report the results of
 * overlap checking, we need a {@code description} that can be understood by the user.
 * <p>
 * To be able to refer back to more extended data, we might need to identify its
 * {@code kind}, and its index in the corresponding tables or other identifier within its
 * kind ({@code idWithinKind})
 */

...
/**
 * This class processes CSL citations in JabRef and interacts directly with LibreOffice
 * using an XTextDocument instance. It is tightly coupled with
 * {@link CSLReferenceMarkManager} for management of reference marks tied to the CSL
 * citations. It uses {@link OpenOfficePreferences} to retrieve the initial style (last
 * selected style), the bibliography title and its paragraph style. Any method in this
 * class is NOT supposed to be moved (OR internally refactored without complete
 * understanding - see implementation note).
 *
 * @implNote UNO API calls are expensive, and any additional operation slows down the net
 * "macro-task" we are trying to achieve in the document. These "additional" operations
 * may or may not be visible at the level of code in the form of additional function
 * calls. In some cases, the same macro-task may be achieved by two different orders of
 * actions, which may look semantically the same overall, but one order may result into
 * more UNO API calls. For example, see the comment inside
 * {@link CSLCitationOOAdapter#insertCitation(XTextCursor, CitationStyle, List, BibDatabaseContext, BibEntryTypesManager)
 * insertCitation}.
 */
public class CSLCitationOOAdapter {
    ...
    public CSLCitationOOAdapter(XTextDocument doc, Supplier<List<BibDatabaseContext>> databasesSupplier,
            OpenOfficePreferences openOfficePreferences, BibEntryTypesManager bibEntryTypesManager)
        ...
        OOStyle initialStyle = openOfficePreferences.getCurrentStyle(); // may be a
                                                                        // jstyle, can
                                                                        // still be used
                                                                        // for detecting
                                                                        // subsequent
        ...
        if (initialStyle instanceof CitationStyle citationStyle) {
            this.currentStyle = citationStyle; // else the currentStyle purposely stays
                                               // null, still causing a difference with
                                               // the subsequent style if CSL (valid
                                               // comparison)
        }
    ...
    /**
     * Inserts a citation for a group of entries. Comparable to LaTeX's \cite command.
     */
    public void insertCitation(XTextCursor cursor, CitationStyle selectedStyle, List<BibEntry> entries,
            BibDatabaseContext bibDatabaseContext, BibEntryTypesManager bibEntryTypesManager)
            throws CreationException, Exception {
        setStyle(selectedStyle);

        // Placing this at the beginning reduces the number of updates needed by 1 (in the
        // positive case)
        if (styleChanged) {
        ...
        if (isAlphanumericStyle) {
            ...
            citation = CitationStyleGenerator.generateCitation(entries, style, HTML_OUTPUT_FORMAT, bibDatabaseContext,
    ...
    /**
     * Inserts in-text citations for a group of entries. Comparable to LaTeX's \citet
     * command.
     *
     * @implNote Very similar to the
     * {@link #insertCitation(XTextCursor, CitationStyle, List, BibDatabaseContext, BibEntryTypesManager)
     * insertCitation} method.
     */
    public void insertInTextCitation(XTextCursor cursor, CitationStyle selectedStyle, List<BibEntry> entries,
            BibDatabaseContext bibDatabaseContext, BibEntryTypesManager bibEntryTypesManager)
            throws CreationException, Exception {
        setStyle(selectedStyle);
        ...
        while (iterator.hasNext()) {
            ...
            if (isAlphanumericStyle) {
                ...
                inTextCitation = CitationStyleGenerator.generateCitation(List.of(currentEntry), style,
    ...
    /**
     * Inserts "empty" citations for a list of entries at the cursor to the document. Adds
     * the entries to the list for which bibliography is to be generated.
     */
     ...
     * The list is generated based on the existing citations, in-text citations and empty
     * citations in the document.
     */
    public void insertBibliography(XTextCursor cursor, CitationStyle selectedStyle, List<BibEntry> entries,
            BibDatabaseContext bibDatabaseContext, BibEntryTypesManager bibEntryTypesManager)
        ...
        OOText title = OOFormat.paragraph(OOText.fromString(openOfficePreferences.getCslBibliographyTitle()),
                openOfficePreferences.getCslBibliographyHeaderFormat());
        OOTextIntoOO.write(document, cursor, OOText.fromString(title.toString()));
        OOText ooBreak = OOFormat.paragraph(OOText.fromString(""),
        ...
        if (isNumericStyle) {
            ...
            for (BibEntry entry : entries) {
                String bibliographyEntry = CitationStyleGenerator
                    .generateBibliography(List.of(entry), style, HTML_OUTPUT_FORMAT, bibDatabaseContext,
                            bibEntryTypesManager)
                    .getFirst();
                ...
                formattedBibliographyEntry = CSLFormatUtils.updateSingleBibliographyNumber(formattedBibliographyEntry,
                        currentNumber);
            ...
            List<String> bibliographyEntries = CitationStyleGenerator.generateBibliography(entries, style,
                    HTML_OUTPUT_FORMAT, bibDatabaseContext, bibEntryTypesManager);
    ...
    /**
     * Inserts references and also adds a space before the citation if not already present
     * ("smart space").
     */
    private void insertReferences(XTextCursor cursor, List<BibEntry> entries, OOText ooText, boolean isNumericStyle)
        ...
        // Check if we're at the start of the document - if yes we set the flag and don't
        // insert a space
        ...
        markManager.insertReferenceIntoOO(entries, document, cursor, ooText, !preceedingSpaceExists,
    ...
    /**
     * Transforms the numbers in the citation to globally-unique (and thus, reusable)
     * numbers.
     */
    ...
    /**
     * Ideally, the methods of this class are supposed to work with
     * {@link CSLReferenceMarkManager}, and not {@link CSLReferenceMark} directly.
     * However, all "generation" of CSL style citations (via
     * {@link CitationStyleGenerator}) occur in this class, and not in
     * {@link CSLReferenceMarkManager}. Furthermore, {@link CSLReferenceMarkManager} is
     * not composed of {@link CitationStyle}. Hence, we keep
     * {@link CSLReferenceMarkManager} independent of {@link CitationStyleGenerator} and
     * {@link CitationStyle}, and keep the following two methods here.
     */
    private void updateAllCitationsWithNewStyle(CitationStyle style, boolean isInTextStyle)
        ...
        /*
         * Entries from multiple libraries may need to be updated, and new libraries could
         * have been opened after the document connection So, to get all databases in real
         * time without having to refresh the connection, we obtain all open databases via
         * the state manager
         */
        ...
        // Next, we get the list of reference marks sorted in order of appearance in the
        // document
        ...
        if (isInTextStyle) {
            ...
            for (CSLReferenceMark mark : marksInOrder) {
                ...
                while (iterator.hasNext()) {
                    ...
                    // We re-generate the citation in the new style and update it in the
                    // document
                    String newCitation;

                    if (isAlphaNumericStyle) {
                        newCitation = CSLFormatUtils.generateAlphanumericInTextCitation(currentEntry,
                                unifiedBibDatabaseContext);
                    }
                    else {
                        newCitation = CitationStyleGenerator.generateCitation(List.of(currentEntry), style.getSource(),
                    ...
                    if (isNumericStyle) {
                        formattedCitation = updateSingleOrMultipleCitationNumbers(formattedCitation,
            ...
            // Same flow as above - for each such reference mark, we get the entries to be
            // updated
            for (CSLReferenceMark mark : marksInOrder) {
                List<String> citationKeys = mark.getCitationKeys();
                List<BibEntry> entries = citationKeys.stream()
    ...
    /**
     * Checks if an entry has already been cited before in the document. Required for
     * consistent numbering of numeric citations - if present, the number is to be reused,
     * else a new number is to be assigned.
     */

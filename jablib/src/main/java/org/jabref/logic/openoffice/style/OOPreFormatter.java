 ...
 * bibliography style. It handles translation of LaTeX italic/bold commands into HTML
 * tags.
 */
public class OOPreFormatter implements LayoutFormatter {
            ...
            if (escaped && (c == '\\')) {
                ...
                if (!incommand) {
                    ...
                    if (!tag.isEmpty()) {
                        ...
                        // constructs like {\aa}. The
                        // correct behaviour should be to substitute the evaluated command
                        // and swallow the brace:
                        String result = OOPreFormatter.CHARS.get(command);
    ...
    private String getHTMLTag(String latexCommand) {
        String result = "";
        switch (latexCommand) {
            // Should really separate between emphasized and italic but since in later
            // stages both are converted to italic...
            case "textit", "it", "emph", "em" -> result = "i"; // Italic
            case "textbf", "bf" -> result = "b"; // Bold font
            case "textsc" -> result = "smallcaps"; // Small caps
                                                   // Not a proper HTML tag, but used here
                                                   // for convenience
            case "underline" -> result = "u"; // Underline
            case "sout" -> result = "s"; // Strikeout
                                         // sout is the "standard" command, although it is
                                         // actually based on the package ulem

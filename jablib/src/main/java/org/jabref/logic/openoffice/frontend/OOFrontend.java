...
public class OOFrontend {
    ...
    public OOFrontend(XTextDocument doc) throws NoDocumentException, WrappedTargetException {
        ...
        Map<CitationGroupId, CitationGroup> citationGroups = readCitationGroupsFromDocument(this.backend, doc,
                citationGroupNames);
    ...
    private static Map<CitationGroupId, CitationGroup> readCitationGroupsFromDocument(Backend52 backend,
            XTextDocument doc, List<String> citationGroupNames) throws WrappedTargetException, NoDocumentException {
    ...
    /**
     * Creates a list of {@code RangeSortable<CitationGroup>} values for our
     * {@code CitationGroup} values. Originally designed to be passed to
     * {@code visualSort}.
     * <p>
     * The elements of the returned list are actually of type
     * {@code RangeSortEntry<CitationGroup>}.
     * <p>
     * The result is sorted within {@code XTextRange.getText()} partitions of the citation
     * groups according to their {@code XTextRange} (before mapping to footnote marks).
     * <p>
     * In the result, RangeSortable.getIndexInPosition() contains unique indexes within
     * the original partition (not after mapFootnotesToFootnoteMarks).
     * @param mapFootnotesToFootnoteMarks If true, replace ranges in footnotes with the
     * range of the corresponding footnote mark. This is used for numbering the citations.
     */
    private List<RangeSortable<CitationGroup>> createVisualSortInput(XTextDocument doc,
         ...
         *
         * But we may want to number citations in a footnote as if it appeared where the
         * footnote mark is.
         *
         * The following code replaces ranges within footnotes with the range for the
         * corresponding footnote mark.
         *
         * This brings further ambiguity if we have multiple citation groups within the
         * same footnote: for the comparison they become indistinguishable. Numbering
         * between them is not controlled. Also combineCiteMarkers will see them in the
         * wrong order (if we use this comparison), and will not be able to merge. To
         * avoid these, we sort textually within each .getText() partition and add
         * indexInPosition accordingly.
         *
         */

        // Sort within partitions
        RangeSort.RangePartitions<RangeSortEntry<CitationGroup>> partitions = RangeSort
            .partitionAndSortRanges(sortables);
    ...
    /**
     * @param mapFootnotesToFootnoteMarks If true, sort reference marks in footnotes as if
     * they appeared at the corresponding footnote mark.
     * @return citation groups sorted by their visual positions. Limitation: for two
     * column layout visual (top-down, left-right) order does not match the expected
     * (textual) order.
     */
    private List<CitationGroup> getVisuallySortedCitationGroups(XTextDocument doc, boolean mapFootnotesToFootnoteMarks,
            FunctionalTextViewCursor fcursor) throws WrappedTargetException, NoDocumentException {
     ...
     * <p>
     * This is (1) sufficient for combineCiteMarkers which looks for consecutive
     * XTextRanges within each XText, (2) not confused by multicolumn layout or multipage
     * display.
     */
    public List<CitationGroup> getCitationGroupsSortedWithinPartitions(XTextDocument doc,
            boolean mapFootnotesToFootnoteMarks) throws NoDocumentException, WrappedTargetException {
     ...
     * <p>
     * On return {@code position} is collapsed, and is after the inserted space, or at the
     * end of the reference mark.
     * @param citationKeys In storage order
     * @param pageInfos In storage order
     * @param position Collapsed to its end.
     * @param insertSpaceAfter If true, we insert a space after the mark, that carries on
     * format of characters from the original position.
     */
    public CitationGroup createCitationGroup(XTextDocument doc, List<String> citationKeys,
            List<Optional<OOText>> pageInfos, CitationType citationType, XTextCursor position, boolean insertSpaceAfter)
        ...
        CitationGroup group = backend.createCitationGroup(doc, citationKeys, pageInfos, citationType, position,
                insertSpaceAfter);
    ...
    public List<RangeForOverlapCheck<CitationGroupId>> citationRanges(XTextDocument doc)
        ...
        for (CitationGroup group : citationGroups.getCitationGroupsUnordered()) {
            ...
            result.add(new RangeForOverlapCheck<>(range, group.groupId, RangeForOverlapCheck.REFERENCE_MARK_KIND,
                    description));
        }
        return result;
    }
    ...
    public List<RangeForOverlapCheck<CitationGroupId>> bibliographyRanges(XTextDocument doc)
        ...
        if (range.isPresent()) {
            ...
            result.add(new RangeForOverlapCheck<>(range.get(), new CitationGroupId("bibliography"),
                    RangeForOverlapCheck.BIBLIOGRAPHY_MARK_KIND, description));
        }
        return result;
    }
    ...
    public List<RangeForOverlapCheck<CitationGroupId>> viewCursorRanges(XTextDocument doc) {
        ...
        if (range.isPresent()) {
            ...
            result.add(new RangeForOverlapCheck<>(range.get(), new CitationGroupId("cursor"),
                    RangeForOverlapCheck.CURSOR_MARK_KIND, description));
        }
        return result;
    }

    /**
     * @return A range for each footnote mark where the footnote contains at least one
     * citation group. Purpose: We do not want markers of footnotes containing reference
     * marks to overlap with reference marks. Overwriting these footnote marks might kill
     * our reference marks in the footnote.
     * <p>
     * Note: Here we directly communicate to the document, not through the backend. This
     * is because mapping ranges to footnote marks does not depend on how do we mark or
     * structure those ranges.
     */
    public List<RangeForOverlapCheck<CitationGroupId>> footnoteMarkRanges(XTextDocument doc,
        ...
        List<RangeForOverlapCheck<CitationGroupId>> result = new ArrayList<>();
        RangeSort.RangePartitions<RangeForOverlapCheck<CitationGroupId>> partitions = RangeSort
            .partitionRanges(citationRanges);
        ...
        // Also: different footnotes cannot share a footnotemark range, we are not
        // creating duplicates.
        for (List<RangeForOverlapCheck<CitationGroupId>> partition : partitions.getPartitions()) {
            ...
            result.add(new RangeForOverlapCheck<>(footnoteMarkRange.get(), citationRange.idWithinKind,
                    RangeForOverlapCheck.FOOTNOTE_MARK_KIND, "FootnoteMark for " + citationRange.format()));
        }
        return result;
    }
    ...
    public OOVoidResult<JabRefException> checkRangeOverlapsWithCursor(XTextDocument doc,
            List<RangeForOverlapCheck<CitationGroupId>> userRanges, boolean requireSeparation)
            throws NoDocumentException, WrappedTargetException {
        ...
        List<RangeOverlap<RangeForOverlapCheck<CitationGroupId>>> overlaps = RangeOverlapBetween.findFirst(doc,
                userRanges, ranges, requireSeparation);

        if (overlaps.isEmpty()) {
            return OOVoidResult.ok();
        }
        return OOVoidResult
            .error(new JabRefException("Found overlapping or touching ranges", rangeOverlapsToMessage(overlaps)));
    }
    ...
    public OOVoidResult<JabRefException> checkRangeOverlaps(XTextDocument doc,
            List<RangeForOverlapCheck<CitationGroupId>> userRanges, boolean requireSeparation, int reportAtMost)
            throws NoDocumentException, WrappedTargetException {
        ...
        List<RangeOverlap<RangeForOverlapCheck<CitationGroupId>>> overlaps = RangeOverlapWithin
            .findOverlappingRanges(doc, ranges, requireSeparation, reportAtMost);
    ...
    /**
     * GUI: Get a list of CitationEntry objects corresponding to citations in the
     * document.
     * <p>
     * Called from: ManageCitationsDialogViewModel constructor.
     * @return A list with entries corresponding to citations in the text, in arbitrary
     * order (same order as from getJabRefReferenceMarkNames). Note: visual or alphabetic
     * order could be more manageable for the user. We could provide these here, but
     * switching between them needs change on GUI (adding a toggle or selector).
     * <p>
     * Note: CitationEntry implements Comparable, where compareTo() and equals() are based
     * on refMarkName. The order used in the "Manage citations" dialog does not seem to
     * use that.
     * <p>
     * The 1st is labeled "Citation" (show citation in bold, and some context around it).
     * <p>
     * The columns can be sorted by clicking on the column title. For the "Citation"
     * column, the sorting is based on the content, (the context before the citation), not
     * on the citation itself.
     * <p>
     * In the "Extra information ..." column some visual indication of the editable part
     * could be helpful.
     * <p>
    ...
    public void imposeGlobalOrder(XTextDocument doc, FunctionalTextViewCursor fcursor)
        ...
        List<CitationGroup> sortedCitationGroups = getVisuallySortedCitationGroups(doc, mapFootnotesToFootnoteMarks,
                fcursor);

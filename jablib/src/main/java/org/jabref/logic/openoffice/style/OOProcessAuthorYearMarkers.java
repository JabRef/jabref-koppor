...
class OOProcessAuthorYearMarkers {
    ...
    /**
     * For each cited source make the citation keys unique by setting the uniqueLetter
     * fields to letters ("a", "b") or Optional.empty()
     *
     * precondition: sortedCitedKeys already has normalized citation markers.
     * precondition: sortedCitedKeys is sorted (according to the order we want the letters
     * to be assigned)
     *
     * Expects to see data for all cited sources here. Clears uniqueLetters before
     * filling.
     *
     * On return: Each citedKey in sortedCitedKeys has uniqueLetter set as needed. The
     * same values are copied to the corresponding citations in citationGroups.
     *
     ...
     * ***************************************
     *
     * Calculate presentation of citation groups (create citMarkers)
     *
     **************************************/

...
class JStyleGetNumCitationMarker {
     ...
     *
     * To support for example numbers in superscript without brackets for the text, but
     * "[1]" form for the bibliography, the style can provide the optional
     * "BracketBeforeInList" and "BracketAfterInList" strings to be used in the
     * bibliography instead of "BracketBefore" and "BracketAfter"
     * @return "[${number}]" where "[" stands for BRACKET_BEFORE_IN_LIST (with fallback
     * BRACKET_BEFORE) "]" stands for BRACKET_AFTER_IN_LIST (with fallback BRACKET_AFTER)
     * "${number}" stands for the formatted number.
     */
    public static OOText getNumCitationMarkerForBibliography(JStyle style, CitationMarkerNumericBibEntry entry) {
        ...
        stringBuilder.append(current.isPresent() ? String.valueOf(current.get())
     ...
     *
     * Given a block containing either a single entry or two or more entries that are
     * joinable into an "i-j" form, append to {@code stringBuilder} the formatted text.
     *
     * Assumes:
     *
     * - block is not empty
     *
     * - For a block with a single element the element may have pageInfo and its num part
     * may be Optional.empty()
     *
     * - For a block with two or more elements
     *
     * - The elements do not have pageInfo and their number part is not empty.
     *
     * - The elements number parts are consecutive positive integers, without repetition.
     *
     */
    private static void emitBlock(List<CitationMarkerNumericEntry> block, JStyle style, int minGroupingCount,
        ...
        if (blockSize == 1) {
            ...
            stringBuilder.append(num.isEmpty() ? (JStyle.UNDEFINED_CITATION_MARKER + entry.getCitationKey())
     ...
     *
     * An Optional.empty() number means: could not look this up in the databases. Positive
     * integers are the valid numbers.
     *
     ...
     * @param minGroupingCount Zero and negative means never group. Only used by tests to
     * override the value in style.
     * @return The text for the citation.
     *
     */
    public static OOText getNumCitationMarker2(JStyle style, List<CitationMarkerNumericEntry> entries,
        ...
        /*
         * Original: [2,3,4] -> [2-4] [0,1,2] -> [??,1,2] [0,1,2,3] -> [??,1-3]
         *
         * Now we have to consider: duplicate numbers and pageInfos [1,1] -> [1] [1,1
         * "pp nn"] -> keep separate if pageInfo differs [1 "pp nn",1 "pp nn"] -> [1
         * "pp nn"]
         */
        ...
        for (int i = 0; i < nCitations; i++) {
            ...
            if (currentBlock.isEmpty()) {
                ...
                else if ((current.getNumber().get() == (prev.getNumber().get() + 1)) && (prev.getPageInfo().isEmpty())

...
public class Pseudonymization {
    ...
    public Result pseudonymizeLibrary(BibDatabaseContext bibDatabaseContext) {
        ...
        List<BibEntry> newEntries = pseudonymizeEntries(
            bibDatabaseContext,
            fieldToValueToIdMap
        );
        ...
        fieldToValueToIdMap.forEach((field, stringToIntMap) ->
            stringToIntMap.forEach((value, id) ->
                valueMapping.put(
                    field.getName().toLowerCase(Locale.ROOT) + "-" + id,
                    value
                )
            )
        );
        ...
        for (BibEntry entry : entries) {
            ...
            for (Field field : entry.getFields()) {
                Map<String, Integer> valueToIdMap =
                    fieldToValueToIdMap.computeIfAbsent(field, k ->
                        new HashMap<>()
                    );
                ...
                Integer id = valueToIdMap.computeIfAbsent(
                    fieldContent,
                    k -> valueToIdMap.size() + 1
                );

...
public record CitationCommandString(@NonNull String prefix, @NonNull String delimiter, @NonNull String suffix) {
    ...
    public static CitationCommandString from(String completeCiteCommand) {
        ...
        if (indexKey1 < 0 || indexKey2 < 0 || indexKey2 < (indexKey1 + CITE_KEY1.length())) {
            LOGGER.info("Wrong indexes {} {} for completeCiteCommand {}. Using default delimiter and suffix.",
                    indexKey1, indexKey2, completeCiteCommand);
        ...
        String delim = completeCiteCommand.substring(completeCiteCommand.lastIndexOf(CITE_KEY1) + CITE_KEY1.length(),
                indexKey2);

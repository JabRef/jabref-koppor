...
public class JournalAbbreviationLoader {
    ...
    public static JournalAbbreviationRepository loadRepository(
        ...
        try (
            ...
            if (resourceAsStream == null) {
                ...
                LOGGER.info(
                    "Loaded journal abbreviations from {}",
                    tempJournalList.toAbsolutePath()
                );
            }
        } catch (IOException e) {
            LOGGER.error(
                "Error while loading journal abbreviation repository",
                e
            );
            return null;
        }
    ...
    private static LtwaRepository loadLtwaRepository() throws IOException {
        try (
            InputStream resourceAsStream =
                JournalAbbreviationRepository.class.getResourceAsStream(
                    "/journals/ltwa-list.mv"
                )
        ) {
            if (resourceAsStream == null) {
                LOGGER.warn(
                    "There is no ltwa-list.mv. We cannot load the LTWA repository."
                );
                ...
                LtwaRepository ltwaRepository = new LtwaRepository(
                    tempLtwaList
                );
                tempDir.toFile().deleteOnExit();
    ...
    public static JournalAbbreviationRepository loadBuiltInRepository() {
        return loadRepository(
            new JournalAbbreviationPreferences(List.of(), true)
        );
    }
}

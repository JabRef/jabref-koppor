 ...
 * <code>[year]_[author]_[firstpage]</code>, using information from a provided BibEntry.
 * The above-mentioned expression would yield <code>2017_Kitsune_123</code> when expanded
 * using the BibTeX entry <code>@Article{ authors = {O. Kitsune}, year = {2017},
 * pages={123-6}}</code>.
 * <p>
 * The embedding in JabRef is explained at
 * <a href="https://docs.jabref.org/setup/citationkeypattern">Customize the citation key
 * generator</a>.
 * </p>
 */
public class BracketedPattern {
    ...
    /**
     * The maximum number of name abbreviations that can be used. If there are more
     * authors, {@code MAX_ALPHA_AUTHORS -
    ...
    /**
     * Matches everything that is not an uppercase ASCII letter. The intended use is to
     * remove all lowercase letters
     */
    private static final Pattern NOT_CAPITAL_CHARACTER = Pattern.compile("[^A-Z]");

    /**
     * Matches uppercase english letters between "({" and "})", which should be used to
     * abbreviate the name of an institution
     */
    ...
    private enum Institution {
        ...
        SCHOOL, DEPARTMENT, UNIVERSITY, TECHNOLOGY;

        /**
         * Matches "uni" followed by "v" or "b", at the start of a string or after a
        ...
        /**
         * Find which types of institutions have words in common with the given name
         * parts.
         * @param nameParts a list of words that constitute parts of an institution's
         * name.
        ...
        public static EnumSet<Institution> findTypes(List<String> nameParts) {
            ...
            if (parts.contains(Institution.TECHNOLOGY)) {
                parts.remove(Institution.UNIVERSITY); // technology institute isn't
                                                      // university :-)
            }
    ...
    /**
     * Expands the current pattern using the given bibentry and database. ";" is used as
     * keyword delimiter.
     * @param bibentry The bibentry to expand.
     * @param database The database to use for string-lookups and cross-refs. May be null.
     * @return The expanded pattern. The empty string is returned, if it could not be
     * expanded.
     */
    ...
    /**
     * Expands the current pattern using the given bibentry, keyword delimiter, and
     * database.
     * @param bibentry The bibentry to expand.
     ...
     */
    public static String expandBrackets(String pattern, Character keywordDelimiter, BibEntry entry,
    ...
    /**
     * Utility method creating a function taking the string representation of the content
     * of a bracketed expression and expanding it.
     * @param keywordDelimiter The keyword delimiter to use
     * @param entry The {@link BibEntry} to use for expansion
     * @param database The {@link BibDatabase} for field resolving. May be null.
     * @return a function accepting a bracketed expression and returning the result of
     * expanding it
     */
    public static Function<String, String> expandBracketContent(Character keywordDelimiter, BibEntry entry,
            BibDatabase database) {
     ...
     * Expands a pattern.
     * @param pattern The pattern to expand
     * @param bracketContentHandler A function taking the string representation of the
     * content of a bracketed pattern and expanding it
     * @return The expanded pattern. Not null.
     */
    ...
    /**
     * Returns the content enclosed between brackets, including enclosed quotes, and
     * excluding the paired enclosing brackets. There may be brackets in it. Intended to
     * be used by
     * {@link BracketedPattern#expandBrackets(String, Character, BibEntry, BibDatabase)}
     * when a [ is encountered, and has been consumed, by the {@code StringTokenizer}.
    ...
    /**
     * Appends the content between, and including, two \" to the provided
     * <code>StringBuilder</code>. Intended to be used by
     * {@link BracketedPattern#expandBrackets(String, Character, BibEntry, BibDatabase)}
     * when a \" is encountered by the StringTokenizer.
     * @param stringBuilder the <code>StringBuilder</code> to which tokens will be
     * appended
     * @param tokenizer the tokenizer producing the tokens
     */
     ...
     * @return String containing the evaluation result. Empty string if the pattern cannot
     * be resolved.
     */
    public static String getFieldValue(BibEntry entry, String pattern, Character keywordDelimiter,
            BibDatabase database) {
        try {
            if (pattern.startsWith("auth") || pattern.startsWith("pureauth")) {
                ...
                if (pattern.startsWith("pure")) {
                    // "pure" is used in the context of authors to resolve to authors only
                    // and not fallback to editors
                    // The other functionality of the pattern "ForeIni", ... is the same
                    // Thus, remove the "pure" prefix so the remaining code in this
                    // section functions correctly
                    //
                    pattern = pattern.substring(4);
                }
                else if (unparsedAuthors.isEmpty()) {
                    // special feature: A pattern starting with "auth" falls back to the
                    // editor
                ...
                return getCamelizedTitle_N(entry.getResolvedFieldOrAlias(StandardField.TITLE, database).orElse(""),
                        num);
            }
    ...
    /**
     * Parses the provided string to an {@link AuthorList}, which are then formatted by
     * {@link LatexToUnicodeAdapter}. Afterward, any institutions are formatted into an
     * institution key.
     * @param unparsedAuthors a string representation of authors or editors
     * @return an {@link AuthorList} consisting of authors and institution keys with
     * resolved latex.
     */
    private static AuthorList createAuthorList(String unparsedAuthors) {
        return AuthorList.parse(unparsedAuthors).getAuthors().stream().map(author -> {
            // If the author is an institution, use an institution key instead of the full
            // name
            String lastName = author.getFamilyName()
                .map(lastPart -> isInstitution(author) ? generateInstitutionKey(lastPart)
                        : LatexToUnicodeAdapter.format(lastPart))
                .orElse(null);
            return new Author(author.getGivenName().map(LatexToUnicodeAdapter::format).orElse(null),
                    author.getGivenNameAbbreviated().map(LatexToUnicodeAdapter::format).orElse(null),
                    author.getNamePrefix().map(LatexToUnicodeAdapter::format).orElse(null), lastName,
                    author.getNameSuffix().map(LatexToUnicodeAdapter::format).orElse(null));
        }).collect(AuthorList.collect());
    }

    /**
     * Checks if an author is an institution which can get a citation key from
     * {@link #generateInstitutionKey(String)}.
     * @param author the checked author
     * @return true if only the last name is present and it contains at least one
     * whitespace character.
     */
    private static boolean isInstitution(Author author) {
        return author.getGivenName().isEmpty() && author.getGivenNameAbbreviated().isEmpty()
                && author.getNameSuffix().isEmpty() && author.getNamePrefix().isEmpty()
                && author.getFamilyName().isPresent() && WHITESPACE.matcher(author.getFamilyName().get()).find();
    }
            ...
            if ("abbr".equals(modifier)) {
                ...
                if (formatter.isPresent()) {
                    resultingLabel = formatter.get().format(resultingLabel);
                        ...
                        && modifier.endsWith(")")) {
                    // Alternate text modifier in parentheses. Should be inserted if the
                    // label is empty
                    if (label.isEmpty() && (modifier.length() > 2)) {
                        resultingLabel = expandBrackets(modifier.substring(1, modifier.length() - 1),
    ...
    /**
     * Capitalises and concatenates the words out of the "title" field in the given BibTeX
     * entry
     */
    ...
    public static String removeSmallWords(String title) {
        ...
        try (Scanner titleScanner = new Scanner(formattedTitle)) {
            return titleScanner.tokens().filter(Predicate.not(Word::isSmallerWord)).collect(Collectors.joining(" "));
        }
    }
     ...
     * @return the surname of an author/editor or the von part if no lastname is prsent or
     * "" if no author was found or both firstname+lastname are empty This method is
     * guaranteed to never return null.
     */
    private static String firstAuthor(AuthorList authorList) {
        return authorList.getAuthors()
            .stream()
            .findFirst()
    ...
    /**
     * Gets the von part and the last name of the first author/editor. No spaces are
     * returned.
     * @param authorList an {@link AuthorList}
     ...
     * @param authorList an {@link AuthorList}
     * @return the forename initial of an author/editor or "" if no author was found This
    ...
    private static String lastAuthorForenameInitials(AuthorList authorList) {
        ...
        return authorList.getAuthor(authorList.getNumberOfAuthors() - 1)
    ...
    /**
     * Gets the last name of all authors/editors. Pattern <code>[authors]</code>.
     * <p>
    ...
    public static String authorsAlpha(AuthorList authorList) {
        ...
        if (authorList.getNumberOfAuthors() == 1) {
            String[] firstAuthor = authorList.getAuthor(0)
                .getNamePrefixAndFamilyName()
                .replaceAll("\\s+", " ")
                .trim()
                .split(" ");
            ...
            List<String> vonAndLastNames = authorList.getAuthors()
                .stream()
                .limit(maxAuthors)
    ...
    /**
     * Returns the authors according to the
     * <a href="https://github.com/michel-kraemer/citeproc-java">BibTeX LNI template</a>
     * Examples: <a href=
     * "https://github.com/gi-ev/biblatex-lni/blob/main/basic-test-en.tex">Examples from
     * the tmplate</a> Also see discussion at the
     * <a href="https://github.com/JabRef/jabref/pull/11614">pull request that introduced
     * this</a>.
     * @param authorList an {@link AuthorList}
     * @return the initials of all authors' names
     */
    ...
    /**
     * Creates a string with all last names separated by a `delimiter`. If the number of
     * authors are larger than `maxAuthors`, replace all excess authors with `suffix`.
     ...
     * @param suffix to replace excess authors with. Also used to replace
     * <code>and others</code>.
     * @return a string consisting of authors' last names separated by a `delimiter` and
     * with any authors excess of `maxAuthors` replaced with `suffix`
     */
    public static String joinAuthorsOnLastName(AuthorList authorList, int maxAuthors, String delimiter,
            final String suffix) {
    ...
    /**
     * Gets the surnames of the first N authors and appends EtAl if there are more than N
     * authors
     * @param authorList an {@link AuthorList}
     * @param n the number of desired authors
     * @return Gets the surnames of the first N authors and appends EtAl if there are more
     * than N authors
     */
    ...
    /**
     * Gets the first part of the last name of the first author/editor, and appends the
     * last name initial of the remaining authors/editors. Maximum 5 characters
     * @param authorList an <{@link AuthorList}
     * @return the surname of all authors/editors
     */
    ...
    static String authEtal(AuthorList authorList, String delim, String append) {
        ...
        if ((authorList.getNumberOfAuthors() <= 2)
            ...
            // exception: If the second author is "and others", then do the appendix
            // handling (in the other branch)
    ...
    /**
     * The first N characters of the Mth author's or editor's last name. M starts counting
     * from 1. In case the Mth author is {@link Author#OTHERS}, <code>+</code> is
     * returned.
     */
     ...
     * @param n The maximum number of characters this string will be long. A negative
     * number or zero will lead to "" be returned.
     */
     ...
     * @return the non-digit prefix of pages (like "L" of L7) or "" if no non-digit prefix
        ...
        for (int i = 0; i < arg.length(); i++) {
            ...
            if ((currentChar == ':') && !escaped && (inParenthesis == 0)) {
                ...
                if (escaped) {
                    ...
                    // " -> regex("...", "...") - escaping should be passed through to the
                    // regex parser
                    // : -> :abc:def
                    current.append('\\');
                    current.append('\\');
    ...
    /**
     * <p>
     * An author or editor may be and institution not a person. In that case the key
     * generator builds very long keys, e.g.: for &ldquo;The Attributed Graph Grammar
     * System (AGG)&rdquo; -> &ldquo;TheAttributedGraphGrammarSystemAGG&rdquo;.
     * </p>
     *
     * <p>
     * An institution name should be inside <code>{}</code> brackets. If the institution
     * name includes its abbreviation this abbreviation should be in <code>{}</code>
     * brackets. For the previous example the value should look like:
     * <code>{The Attributed Graph Grammar System ({AGG})}</code>.
     * </p>
     *
     * <p>
     * If an institution includes its abbreviation, i.e. "...({XYZ})", first such
     * abbreviation should be used as the key value part of such author.
     * </p>
     *
     * <p>
     * If an institution does not include its abbreviation the key should be generated
     * from its name in the following way:
     * </p>
     *
     * <p>
     * The institution value can contain: institution name, part of the institution,
     * address, etc. These values should be comma separated. Institution name and possible
     * part of the institution should be in the beginning, while address and secondary
     * information should be in the end.
     * </p>
     * <p>
     * Each part is examined separately:
     * <ol>
     * <li>We remove all tokens of a part which are one of the defined ignore words (the,
     * press), which end with a dot (ltd., co., ...) and which first character is
     * lowercase (of, on, di, ...).</li>
     * <li>We detect the types of the part: university, technology institute, department,
     * school, rest
     * <ul>
     * <li>University: <code>"Uni[NameOfTheUniversity]"</code></li>
     * <li>Department: If the institution value contains more than one comma separated
     * part, the department will be an abbreviation of all words beginning with the
     * uppercase letter except of words: <code>d[ei]p.*</code>, school, faculty</li>
     * <li>School: same as department</li>
     * <li>Rest: If there are less than 3 tokens in such part than the result is a
     * concatenation of those tokens. Otherwise, the result will be built from the first
     * letter in each token.</li>
     * </ul>
     * </ol>
     * <p>
     ...
     * <li>If there is a department part and it is not same as school part append it.</li>
     * </ul>
     * <p>
     * Rest part is only the first part which do not match any other type. All other parts
     * (address, ...) are ignored.
    ...
    static String generateInstitutionKey(String content) {
        ...
        // Special characters can't be allowed past this point because the citation key
        // generator might replace them with multiple mixed-case characters
        ...
        for (int index = 0; index < institutionNameTokens.length; index++) {
            ...
            if (tokenTypes.contains(Institution.UNIVERSITY)) {
                ...
                if (tokenParts.size() >= 3) {
                    // If there are more than 3 parts, only keep the first character of
                    // each word
        ...
        return (university == null ? Objects.toString(rest, "") : university) + (school == null ? "" : school)
                + ((department == null) || (department.equals(school)) ? "" : department);
    }

    /**
     * Helper method for {@link BracketedPattern#generateInstitutionKey(String)}. Checks
     * that the word is not an institution keyword and has an uppercase first letter,
     * except univ/tech key word.
     * @param word to check
     */
    private static boolean noOtherInstitutionKeyWord(String word) {
        return !DEPARTMENTS.matcher(word).matches() && !StandardField.SCHOOL.getName().equalsIgnoreCase(word)
                && !"faculty".equalsIgnoreCase(word) && !NOT_CAPITAL_CHARACTER.matcher(word).replaceAll("").isEmpty();
    }
    ...
    private static List<String> getValidInstitutionNameParts(String name) {
        ...
        for (String part : name.replaceAll("\\{[A-Z]+}", "").split("[ \\-_]")) {
            if ((!part.isEmpty() // remove empty
                    && !ignore.contains(part.toLowerCase(Locale.ENGLISH)) // remove
                                                                          // ignored words
                    && (part.charAt(part.length() - 1) != '.') && Character.isUpperCase(part.charAt(0)))

...
public class BibDatabaseDiff {
    ...
    private BibDatabaseDiff(
        ...
        if (LOGGER.isDebugEnabled() && !isEmpty()) {
            ...
            metaDataDiff.ifPresent(diff ->
                LOGGER.debug("Metadata differences: {}", diff)
            );
            preambleDiff.ifPresent(diff ->
                LOGGER.debug("Premble differences: {}", diff)
            );
    ...
    private static List<BibEntryDiff> compareEntries(
        ...
        for (BibEntry originalEntry : notMatched) {
            ...
            boolean isDuplicate = duplicateCheck.isDuplicate(
                originalEntry,
                bestEntry,
                mode
            );
            boolean hasEqualCitationKey = hasEqualCitationKey(
                originalEntry,
                bestEntry
            );
            ...
            boolean ratioOfEqualFieldsAboveThreshold =

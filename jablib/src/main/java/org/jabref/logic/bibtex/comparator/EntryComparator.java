...
/**
 * This implementation of Comparator takes care of most of the details of sorting BibTeX
 * entries in JabRef. It is structured as a node in a linked list of comparators, where
 * each node can contain a link to a new comparator that decides the ordering (by
 * recursion) if this one can't find a difference. The next node, if any, is given at
 * construction time, and an arbitrary number of nodes can be included. If the entries are
 * equal by this comparator, and there is no next entry, the entries' unique IDs will
 * decide the ordering.
 */
public class EntryComparator implements Comparator<BibEntry> {
    ...
    /**
     * @param binary true: the presence of fields is checked; false: the content of the
     * fields is compared
     * @param descending true: if the most different entry should get the highest score
     * @param field the field to sort on
     * @param next the next comparator to use (if the current comparator results in
     * equality)
     */
    ...
    public int compare(BibEntry e1, BibEntry e2) {
        // default equals
        // TODO: with the new default equals this does not only return 0 for identical
        // objects,

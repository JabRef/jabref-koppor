...
/**
 * Converts JabRef's internal BibTeX representation of a BibTeX field to BibTeX text
 * representation
 */
public class FieldWriter {
    ...
    private static void checkBraces(String text) throws InvalidFieldValueException {
        ...
        if (right != 0 && (left == 0)) {
            LOGGER.error("Unescaped '}' character without opening bracket ends string prematurely. Field value: {}",
                    text);
     ...
     * @param field the name of the field - used to trigger different serializations,
     * e.g., turning off resolution for some strings
     * @param content the content of the field
     * @return a formatted string suitable for output
     * @throws InvalidFieldValueException if content is not a correct bibtex string, e.g.,
     * because of improperly balanced braces or using # not paired
     */
     ...
     * <p>
     * For instance, <code>#jan# - #feb#</code> gets <code>jan #{ - } # feb</code>
     * (see @link{org.jabref.logic.bibtex.LatexFieldFormatterTests#makeHashEnclosedWordsRealStringsInMonthField()})
     */
        ...
        while (pivot < content.length()) {
            ...
            if (pos2 == -1) {
                if (neverFailOnHashes) {
                    pos1 = content.length(); // just write out the rest of the text, and
                                             // throw no exception
                }
                else {
                    LOGGER.error(
                            ...
                            BIBTEX_STRING_START_END_SYMBOL, BIBTEX_STRING_START_END_SYMBOL, content);
     ...
     */
    private void writeStringLabel(StringBuilder stringBuilder, String text, int startPos, int endPos, boolean isFirst,
            boolean isLast) {

...
public class MetaDataDiff {
    ...
    public enum DifferenceType {
        ...
        CONTENT_SELECTOR, DEFAULT_KEY_PATTERN, ENCODING, LIBRARY_SPECIFIC_FILE_DIRECTORY, GROUPS, KEY_PATTERNS,
        LATEX_FILE_DIRECTORY, MODE, PROTECTED, SAVE_ACTIONS, SAVE_SORT_ORDER, USER_FILE_DIRECTORY
    ...
    public static Optional<MetaDataDiff> compare(MetaData originalMetaData, MetaData newMetaData) {
        if (originalMetaData.equals(newMetaData)) {
            ...
            List<Difference> differences = diff
                .getDifferences(new GlobalCitationKeyPatterns(CitationKeyPattern.NULL_CITATION_KEY_PATTERN));
    ...
    private static boolean isDefaultContentSelectors(ContentSelectors contentSelectors) {
        ...
        Map<Field, List<String>> fieldKeywordsMap = ContentSelectors
            .getFieldKeywordsMap(contentSelectors.getContentSelectors());
        return ContentSelectors.isDefaultMap(fieldKeywordsMap);
    }

    private void addToListIfDiff(List<Difference> changes, DifferenceType differenceType, Object originalObject,
            Object newObject) {
    ...
    public List<Difference> getDifferences(GlobalCitationKeyPatterns globalCitationKeyPatterns) {
        ...
        addToListIfDiff(changes, DifferenceType.SAVE_SORT_ORDER, originalMetaData.getSaveOrder(),
        ...
        addToListIfDiff(changes, DifferenceType.USER_FILE_DIRECTORY, originalMetaData.getUserFileDirectories(),
                newMetaData.getUserFileDirectories());
        addToListIfDiff(changes, DifferenceType.LATEX_FILE_DIRECTORY, originalMetaData.getLatexFileDirectories(),
                newMetaData.getLatexFileDirectories());
        addToListIfDiff(changes, DifferenceType.DEFAULT_KEY_PATTERN, originalMetaData.getDefaultCiteKeyPattern(),
                newMetaData.getDefaultCiteKeyPattern());
        addToListIfDiff(changes, DifferenceType.SAVE_ACTIONS, originalMetaData.getSaveActions(),
                newMetaData.getSaveActions());
        addToListIfDiff(changes, DifferenceType.MODE, originalMetaData.getMode(), newMetaData.getMode());
        addToListIfDiff(changes, DifferenceType.LIBRARY_SPECIFIC_FILE_DIRECTORY,
                originalMetaData.getLibrarySpecificFileDirectory(), newMetaData.getLibrarySpecificFileDirectory());
        addToListIfDiff(changes, DifferenceType.CONTENT_SELECTOR, originalMetaData.getContentSelectors(),
    ...
    /**
     * Currently, the groups diff is contained here - and as entry in
     * {@link #getDifferences(GlobalCitationKeyPatterns)}
     */
    ...
    public String toString() {
        return "MetaDataDiff{" + "groupDiff=" + groupDiff + ", originalMetaData=" + originalMetaData + ", newMetaData="
                + getNewMetaData() + ", getDifferences()="
                + getDifferences(new GlobalCitationKeyPatterns(CitationKeyPattern.NULL_CITATION_KEY_PATTERN)) + '}';
    }

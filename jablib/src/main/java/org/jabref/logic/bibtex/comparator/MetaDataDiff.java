...
public class MetaDataDiff {
     ...
     */
    private static boolean isDefaultContentSelectors(
    ...
    private void addToListIfDiff(
        ...
        if (!Objects.equals(originalObject, newObject)) {
            if (differenceType == DifferenceType.CONTENT_SELECTOR) {
                ContentSelectors originalContentSelectors =
                    (ContentSelectors) originalObject;
                ...
                Optional<GroupTreeNode> originalGroups = (Optional<
                    GroupTreeNode
                >) originalObject;
                Optional<GroupTreeNode> newGroups = (Optional<
                    GroupTreeNode
                >) newObject;
                if (
                    isDefaultGroup(originalGroups) && isDefaultGroup(newGroups)
                ) {
                    return;
                }
            }
    ...
    private boolean isDefaultGroup(Optional<GroupTreeNode> groups) {
        ...
        return groupRoot
            .getGroup()
            .equals(DefaultGroupsFactory.getAllEntriesGroup());
    }
    ...
    public List<Difference> getDifferences(
        ...
        addToListIfDiff(
            changes,
            DifferenceType.LIBRARY_SPECIFIC_FILE_DIRECTORY,
            originalMetaData.getLibrarySpecificFileDirectory(),
            newMetaData.getLibrarySpecificFileDirectory()
        );
        addToListIfDiff(
            changes,

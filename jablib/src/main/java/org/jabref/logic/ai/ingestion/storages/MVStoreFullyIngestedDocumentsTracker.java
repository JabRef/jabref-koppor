...
public class MVStoreFullyIngestedDocumentsTracker extends MVStoreBase implements FullyIngestedDocumentsTracker {
    ...
    // This map stores the ingested documents. The key is LinkedDocument.getLink(), and
    // the value is the modification time in seconds.
    // If an entry is present, then it means the document was ingested. Otherwise,
    ...
    // any embeddings because when we ingest a document embeddings are generated in
    // series, so if 1 embedding is present

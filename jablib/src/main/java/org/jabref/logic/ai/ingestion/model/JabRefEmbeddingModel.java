...
public class JabRefEmbeddingModel implements EmbeddingModel, AutoCloseable {
    ...
    private final ExecutorService executorService =
        Executors.newCachedThreadPool(
            new ThreadFactoryBuilder()
                .setNameFormat("ai-embedding-pool-%d")
                .build()
        );

    private final ObjectProperty<
        Optional<DeepJavaEmbeddingModel>
    > predictorProperty = new SimpleObjectProperty<>(Optional.empty());
    ...
    private void setupListeningToPreferencesChanges() {
        ...
        aiPreferences
            ...
            .addListener(obs -> startRebuildingTask());
    }
    ...
    public Response<List<Embedding>> embedAll(List<TextSegment> list) {
        if (predictorProperty.get().isEmpty()) {
            ...
            throw new RuntimeException(
                Localization.lang("Embedding model is not set up")
            );
        }

...
public class ChatHistoryService implements AutoCloseable {
    ...
    private final TreeMap<
        BibEntry,
        ChatHistoryManagementRecord
    > bibEntriesChatHistory = new TreeMap<>(
    ...
    public void closeChatHistoryForEntry(BibEntry entry) {
        ChatHistoryManagementRecord chatHistoryManagementRecord =
            bibEntriesChatHistory.get(entry);
    ...
    private boolean correctCitationKey(
        BibDatabaseContext bibDatabaseContext,
        BibEntry bibEntry
    ) {
        if (
        ...
        return CitationKeyCheck.citationKeyIsPresentAndUnique(
            bibDatabaseContext,
            bibEntry
        );
    }
    ...
    public void close() {
        ...
        new HashSet<>(bibEntriesChatHistory.keySet()).forEach(
            this::closeChatHistoryForEntry
        );

        // Clone is for the same reason, as written above.
        new HashSet<>(groupsChatHistory.keySet()).forEach(
            this::closeChatHistoryForGroup
        );
    ...
    private void transferGroupHistory(
        ...
        List<ChatMessage> chatMessages = groupsChatHistory.computeIfAbsent(
            groupTreeNode,
            e ->
    ...
    private void transferEntryHistory(
        ...
        List<ChatMessage> chatMessages = bibEntriesChatHistory.computeIfAbsent(
            entry,
            e ->
                new ChatHistoryManagementRecord(
                    Optional.of(bibDatabaseContext),

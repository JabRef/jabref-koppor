...
public class IngestionService {
    ...
    private final TreeMap<
        LinkedFile,
        ProcessingInfo<LinkedFile, Void>
    > ingestionStatusMap = new TreeMap<>(
        Comparator.comparing(LinkedFile::getLink)
    );

    private final List<List<LinkedFile>> listsUnderIngestion =
        new ArrayList<>();
    ...
    public ProcessingInfo<LinkedFile, Void> ingest(
        ...
        ProcessingInfo<LinkedFile, Void> processingInfo = getProcessingInfo(
            linkedFile
        );

        if (processingInfo.getState() == ProcessingState.STOPPED) {
            startEmbeddingsGenerationTask(
                linkedFile,
                bibDatabaseContext,
                processingInfo
            );
     ...
     */
    public ProcessingInfo<LinkedFile, Void> getProcessingInfo(
        LinkedFile linkedFile
    ) {
        return ingestionStatusMap.computeIfAbsent(linkedFile, file ->
            new ProcessingInfo<>(linkedFile, ProcessingState.STOPPED)
        );
    }

    public List<ProcessingInfo<LinkedFile, Void>> getProcessingInfo(
    ...
    public List<ProcessingInfo<LinkedFile, Void>> ingest(
        ...
        List<ProcessingInfo<LinkedFile, Void>> result = getProcessingInfo(
            linkedFiles
        );
        ...
        List<ProcessingInfo<LinkedFile, Void>> needToProcess = result
            .stream()
            .filter(
                processingInfo ->
    ...
    private void startEmbeddingsGenerationTask(
        ...
        linkedFiles.forEach(processingInfo ->
            processingInfo.setState(ProcessingState.PROCESSING)
        );
    ...
    public void clearEmbeddingsFor(List<LinkedFile> linkedFiles) {
        ...
        ingestionStatusMap
            .values()
            .forEach(processingInfo ->
                processingInfo.setState(ProcessingState.STOPPED)
            );
    }
}

...
/**
 * Main class for generating embedding for files. Use this class in the logic and UI.
 */
public class IngestionService {

    // We use a {@link TreeMap} here for the same reasons we use it in {@link
    // ChatHistoryService}.
    private final TreeMap<LinkedFile, ProcessingInfo<LinkedFile, Void>> ingestionStatusMap = new TreeMap<>(
            Comparator.comparing(LinkedFile::getLink));
    ...
    public IngestionService(AiPreferences aiPreferences, ReadOnlyBooleanProperty shutdownSignal,
            EmbeddingModel embeddingModel, EmbeddingStore<TextSegment> embeddingStore,
            FullyIngestedDocumentsTracker fullyIngestedDocumentsTracker, FilePreferences filePreferences,
            TaskExecutor taskExecutor) {
        this.aiPreferences = aiPreferences;
        this.filePreferences = filePreferences;
        this.taskExecutor = taskExecutor;

        this.fileEmbeddingsManager = new FileEmbeddingsManager(aiPreferences, shutdownSignal, embeddingModel,
                embeddingStore, fullyIngestedDocumentsTracker);
    ...
    /**
     * Start ingesting of a {@link LinkedFile}, if it was not ingested. This method
     * returns a {@link ProcessingInfo} that can be used for tracking state of the
     * ingestion. Returned {@link ProcessingInfo} is related to the passed
     * {@link LinkedFile}, so if you call this method twice on the same
     * {@link LinkedFile}, the method will return the same {@link ProcessingInfo}.
     */
    ...
    /**
     * Get {@link ProcessingInfo} of a {@link LinkedFile}. Initially, it is in state
     * {@link ProcessingState#STOPPED}. This method will not start ingesting. If you need
     * to start it, use {@link IngestionService#ingest(LinkedFile, BibDatabaseContext)}.
     */
    public ProcessingInfo<LinkedFile, Void> getProcessingInfo(LinkedFile linkedFile) {
        return ingestionStatusMap.computeIfAbsent(linkedFile,
                file -> new ProcessingInfo<>(linkedFile, ProcessingState.STOPPED));
    }
    ...
    public List<ProcessingInfo<LinkedFile, Void>> ingest(StringProperty groupName, List<LinkedFile> linkedFiles,
        ...
        List<ProcessingInfo<LinkedFile, Void>> needToProcess = result.stream()
    ...
    private void startEmbeddingsGenerationTask(LinkedFile linkedFile, BibDatabaseContext bibDatabaseContext,
    ...
    private void startEmbeddingsGenerationTask(StringProperty groupName,
            List<ProcessingInfo<LinkedFile, Void>> linkedFiles, BibDatabaseContext bibDatabaseContext) {
        linkedFiles.forEach(processingInfo -> processingInfo.setState(ProcessingState.PROCESSING));

        new GenerateEmbeddingsForSeveralTask(groupName, linkedFiles, fileEmbeddingsManager, bibDatabaseContext,
                filePreferences, taskExecutor, shutdownSignal)
            .executeWith(taskExecutor);
    }

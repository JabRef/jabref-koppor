...
public class JabRefChatLanguageModel implements ChatModel, AutoCloseable {
    ...
    private final ExecutorService executorService =
        Executors.newSingleThreadExecutor(
            new ThreadFactoryBuilder()
                .setNameFormat("ai-api-connection-pool-%d")
                .build()
        );
    ...
    private void rebuild() {
        String apiKey = aiPreferences.getApiKeyForAiProvider(
        ...
        switch (aiPreferences.getAiProvider()) {
            case OPEN_AI -> langchainChatModel = Optional.of(
                new JvmOpenAiChatLanguageModel(aiPreferences, httpClient)
            );
            case GPT4ALL -> langchainChatModel = Optional.of(
                new Gpt4AllModel(aiPreferences, httpClient)
            );
            case MISTRAL_AI -> langchainChatModel = Optional.of(
            ...
            case GEMINI -> langchainChatModel = Optional.of( // NOTE: {@link GoogleAiGeminiChatModel} doesn't support API base url.
            ...
            case HUGGING_FACE -> langchainChatModel = Optional.of( // NOTE: {@link HuggingFaceChatModel} doesn't support API base url.
    ...
    private void setupListeningToPreferencesChanges() {
        ...
        aiPreferences
            .enableAiProperty()
            .addListener(_ -> langchainChatModel = Optional.empty());
        aiPreferences
            .aiProviderProperty()
            .addListener(_ -> langchainChatModel = Optional.empty());
        aiPreferences
        ...
        aiPreferences.addListenerToChatModels(() ->
            langchainChatModel = Optional.empty()
        );
        aiPreferences.addListenerToApiBaseUrls(() ->
            langchainChatModel = Optional.empty()
        );
        aiPreferences.setApiKeyChangeListener(() ->
            langchainChatModel = Optional.empty()
        );
    }
    ...
    public ChatResponse chat(List<ChatMessage> list) {
        ...
        if (langchainChatModel.isEmpty()) {
            if (!aiPreferences.getEnableAi()) {
                ...
                if (langchainChatModel.isEmpty()) {
                    throw new RuntimeException(
                        Localization.lang("Unable to chat with AI.")
                    );
                }
            }
        }

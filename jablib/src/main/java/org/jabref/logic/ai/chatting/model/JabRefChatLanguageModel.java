 ...
 * <p>
 * Notice, that the real chat model is created lazily, when it's needed. This is done, so
 * API key is fetched only, when user wants to chat with AI.
 */
public class JabRefChatLanguageModel implements ChatModel, AutoCloseable {
    ...
    private final ExecutorService executorService = Executors
        .newSingleThreadExecutor(new ThreadFactoryBuilder().setNameFormat("ai-api-connection-pool-%d").build());
    ...
    /**
     * Update the underlying {@link dev.langchain4j.model.chat.ChatModel} by current
     * {@link AiPreferences} parameters. When the model is updated, the chat messages are
     * not lost. See {@link AiChatLogic}, where messages are stored in {@link ChatMemory},
    ...
    private void rebuild() {
        ...
        switch (aiPreferences.getAiProvider()) {
            ...
            case MISTRAL_AI -> langchainChatModel = Optional.of(MistralAiChatModel.builder()
                .apiKey(apiKey)
            ...
            case GEMINI -> // NOTE: {@link GoogleAiGeminiChatModel} doesn't support API
                           // base url.
                langchainChatModel = Optional.of(GoogleAiGeminiChatModel.builder()
                    .apiKey(apiKey)
                    .modelName(aiPreferences.getSelectedChatModel())
                    .temperature(aiPreferences.getTemperature())
            ...
            case HUGGING_FACE -> // NOTE: {@link HuggingFaceChatModel} doesn't support API
                                 // base url.
                langchainChatModel = Optional.of(HuggingFaceChatModel.builder()
                    .accessToken(apiKey)
    ...
    private void setupListeningToPreferencesChanges() {
        // Setting "langchainChatModel" to "Optional.empty()" will trigger a rebuild on
        // the next usage
        ...
        if (langchainChatModel.isEmpty()) {
            if (!aiPreferences.getEnableAi()) {
                ...
                throw new RuntimeException(Localization
                    .lang("In order to use AI chat, set an API key inside JabRef preferences (AI tab)."));
            }
            else {
                rebuild();
                if (langchainChatModel.isEmpty()) {

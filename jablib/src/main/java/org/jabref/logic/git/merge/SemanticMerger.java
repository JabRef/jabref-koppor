...
public class SemanticMerger {
     ...
     */
    public static void applyMergePlan(
        BibDatabaseContext localCopy,
        MergePlan plan
    ) {
        ...
        for (BibEntry newEntry : plan.newEntries()) {
                ...
                .ifPresent(citationKey ->
                    localCopy
                        ...
                        .ifPresent(existing -> {
                            ...
                            LOGGER.debug(
                                "Removed existing entry '{}' before re-inserting",
                                citationKey
                            );
                        })
                );
            ...
            LOGGER.debug(
                "Inserted (or replaced) entry '{}', fields={}, marked as changed",
    ...
    public static void applyPatchToDatabase(
        BibDatabaseContext localCopy,
        Map<String, Map<Field, String>> patchMap
    ) {
        for (Map.Entry<
            String,
            Map<Field, String>
        > entry : patchMap.entrySet()) {
            String key = entry.getKey();
            Map<Field, String> fieldPatch = entry.getValue();
            Optional<BibEntry> localEntryOpt = localCopy
                .getDatabase()
                .getEntryByCitationKey(key);

            if (localEntryOpt.isEmpty()) {
                LOGGER.warn(
                    "Skip patch: local does not contain entry '{}'",
                    key
                );
                continue;
            }
    ...
    public static void applyFieldPatchToEntry(
        BibEntry localEntry,
        Map<Field, String> patch
    ) {

 ...
 * <p>
 * Ensures that each Git repository (identified by its root path) has a single
 * {@code GitHandler} instance shared across the application.
 * <p>
 * Usage: - {@link #get(Path)} — for known repository root paths (must contain a .git
 * folder). - {@link #fromAnyPath(Path)} — for arbitrary paths inside a Git repo; will
 * locate the repo root first.
 */
public class GitHandlerRegistry {
    ...
    public GitHandler get(Path repoPath) {
        Path normalized = Objects.requireNonNull(repoPath, "Path must not be null").toAbsolutePath().normalize();

...
/**
 * This class handles the updating of the local and remote git repository that is located
 * at the repository path This provides an easy-to-use interface to manage a git
 * repository
 */
public class GitHandler {
    ...
    public void initIfNeeded() {
        ...
        try {
            try (Git git = Git.init().setDirectory(repositoryPathAsFile).setInitialBranch("main").call()) {
                // "git" object is not used later, but we need to close it after
                // initialization
            }
            setupGitIgnore();
    ...
    private Optional<CredentialsProvider> resolveCredentials() {
        ...
        // Not implemented in August, 2025, because implications to SLR component not
        // clear.
    ...
    // TODO: GitHandlerRegistry should get passed GitPasswordPreferences (or similar),
    // pass this to GitHandler instance, which uses it here#
    ...
    boolean isGitRepository() {
        // For some reason the solution from
        // https://www.eclipse.org/lists/jgit-dev/msg01892.html does not work
        // This solution is quite simple but might not work in special cases, for us it
        // should suffice.
    ...
    /**
     * Returns the reference of the specified branch If it does not exist returns an empty
     * optional
     */
     ...
     * @return Returns true if a new commit was created. This is the case if the
     * repository was not clean on method invocation
     */
    public boolean createCommitOnCurrentBranch(String commitMessage, boolean amend)
        ...
        try (Git git = Git.open(this.repositoryPathAsFile)) {
            ...
            if (dirty) {
                ...
                commitCreated = true;
                git.commit().setAmend(amend).setAllowEmpty(true).setMessage(commitMessage).call();
            }
        }
    ...
    /**
     * Pushes all commits made to the branch that is tracked by the currently checked out
     * branch. If pushing to remote fails, it fails silently.
     */
    ...
    /**
     * Try to locate the Git repository root by walking up the directory tree starting
     * from the given path.
     * <p>
     * If a directory containing a .git folder is found, return that path.
     * @param anyPathInsideRepo the file or directory path that is assumed to be located
    ...
    public boolean hasRemote(String remoteName) {
        try (Git git = Git.open(this.repositoryPathAsFile)) {
            return git.getRepository().getConfig().getSubsections("remote").contains(remoteName);
        }
        catch (IOException e) {
    ...
    /// Puts the repo into MERGING state, sets MERGE_HEAD=remote; working tree becomes
    /// "ours".
    ...
    /// 1) Clear merge state files (MERGE_HEAD / MERGE_MSG, etc.). Since there is no
    /// direct equivalent for git merge --abort in JGit.
    ...
    public void abortSemanticMerge(Path absoluteFilePath, boolean allowHardReset) throws IOException, GitAPIException {
        try (Git git = Git.open(this.repositoryPathAsFile)) {
            ...
            // 2.1 Reset the file in the index to HEAD (Equivalent to: `git reset --
            // <path>`)
            git.reset().addPath(rel).call();

            // 2.2 Restore the file in the working tree from HEAD (Equivalent to: `git
            // checkout -- <path>`)
    ...
    public static final class MergeGuard implements AutoCloseable {
        ...
        // Finalize: create the commit (in MERGING this becomes a merge commit with two
        // parents).
        ...
        public void close() {
            ...
            try {
                ...
                // Deliberately catching RuntimeException here because this is a
                // best-effort cleanup in AutoCloseable.close().
                // have to NOT throw from close() to avoid masking the primary
                // failure/result of pull/merge.

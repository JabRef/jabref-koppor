 ...
 * <p>
 * A word can have protected chars (enclosed in '{' '}') and may be a small (a, an, the,
 * ...) word.
 */
public final class Word {
    ...
    // Conjunctions used as part of Title case capitalisation to specifically check if
    // word is conjunction or not
    ...
    static {
        ...
        // Conjunctions used as part of all case capitalisation to check if it is a small
        // word or not
    ...
    public void toUpperFirstIgnoreHyphen() {
        for (int i = 0; i < chars.length; i++) {
            if (!protectedChars[i]) {
                chars[i] = i == 0 || (DASHES.contains(chars[i - 1])) ? Character.toUpperCase(chars[i])

...
public class MSBibConverter {
    ...
    public static MSBibEntry convert(BibEntry entry) {
        ...
        // define new type
        String msBibType = MSBibMapping.getMSBibEntryType(
            entry.getType()
        ).name();
        ...
        if ("Interview".equals(msBibType)) {
            result.broadcastTitle = entry
                .getFieldLatexFree(StandardField.TITLE)
                .orElse(null);
        }

        result.number = entry
            .getFieldLatexFree(StandardField.NUMBER)
            .orElse(null);

        if (entry.getType().equals(IEEETranEntryType.Patent)) {
            result.patentNumber = entry
                .getFieldLatexFree(StandardField.NUMBER)
                .orElse(null);
            result.number = null;
        }
        ...
        result.address = entry
            .getFieldOrAliasLatexFree(StandardField.ADDRESS)
            .orElse(null);

        if (entry.getFieldLatexFree(StandardField.TYPE).isPresent()) {
            result.thesisType = entry
                .getFieldLatexFree(StandardField.TYPE)
                .get();
        } else {
    ...
    private static List<MsBibAuthor> getAuthors(
        ...
        boolean corporate = !REMOVE_BRACES_FORMATTER.format(authors).equals(
            authors
        );
        ...
        return AuthorList.parse(authorLatexFree)
            .getAuthors()
            .stream()

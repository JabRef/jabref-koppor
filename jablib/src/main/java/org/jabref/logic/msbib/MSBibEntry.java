...
class MSBibEntry {
    ...
    private void populateFromXml(Element entry) {
        ...
        internetSiteTitle = getXmlElementTextContent(
            "InternetSiteTitle",
            entry
        );
    ...
    private List<MsBibAuthor> getSpecificAuthors(String type, Element authors) {
        ...
        nodeLst = ((Element) nodeLst.item(0)).getElementsByTagNameNS(
            "*",
            "NameList"
        );
        if (nodeLst.getLength() <= 0) {
        ...
        NodeList person = ((Element) nodeLst.item(0)).getElementsByTagNameNS(
            "*",
            "Person"
        );
        ...
        for (int i = 0; i < person.getLength(); i++) {
            NodeList firstName = ((Element) person.item(
                    i
                )).getElementsByTagNameNS("*", "First");
            NodeList lastName = ((Element) person.item(
                    i
                )).getElementsByTagNameNS("*", "Last");
            NodeList middleName = ((Element) person.item(
                    i
                )).getElementsByTagNameNS("*", "Middle");
    ...
    public Element getEntryDom(Document document) {
        ...
        Optional.ofNullable(dateAccessed).ifPresent(field ->
            addDateAcessedFields(document, rootNode)
        );

 ...
 * the structured persistence of the crawling results for the study within the file based
 * repository, as well as the sharing, and versioning of results using git.
 */
public class StudyRepository {
    ...
    // Currently we make assumptions about the configuration: the remotes, work and search
    // branch names
     ...
     * @param gitHandler The git handler that manages any interaction with the remote
     * repository
     * @throws IllegalArgumentException If the repository root directory does not exist,
     * or the root directory does not contain the study definition file.
     * @throws IOException Thrown if the given repository does not exist, or the study
     * definition file does not exist
     */
    public StudyRepository(Path pathToRepository, SlrGitHandler gitHandler, CliPreferences preferences,
            FileUpdateMonitor fileUpdateMonitor, BibEntryTypesManager bibEntryTypesManager) throws IOException {
        ...
        try {
            ...
            // If study definition does not exist on this branch or was changed on work
            // branch, copy it from work
    ...
    public BibDatabaseContext getFetcherResultEntries(String query, String fetcherName) throws IOException {
        if (Files.exists(getPathToFetcherResultFile(query, fetcherName))) {
            return OpenDatabase
                .loadDatabase(getPathToFetcherResultFile(query, fetcherName), preferences.getImportFormatPreferences(),
                        fileUpdateMonitor)
                .getDatabaseContext();
        }
    ...
    public BibDatabaseContext getQueryResultEntries(String query) throws IOException {
        if (Files.exists(getPathToQueryResultFile(query))) {
            return OpenDatabase
                .loadDatabase(getPathToQueryResultFile(query), preferences.getImportFormatPreferences(),
                        fileUpdateMonitor)
                .getDatabaseContext();
        }
        return new BibDatabaseContext();
    ...
    public BibDatabaseContext getStudyResultEntries() throws IOException {
        if (Files.exists(getPathToStudyResultFile())) {
            return OpenDatabase
                .loadDatabase(getPathToStudyResultFile(), preferences.getImportFormatPreferences(), fileUpdateMonitor)
                .getDatabaseContext();
        }
        return new BibDatabaseContext();
    }
    ...
    /**
     * The study definition file contains all the definitions of a study. This method
     * extracts this study from the yaml study definition file
     ...
     * @throws IllegalArgumentException If a transformation from Library entry to
     * LibraryDefinition fails
     */
    public List<StudyDatabase> getActiveLibraryEntries() throws IllegalArgumentException {
        return study.getDatabases().parallelStream().filter(StudyDatabase::isEnabled).collect(Collectors.toList());
    }
    ...
    /**
     * Persists the result locally and remotely by following the steps: Precondition:
     * Currently checking out work branch
     * <ol>
     * <li>Update the work and search branch</li>
     * <li>Persist the results on the search branch</li>
     * <li>Manually patch the diff of the search branch onto the work branch (as the
     * merging will not work in certain cases without a conflict as it is context
     * sensitive. But for this use case we do not need it to be context sensitive. So we
     * can just prepend the patch without checking the "context" lines.</li>
    ...
    /**
     * Update the remote tracking branches of the work and search branches The currently
     * checked out branch is not changed if the method is executed successfully
     */
    ...
    /**
     * Updates the local work and search branches with changes from their tracking remote
     * branches The currently checked out branch is not changed if the method is executed
     * successfully
     */
    ...
    /**
     * Create for each query a folder, and for each fetcher a bib file in the query folder
    ...
    private void setUpRepositoryStructureForQueriesAndFetchers() throws IOException {
        ...
        StudyCatalogToFetcherConverter converter = new StudyCatalogToFetcherConverter(this.getActiveLibraryEntries(),
    ...
    /**
     * Creates a folder using the query and its corresponding query id. This folder name
     * is unique for each query, as long as the query id in the study definition is unique
     * for each query.
    ...
    /**
     * Returns a string that can be used as a folder name. This removes all characters
     * from the query that are illegal for directory names. Structure: ID-trimmed query
     *
     * Examples: Input: '(title: test-title AND abstract: Test)' as a query entry with id
     * 12345678 Output: '12345678 - title= test-title AND abstract= Test'
     *
     * Input: 'abstract: Test*' as a query entry with id 87654321 Output: '87654321 -
     * abstract= Test'
     *
     * Input: '"test driven"' as a query entry with id 12348765 Output: '12348765 - test
     * driven'
     *
     * Note that this method might be similar to
     * {@link org.jabref.logic.util.io.FileUtil#getValidFileName(String)} or
     * {@link org.jabref.logic.util.io.FileNameCleaner#cleanFileName(String)}
    ...
    private void persistResults(List<QueryResult> crawlResults) throws IOException, SaveException {
        ...
        for (QueryResult result : crawlResults) {
            ...
            for (FetchResult fetcherResult : result.getResultsPerFetcher()) {
                ...
                BibDatabaseContext existingFetcherResult = getFetcherResultEntries(result.getQuery(),
                ...
                writeResultToFile(getPathToFetcherResultFile(result.getQuery(), fetcherResult.getFetcherName()),
    ...
    private void writeResultToFile(Path pathToFile, BibDatabaseContext context) throws SaveException {
        try (AtomicFileWriter fileWriter = new AtomicFileWriter(pathToFile, StandardCharsets.UTF_8)) {
            ...
            BibDatabaseWriter databaseWriter = new BibDatabaseWriter(bibWriter, saveConfiguration,
                    preferences.getFieldPreferences(), preferences.getCitationKeyPatternPreferences(),

...
/**
 * @implNote SSL certificates are installed at
 * {@link TrustStoreManager#configureTrustStore(Path)}
 */
public class TrustStoreManager {
    ...
    /**
     * Deletes all custom certificates, Custom certificates are certificates with alias
     * that ends with {@code [custom]}
     */
    ...
    /**
     * This method checks to see if the truststore is present in {@code storePath}, and if
     * it isn't, it copies the default JDK truststore to the specified location.
     ...
     */
    private static void configureTrustStore(Path myStorePath) throws NoSuchAlgorithmException, KeyManagementException,
    ...
    private static X509TrustManager findDefaultTrustManager(KeyStore keyStore)
        ...
        tmf.init(keyStore); // If keyStore is null, tmf will be initialized with the
                            // default trust store

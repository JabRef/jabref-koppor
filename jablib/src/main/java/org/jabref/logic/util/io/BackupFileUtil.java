...
public class BackupFileUtil {
    ...
    /// In case that fails, the return path of the .bak file is set to be next to the .bib
    /// file
    ///
    ...
    public static Optional<Path> getPathOfLatestExistingBackupFile(Path targetFile, BackupFileType fileType,
        ...
        if (Files.notExists(backupDir)) {
            // In case there is no app directory, we search in the directory of the bib
            // file
            Path result = FileUtil.addExtension(targetFile, extension);
     ...
     * Just adding ".bak" suffix to the filename, does not work in all cases: It may be
     * possible that the user has opened "paper.bib" twice. Thus, we need to create a
     * unique prefix to distinguish these files.
     * </p>
     */

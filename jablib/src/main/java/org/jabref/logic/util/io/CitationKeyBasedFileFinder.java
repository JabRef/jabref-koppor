...
class CitationKeyBasedFileFinder implements FileFinder {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(
        CitationKeyBasedFileFinder.class
    );
    ...
    public List<Path> findAssociatedFiles(
        ...
        if (exactKeyOnly) {
            ...
            filteringFunction = (Path p) ->
                FileUtil.getBaseName(p.getFileName().toString()).equals(
                    citeKey
                );
        } else {
            // LOGGER.debug("Found non-exact match for key {} in file {}", citeKey, file);
            filteringFunction = (Path p) ->
                matches(p.getFileName().toString(), citeKey);
        }

        SortedSet<Path> result = findFilesByExtension(
            directories,
            extensions,
            filteringFunction
        );
     ...
     */
    private SortedSet<Path> findFilesByExtension(
        List<Path> directories,

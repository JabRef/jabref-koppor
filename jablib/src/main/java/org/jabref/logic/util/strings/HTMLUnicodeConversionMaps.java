...
public class HTMLUnicodeConversionMaps {
    ...
    // most of the LaTeX commands can be read at
    // http://en.wikibooks.org/wiki/LaTeX/Accents
    // The symbols can be seen at
    // http://www.fileformat.info/info/unicode/char/a4/index.htm. Replace "a4" with the U+
    // number
    // http://detexify.kirelabs.org/classify.html and
    // http://www.ctan.org/tex-archive/info/symbols/comprehensive/ might help to find the
    // right LaTeX command
    ...
    /*
     * Portions © International Organization for Standardization 1986: Permission to copy
     * in any form is granted for use with conforming SGML systems and applications as
    ...
    // {"decimal number of HTML entity", "text HTML entity", "corresponding LaTeX
    // command"}
    // Leaving a field empty is OK as it then will not be included
    private static final String[][] CONVERSION_LIST = new String[][] { { "160", "nbsp", "{~}" }, // no-break
                                                                                                 // space
                                                                                                 // =
                                                                                                 // non-breaking
                                                                                                 // space,
            // U+00A0 ISOnum
            { "161", "iexcl", "{\\textexclamdown}" }, // inverted exclamation mark, U+00A1
                                                      // ISOnum
            ...
            { "170", "ordf", "{\\textordfeminine}" }, // feminine ordinal indicator,
                                                      // U+00AA ISOnum
            { "171", "laquo", "{\\guillemotleft}" }, // left-pointing double angle
                                                     // quotation mark
            // = left pointing guillemet, U+00AB ISOnum
            { "172", "not", "$\\neg$" }, // not sign, U+00AC ISOnum
            { "173", "shy", "\\-" }, // soft hyphen = discretionary hyphen,
            // U+00AD ISOnum
            { "174", "reg", "{\\textregistered}" }, // registered sign = registered trade
                                                    // mark sign,
            // U+00AE ISOnum
            ...
            // U+00B1 ISOnum
            { "178", "sup2", "\\textsuperscript{2}" }, // superscript two = superscript
                                                       // digit two
            // = squared, U+00B2 ISOnum
            { "179", "sup3", "\\textsuperscript{3}" }, // superscript three = superscript
                                                       // digit three
            // = cubed, U+00B3 ISOnum
            ...
            { "185", "sup1", "\\textsuperscript{1}" }, // superscript one = superscript
                                                       // digit one,
            // U+00B9 ISOnum
            { "186", "ordm", "{\\textordmasculine}" }, // masculine ordinal indicator,
            // U+00BA ISOnum
            { "187", "raquo", "{\\guillemotright}" }, // right-pointing double angle
                                                      // quotation mark
            ...
            // U+0394 ISOgrk3
            { "917", "Epsilon", "{{$\\Epsilon$}}" }, // greek capital letter epsilon,
                                                     // U+0395
            ...
            { "927", "Omicron", "{{$\\Omicron$}}" }, // greek capital letter omicron,
                                                     // U+039F
            ...
            /* General Punctuation */
            { "8211", "ndash", "$\\textendash$" }, { "8212", "mdash", "$\\textemdash$" },
            ...
            { "8501", "alefsym", "$\\aleph$" }, // alef symbol = first transfinite
                                                // cardinal,
            // U+2135 NEW
            /*
            ...
            { "8656", "lArr", "{{$\\Leftarrow$}}" }, // leftwards double arrow, U+21D0
                                                     // ISOtech
            /*
             ...
             * ISOtech suggests
             */
            { "8659", "dArr", "{{$\\Downarrow$}}" }, // downwards double arrow, U+21D3
                                                     // ISOamsa
            ...
            { "8249", "lsaquo", "{\\guilsinglleft}" }, // single left-pointing angle
                                                       // quotation mark,
            // U+2039 ISO proposed
            /* lsaquo is proposed but not yet ISO standardized */
            { "8250", "rsaquo", "{\\guilsinglright}" }, // single right-pointing angle
                                                        // quotation mark,
            // U+203A ISO proposed
            ...
            { "35", "", "\\#" }, // Hash
            { "36", "dollar", "\\$" }, // Dollar
            { "37", "#37", "\\%" }, // Percent (&percnt; is not displayed correctly in the
                                    // entry preview)
            { "39", "apos", "'" }, // Apostrophe
            ...
            { "536", "", "{{\\cb{S}}}" }, // capital letter S with comma below, require
                                          // combelow
            { "537", "", "{\\cb{s}}" }, // small letter S with comma below, require
                                        // combelow
            { "538", "", "{{\\cb{T}}}" }, // capital letter T with comma below, require
                                          // combelow
            { "539", "", "{\\cb{t}}" }, // small letter T with comma below, require
                                        // combelow
            ...
            { "8251", "", "{\\textreferencemark}" }, { "8253", "", "{\\textinterrobang}" }, { "8320", "", "$_{0}$" }, // sub-script
                                                                                                                      // 0
            ...
            { "8333", "", "$_{(}$" }, // sub-script (
            { "8334", "", "$_{)}$" }, // sub-script )
            { "8450", "complexes", "$\\mathbb{C}$" }, // double struck capital C --
                                                      // requires e.g. amsfonts
            { "8451", "", "{\\textcelsius}" }, // Degree Celsius
            { "8459", "Hscr", "{{$\\mathcal{H}$}}" }, // script capital H -- possibly use
                                                      // \mathscr
            { "8460", "Hfr", "{{$\\mathbb{H}$}}" }, // black letter capital H -- requires
                                                    // e.g. amsfonts
            { "8466", "Lscr", "{{$\\mathcal{L}$}}" }, // script capital L -- possibly use
                                                      // \mathscr
            { "8467", "ell", "{$\\ell$}" }, // script small l
            { "8469", "naturals", "{{$\\mathbb{N}$}}" }, // double struck capital N --
                                                         // requires e.g. amsfonts
            { "8474", "Qopf", "{{$\\mathbb{Q}$}}" }, // double struck capital Q --
                                                     // requires e.g. amsfonts
            { "8477", "reals", "{{$\\mathbb{R}$}}" }, // double struck capital R --
                                                      // requires e.g. amsfonts
            { "8486", "", "${{\\Omega}}$" }, // Omega
            ...
            { "8710", "", "$\\triangle$" }, // Increment - could use a more appropriate
                                            // symbol
            ...
            { "9653", "utri", "$\\triangle$" }, // White up-pointing small triangle --
                                                // \vartriangle probably
            // better but requires amssymb
            { "10877", "les", "$\\leqslant$" }, // Less than slanted equal -- requires
                                                // amssymb
            { "10878", "ges", "$\\geqslant$" }, // Less than slanted equal -- requires
                                                // amssymb
            ...
            { "119978", "Oscr", "$\\mathcal{O}$" }, // script capital O -- possibly use
                                                    // \mathscr
            { "119984", "Uscr", "$\\mathcal{U}$" }, // script capital U -- possibly use
                                                    // \mathscr
            { "120598", "", "$\\epsilon$" }, // mathematical italic epsilon U+1D716 --
                                             // requires amsmath
    ...
    static {
        ...
        // Manual corrections
        LATEX_HTML_CONVERSION_MAP.put("AA", "&Aring;"); // Overwritten by &angst; which is
                                                        // less supported

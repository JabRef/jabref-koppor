...
public class MetaDataSerializer {
    ...
    public static Map<String, String> getSerializedStringMap(
        ...
        stringyMetaData.putAll(
            serializeCiteKeyPatterns(metaData, globalCiteKeyPatterns)
        );
        metaData
            .getMode()
            .ifPresent(mode ->
        ...
        metaData
            ...
            .ifPresent(version ->
                serializedMetaData.put(
                    MetaData.GROUPS_SEARCH_SYNTAX_VERSION,
                    version.toString()
                )
            );
    ...
    private static Map<String, String> serializeMetaData(
        ...
        > metaItem : stringyMetaData.entrySet()) {
            ...
            boolean isSaveActions = MetaData.SAVE_ACTIONS.equals(
                metaItem.getKey()
            );
            // The last "MetaData.SEPARATOR_STRING" adds compatibility to JabRef v5.9 and earlier
    ...
    private static Map<String, List<String>> serializeCiteKeyPatterns(
        ...
        for (EntryType key : citationKeyPattern.getAllKeys()) {
            if (!citationKeyPattern.isDefaultValue(key)) {
                ...
                data.add(
                    citationKeyPattern.getValue(key).stringRepresentation()
                );
        ...
        if (
            ...
            List<String> data = new ArrayList<>();
            data.add(
                citationKeyPattern.getDefaultValue().stringRepresentation()
            );

...
public class DatabaseMerger {
    ...
    /**
     * Merges all entries and strings of the other database into the target database. Any
     * duplicates are ignored. In case a string has a different content, it is added with
     * a new unique name. The unique name is generated by suffix "_i", where i runs from 1
     * onwards.
    ...
    /**
     * Merges all entries, strings, and metaData of the other database context into the
     * target database context. Any duplicates are ignored. In case a string has a
     * different content, it is added with a new unique name. The unique name is generated
     * by suffix "_i", where i runs from 1 onwards.
     * @param other The other databases that is merged into this database
     */
    ...
    private void mergeEntries(BibDatabase target, BibDatabase other) {
        ...
        List<BibEntry> newEntries = other.getEntries()
            .stream()
            // Remove all entries that are already part of the database (duplicate)
            .filter(entry -> duplicateCheck.containsDuplicate(target, entry, BibDatabaseModeDetection.inferMode(target))
                .isEmpty())
        ...
        for (BibtexString bibtexString : other.getStringValues()) {
            ...
            if (target.hasStringByName(bibtexStringName)) {
                ...
                if (!importedContent.equals(existingContent)) {
                    LOGGER.info("String contents differ for {}: {} != {}", bibtexStringName, importedContent,
                            existingContent);
                    int suffix = 1;
     ...
     * @param otherFilename the filename of the other library. Pass "unknown" if not
     * known.
     */
    ...
    private void mergeGroups(MetaData target, MetaData other, String otherFilename, List<BibEntry> allOtherEntries) {
        ...
        // type AllEntriesGroup, since every tree has exactly one AllEntriesGroup (its
        // root). The <b>newGroups</b> are
        ...
        other.getGroups().ifPresent(newGroups -> {
            ...
            target.getGroups()
                .ifPresentOrElse(newGroups::moveTo,
                        // target does not contain any groups, so we can just use the new
                        // groups

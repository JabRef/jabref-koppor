...
class SearchQuerySQLConversionTest {
    ...
    @ParameterizedTest
    @MethodSource
    void searchConversion(String searchExpression, String expected)
        throws SQLException {
        try (Connection connection = pg.getPostgresDatabase().getConnection()) {
            SqlQueryNode sqlQueryNode = SearchQueryConversion.searchToSql(
                "tableName",
                new SearchQuery(searchExpression)
            );
            try (
                PreparedStatement preparedStatement =
                    connection.prepareStatement(sqlQueryNode.cte())
            ) {
                for (int i = 0; i < sqlQueryNode.params().size(); i++) {
                    preparedStatement.setString(
                        i + 1,
                        sqlQueryNode.params().get(i)
                    );
                }
    ...
    @ParameterizedTest
    @MethodSource
    void unFieldedTermsWithSearchBarFlags(

...
public class ConferenceUtilsTest {
    ...
    void acronymExtraction(String input, String expectedResult) {
        assertEquals(
            ...
            extractStringFromParentheses(input)
        );
    }

    @ParameterizedTest(
        name = "RemoveParentheses for \"{0}\" should return \"{1}\""
    )
    @CsvSource(
        {
            "'', ''", // Empty string
    ...
    @ParameterizedTest(
        name = "Acronym candidates for \"{0}\" with cutoff at \"{1}\" should return \"{2}\""
    )
    @MethodSource("generateAcronymCandidateTestData")
    void acronymCandidateGeneration(
        String input,
        int cutoff,
    ...
    static Stream<Arguments> generateAcronymCandidateTestData() {
        return Stream.of(
            ...
            Arguments.of(
                "ACRO+NYM@CONF#2018",
                20,
                Set.of("ACRO+NYM@CONF#2018")
            ), // Does not split non-delimiters
            ...
            Arguments.of(
                "_,-:.ACRO_-NYM.:-,_",
                10,
                Set.of("ACRO_-NYM", "ACRO", "NYM")
            ), // trimming keeps delimiters between acronym but strips other subsets
            Arguments.of(
                "a.b_c:d",
                3,
                Set.of("a.b", "b_c", "c:d", "a", "b", "c", "d")
            ), // interleaved delimiters between valid characters preserves middle-delimiters in subsets
            // Subset generation with real-world examples
            Arguments.of(
                "CLOSER'2022",
                11,
                Set.of("CLOSER'2022", "CLOSER", "2022")
            ), // keeps original string at the head if below cutoff
            Arguments.of("CLOSER'2022", 6, Set.of("CLOSER", "2022")), // discards original string if above cutoff
            Arguments.of(
                "ACM_WiSec'2022",
                11,
                Set.of("WiSec'2022", "ACM_WiSec", "WiSec", "2022", "ACM")
            ), // keeps longer candidates as well
            Arguments.of(
                "IEEE-IV'2022",
                11,
                Set.of("IEEE-IV", "IV'2022", "2022", "IEEE", "IV")
            ) // composite acronyms are pushed ahead for better matching
        );
    }

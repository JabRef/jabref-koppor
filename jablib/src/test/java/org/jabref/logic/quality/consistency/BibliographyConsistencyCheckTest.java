...
class BibliographyConsistencyCheckTest {
    ...
    void checkSimpleLibrary(@TempDir Path tempDir) {
        ...
        BibliographyConsistencyCheck.Result result =
            new BibliographyConsistencyCheck().check(
                bibContext,
                (count, total) -> {}
            );
    ...
    void checkDifferentOutputSymbols(@TempDir Path tempDir) {
        ...
        bibContext.setMode(BibDatabaseMode.BIBTEX);
        BibliographyConsistencyCheck.Result result =
            new BibliographyConsistencyCheck().check(bibContext, (_, _) -> {});
    ...
    void nonFilteredFieldDifferenceIsReported() {
        BibEntry withAuthor = new BibEntry(
        ...
        BibDatabase bibDatabase = new BibDatabase(
            List.of(withAuthor, withoutAuthor)
        );
        BibDatabaseContext bibContext = new BibDatabaseContext(bibDatabase);
        ...
        BibliographyConsistencyCheck.EntryTypeResult typeResult = result
    ...
    void unsetRequriedFieldsReported() {
        ...
        assertEquals(
            List.of(withDate, withoutDate),
            typeResult.sortedEntries().stream().toList()
        );
    }
    ...
    void checkFieldEntriesWithFieldDifferences() {
        ...
        List<BibEntry> result =
            new BibliographyConsistencyCheck().filterAndSortEntriesWithFieldDifferences(
    ...
    void checkComplexLibraryWithAdditionalEntry(@TempDir Path tempDir) {
        ...
        BibliographyConsistencyCheck.Result actualResult =
            new BibliographyConsistencyCheck().check(bibContext, (_, _) -> {});
        ...
        BibliographyConsistencyCheck.EntryTypeResult expectedInProceedings =
            new BibliographyConsistencyCheck.EntryTypeResult(
                Set.of(
                    StandardField.PAGES,
                    StandardField.PUBLISHER,
                ...
                List.of(fifth, fourth, sixth, third)
            );
        BibliographyConsistencyCheck.Result expectedResult =
            new BibliographyConsistencyCheck.Result(
                Map.of(
                    StandardEntryType.Article,
                    articleResult,
                    StandardEntryType.InProceedings,
                    expectedInProceedings
                )
            );

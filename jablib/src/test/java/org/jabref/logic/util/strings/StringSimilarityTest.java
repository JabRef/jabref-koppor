...
class StringSimilarityTest {
    ...
    @ParameterizedTest(
        name = "\"{0}\" should match \"{1}\" with similarity rating of \"{2}\"."
    )
    @CsvSource(
        {
    ...
    void stringSimilarity(String a, String b, String expectedResult) {
        assertEquals(
            Double.parseDouble(expectedResult),
            similarityChecker.similarity(a, b),
            EPSILON_SIMILARITY
        );
    }

    @ParameterizedTest(
        name = "\"{0}\" should match \"{1}\" with LCS similarity rating of \"{2}\"."
    )
    @CsvSource(
        {
    ...
    void LCSSimilarity(String a, String b, String expectedResult) {
        assertEquals(
            Double.parseDouble(expectedResult),
            StringSimilarity.LCSSimilarity(a, b),
            EPSILON_SIMILARITY
        );
    }
}

...
class MakeLabelWithoutDatabaseTest {
    ...
    @BeforeEach
    void setUp() {
        GlobalCitationKeyPatterns keyPattern = new GlobalCitationKeyPatterns(
                CitationKeyPattern.NULL_CITATION_KEY_PATTERN);
        keyPattern.setDefaultValue("[auth]");
        patternPreferences = new CitationKeyPatternPreferences(false, false, false,
                CitationKeyPatternPreferences.KeySuffix.SECOND_WITH_A, "", "",
                CitationKeyGenerator.DEFAULT_UNWANTED_CHARACTERS, keyPattern, "", new SimpleObjectProperty<>(','));
    ...
    void makeAuthorLabelForFileSearch() {
        BibEntry entry = new BibEntry().withField(StandardField.AUTHOR, "John Doe")
    ...
    void bamford_1972_Comprehensive_Reaction_V_7_EN() {
        // Example taken from
        // https://github.com/JabRef/jabref/issues/11367#issuecomment-2162250948
        ...
        citationKeyGenerator = new CitationKeyGenerator(GlobalCitationKeyPatterns.fromPattern(
                "[edtr]_[YEAR]_[MAINTITLE:regex(\"(\\w+).*\", \"$1\")]_[TITLE:regex(\"(\\w+).*\", \"$1\")]_V_[VOLUME]_[LANGUAGE:regex(\"english\", \"EN\"):regex(\"french\", \"FR\")]"),
                new BibDatabase(), patternPreferences);
        String label = citationKeyGenerator.generateKey(entry);
    ...
    @Test
    void frenchRegEx() {
        BibEntry entry = new BibEntry(StandardEntryType.Book).withField(StandardField.LANGUAGE, "french");
        citationKeyGenerator = new CitationKeyGenerator(
                GlobalCitationKeyPatterns
                    .fromPattern("[LANGUAGE:regex(\"english\", \"EN\"):regex(\"french\", \"FR\")]"),
                new BibDatabase(), patternPreferences);
        String label = citationKeyGenerator.generateKey(entry);

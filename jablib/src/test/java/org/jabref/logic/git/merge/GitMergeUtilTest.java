...
class GitMergeUtilTest {
    ...
    void replaceEntriesReplacesMatchingByCitationKey() {
        ...
        BibDatabaseContext result = GitMergeUtil.replaceEntries(
            remoteContext,
            List.of(resolvedA)
        );

        List<BibEntry> finalEntries = result.getDatabase().getEntries();
        BibEntry resultA = finalEntries
            .stream()
            .filter(e -> "keyA".equals(e.getCitationKey().orElse("")))
            .findFirst()
            .orElseThrow();
        BibEntry resultB = finalEntries
            .stream()
        ...
        assertEquals(
            "resolved author A",
            resultA.getField(StandardField.AUTHOR).orElse("")
        );
        assertEquals(
            "original author B",
            resultB.getField(StandardField.AUTHOR).orElse("")
        );
    }
    ...
    void replaceEntriesIgnoresResolvedWithoutCitationKey() {
        ...
        BibEntry resolved = new BibEntry(StandardEntryType.Article).withField(
            StandardField.TITLE,
            "New Title"
        );

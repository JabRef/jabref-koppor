...
/**
 * Test Interface for all transformers that use infix notation for their logical binary
 * operators
 */
public abstract class InfixTransformerTest<T extends AbstractQueryTransformer> {
    ...
    /*
     * All prefixes have to include the used separator Example in the case of ':':
     * <code>"author:"</code>
     */
    ...
    public void notOperatorPrefix() throws QueryNodeParseException {
        ...
        QueryNode luceneQuery = new StandardSyntaxParser().parse(queryString,
                AbstractQueryTransformer.NO_EXPLICIT_FIELD);
        Optional<String> searchQuery = getTransformer().transformLuceneQuery(luceneQuery);
        Optional<String> expected = Optional
            .of(getTransformer().getLogicalNotOperator() + "(" + getAuthorPrefix() + "\"Igor Steinmacher\""
                    + getTransformer().getLogicalOrOperator() + getAuthorPrefix() + "\"Christoph Treude\")");
        assertEquals(expected, searchQuery);
    }

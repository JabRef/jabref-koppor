...
public class MetaDataParserTest {
    ...
    void parsesUserSpecificBlgPathSuccessfully() throws ParseException {
        ...
        assertEquals(
            Optional.of(Path.of("/home/user/test.blg")),
            parsed.getBlgFilePath(user)
        );
    }

    @Test
    void parsesLatexFileDirectoryForUserHostSuccessfully()
        ...
        assertEquals(
            Optional.of(Path.of("/home/user/latex")),
            parsed.getLatexFileDirectory(userHost)
        );
    }

    @Test
    void parsesMultipleLatexFileDirectoriesSuccessfully()
        throws ParseException {
        String userHost1 = "user1-host1";
        String userHost2 = "user2-host2";

        Map<String, String> data = Map.of(
            "fileDirectoryLatex-" + userHost1,
            "/path/for/host1;",
            "fileDirectoryLatex-" + userHost2,
            "/path/for/host2;"
        );
        ...
        assertEquals(
            Optional.of(Path.of("/path/for/host1")),
            parsed.getLatexFileDirectory(userHost1)
        );
        assertEquals(
            Optional.of(Path.of("/path/for/host2")),
            parsed.getLatexFileDirectory(userHost2)
        );
    }

    @Test
    void retrievesLatexFileDirectoryForDifferentUserOnSameHost()
        throws ParseException {
        String originalUserHost = "user1-host";
        String newUserHost = "user2-host";

        Map<String, String> data = Map.of(
            "fileDirectoryLatex-" + originalUserHost,
            "/path/to/latex;"
        );
        ...
        assertEquals(
            Optional.of(Path.of("/path/to/latex")),
            parsed.getLatexFileDirectory(newUserHost)
        );
    }

    @Test
    void parsesWindowsPathsInLatexFileDirectoryCorrectly()
        throws ParseException {
        String userHost = "user-host";
        String rawKey = "fileDirectoryLatex-" + userHost;
        ...
        assertEquals(
            Optional.of(Path.of("C:\\Path\\To\\Latex")),
            parsed.getLatexFileDirectory(userHost)
        );
    }
}

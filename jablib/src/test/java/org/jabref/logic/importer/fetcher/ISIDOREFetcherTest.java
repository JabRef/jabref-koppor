...
class ISIDOREFetcherTest {
    ...
    void checkArticle1() throws FetcherException {
        BibEntry expected = new BibEntry(StandardEntryType.Article).withField(StandardField.TITLE,
        ...
        List<BibEntry> actual = fetcher.performSearch(
                "Investigating day-to-day variability of transit usage on a multimonth scale with smart card data. A case study in Lyon");
    ...
    void checkThesis() throws FetcherException {
        BibEntry expected = new BibEntry(StandardEntryType.Thesis).withField(StandardField.TITLE,
        ...
        // The results obtained by the fetcher contain three entries (as of 2024), and the
        // first matches our expected entry. So, we can use actual.getFirst(), but shall
        // still use findFirst() in case the order of returned items changes.
        Optional<BibEntry> matchingEntry = actual.stream().filter(entry -> entry.equals(expected)).findFirst();
    ...
    void checkArticle3() throws FetcherException {
        ...
        List<BibEntry> actual = fetcher
            .performSearch("Salvage Lymph Node Dissection for Nodal Recurrent Prostate Cancer: A Systematic Review.");
    ...
    void author() throws FetcherException {
        ...
        assertEquals(List.of(new BibEntry(StandardEntryType.Article)
            ...
            .withField(StandardField.YEAR, "1990")), actual);
    }

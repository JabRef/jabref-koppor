...
class IEEETest
    ...
    private static final Optional<String> API_KEY = Optional.of(
        new BuildInfo().ieeeAPIKey
    );

    private static final BibEntry IGOR_NEWCOMERS = new BibEntry(
    ...
    static void ensureIeeeIsAvailable() throws FetcherException {
        ...
        when(importerPreferences.getApiKey(IEEE.FETCHER_NAME)).thenReturn(
            API_KEY
        );
        IEEE ieee = new IEEE(importFormatPreferences, importerPreferences);

        assumeFalse(
            List.of().equals(ieee.performSearch("article_number:8801912"))
        );
    }

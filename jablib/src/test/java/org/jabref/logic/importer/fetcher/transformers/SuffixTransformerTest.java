...
/**
 * Test Interface for all transformers that use suffix notation for their logical binary
 * operators
 */
public abstract class SuffixTransformerTest<T extends AbstractQueryTransformer> {
    ...
    public void notOperatorSufix() throws QueryNodeParseException {
        ...
        Optional<String> expected = Optional
            .of(getTransformer().getLogicalNotOperator() + "(" + "\"Igor Steinmacher\"" + getAuthorSuffix()
                    + getTransformer().getLogicalOrOperator() + "\"Christoph Treude\")" + getAuthorSuffix());
        assertEquals(expected, searchQuery);
    }

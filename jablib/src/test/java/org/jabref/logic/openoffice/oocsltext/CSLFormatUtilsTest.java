...
class CSLFormatUtilsTest {
    ...
    private static final BibDatabaseContext TEST_ENTRY_CONTEXT = new BibDatabaseContext(
    ...
    static Stream<Arguments> ooHTMLTransformFromRawHTML() {
        return Stream.of(
                ...
                Arguments.of("Text with non-breaking spaces", "Text with&nbsp;non-breaking&nbsp;spaces"),
                ...
                Arguments.of("Text with <b>bold</b>", "Text with <span style=\"font-weight:bold;\">bold</span>"),

                // Replacing span tags with inline styles for italic
                Arguments.of("Text with <i>italic</i>", "Text with <span style=\"font-style:italic;\">italic</span>"),

                // Replacing span tags with inline styles for underline
                Arguments.of("Text with <u>underline</u>",
                        "Text with <span style=\"text-decoration:underline;\">underline</span>"),

                // Replacing span tags with inline styles for small-caps
                Arguments.of("Text with <smallcaps>small caps</smallcaps>",
                        "Text with <span style=\"font-variant:small-caps;\">small caps</span>"),

                // Test case for cleaning up remaining span tags
                Arguments.of("Text with unnecessary span", "Text with <span>unnecessary span</span>"),

                // Test case combining multiple transformations
                Arguments.of("[1] <b>Author</b>, \"Title,\" <i>Journal</i>, vol. 1, no. 1, pp. 1-10, 2023.",
                ...
                // Comprehensive test
                Arguments.of(
                        "[1] <b>Smith & Jones</b>, " + "\"<i>Analysis of <code> in HTML</i>,\" "
                                + "<smallcaps>Journal of Web Development</smallcaps>, "
                                + "vol. 1, no. 1, pp. 1-10, 2023. " + "https://doi.org/10.1000/example",
    ...
    /**
     * Test to check correct transformation of raw CSL bibliography generated by
     * citeproc-java methods into OO-ready text.
     *
     * @implSpec Assumes that
     * {@link CitationStyleGenerator#generateBibliography(List, String, CitationStyleOutputFormat, BibDatabaseContext, BibEntryTypesManager)
     * generateBibliography} works as expected.
     */
    @ParameterizedTest
    @MethodSource
    void ooHTMLTransformFromRawBibliography(String expected, CitationStyle style) {
        String citation = CitationStyleGenerator
            .generateBibliography(List.of(TEST_ENTRY), style.getSource(), HTML_OUTPUT_FORMAT, TEST_ENTRY_CONTEXT,
                    ENTRY_TYPES_MANAGER)
            .getFirst();
    ...
    static Stream<Arguments> ooHTMLTransformFromRawBibliography() {
        return Stream.of(
                ...
                // Non-numeric, parentheses, commas, full stops, slashes, hyphens, colons,
                // italics
                Arguments.of(
                ...
                Arguments.of(
                        ...
                        STYLE_LIST.stream()
                            .filter(e -> "Springer - Lecture Notes in Computer Science".equals(e.getTitle()))
                            .findAny()
                            .get()),
                ...
                Arguments.of(
                        ...
                        STYLE_LIST.stream()
                            .filter(e -> "Chicago Manual of Style 17th edition (author-date)".equals(e.getTitle()))
                            .findAny()
                            .get()),
                ...
                Arguments.of(
                        ...
                        STYLE_LIST.stream()
                            .filter(e -> "American Medical Association 11th edition".equals(e.getTitle()))
                            .findAny()
                            .get()),
                ...
                Arguments.of(
                        ...
                        STYLE_LIST.stream()
                            .filter(e -> "De Montfort University - Harvard".equals(e.getTitle()))
                            .findAny()
                            .get()),
                ...
                Arguments.of(
                        ...
                        STYLE_LIST.stream()
                            .filter(e -> "Modern Language Association 7th edition (underline)".equals(e.getTitle()))
                            .findAny()
                            .get()),
                ...
                Arguments.of(
                        ...
                        STYLE_LIST.stream()
                            .filter(e -> "The Journal of Veterinary Medical Science".equals(e.getTitle()))
                            .findAny()
                            .get()),
                ...
                Arguments.of(
                        ...
                        STYLE_LIST.stream()
                            .filter(e -> "Acta Anaesthesiologica Taiwanica".equals(e.getTitle()))
                            .findAny()
                            .get()),
                ...
                Arguments.of(
                        ...
                        STYLE_LIST.stream()
                            .filter(e -> "Chemical and Pharmaceutical Bulletin".equals(e.getTitle()))
                            .findAny()
                            .get()),
                ...
                Arguments.of(
                        ...
                        STYLE_LIST.stream()
                            .filter(e -> "American Institute of Physics 4th edition".equals(e.getTitle()))
                            .findAny()
                            .get()));
    }

    /**
     * Test to check correct transformation of raw CSL citation with a single entry
     * generated by citeproc-java methods into OO-ready text.
     *
     * @implSpec Assumes that
     ...
     * generateCitation} works as expected.
     */
    @ParameterizedTest
    @MethodSource
    void ooHTMLTransformFromCitationWithSingleEntry(String expected, CitationStyle style) {
        String citation = CitationStyleGenerator.generateCitation(List.of(TEST_ENTRY), style.getSource(),
                HTML_OUTPUT_FORMAT, TEST_ENTRY_CONTEXT, ENTRY_TYPES_MANAGER);
        String actual = CSLFormatUtils.transformHTML(citation);
    ...
    static Stream<Arguments> ooHTMLTransformFromCitationWithSingleEntry() {
        return Stream.of(
                Arguments.of("(Smith et al., 2016)",
                        STYLE_LIST.stream()
                            .filter(e -> "American Psychological Association 7th edition".equals(e.getTitle()))
                            .findAny()
                            .get()),

                Arguments.of("[1]", STYLE_LIST.stream().filter(e -> "IEEE".equals(e.getTitle())).findAny().get()),
                ...
                Arguments.of("(Smith, Jones, and Williams 2016)",
                        STYLE_LIST.stream()
                            .filter(e -> "Chicago Manual of Style 17th edition (author-date)".equals(e.getTitle()))
                            .findAny()
                            .get()),

                Arguments.of("(1)", STYLE_LIST.stream().filter(e -> "Vancouver".equals(e.getTitle())).findAny().get()),

                Arguments.of("<sup>1</sup>",
                        STYLE_LIST.stream().filter(e -> "Nature".equals(e.getTitle())).findAny().get()),
                ...
                Arguments.of("(Smith, Jones, & Williams, “Title of the test entry”)",
                        STYLE_LIST.stream()
                            .filter(e -> "Modern Language Association 7th edition (underline)".equals(e.getTitle()))
                            .findAny()
                            .get()),

                Arguments.of("Smith, B., B. Jones, and J. Williams, 2016.",
                        STYLE_LIST.stream()
                            .filter(e -> "Histoire & Mesure (Français)".equals(e.getTitle()))
                            .findAny()
                            .get()),
                ...
                // Note: not sure if the right parenthesis outside the superscript is
                // correct, but that's how citeproc-java generates it in raw form as well.
                Arguments.of("<sup>1</sup>)",
                        STYLE_LIST.stream()
                            .filter(e -> "Chemical and Pharmaceutical Bulletin".equals(e.getTitle()))
                            .findAny()
    ...
    /**
     * Test to check correct transformation of raw CSL citations with multiple entries
     * generated by citeproc-java methods into OO-ready text.
     *
     * @implSpec Assumes that
     * {@link CitationStyleGenerator#generateCitation(List, String, CitationStyleOutputFormat, BibDatabaseContext, BibEntryTypesManager)
    ...
    void ooHTMLTransformFromCitationWithMultipleEntries(String expected, CitationStyle style) {
        ...
        String citation = CitationStyleGenerator.generateCitation(entries, style.getSource(), HTML_OUTPUT_FORMAT,
                context, ENTRY_TYPES_MANAGER);
        String actual = CSLFormatUtils.transformHTML(citation);
        assertEquals(expected, actual);
    }
    ...
    static Stream<Arguments> ooHTMLTransformFromCitationWithMultipleEntries() {
        return Stream.of(
                Arguments.of("(Garcia & Lee, 2021; Smith & Johnson, 2020)",
                        STYLE_LIST.stream()
                            .filter(e -> "American Psychological Association 7th edition".equals(e.getTitle()))
                            .findAny()
                            .get()),

                Arguments.of("[1], [2]", STYLE_LIST.stream().filter(e -> "IEEE".equals(e.getTitle())).findAny().get()),
                ...
                Arguments.of("(Garcia and Lee 2021; Smith and Johnson 2020)",
                        STYLE_LIST.stream()
                            .filter(e -> "Chicago Manual of Style 17th edition (author-date)".equals(e.getTitle()))
                            .findAny()
                            .get()),
                ...
                Arguments.of("Garcia, M. and D. Lee, 2021 ; Smith, J. and E. Johnson, 2020.",
                        STYLE_LIST.stream()
                            .filter(e -> "Histoire & Mesure (Français)".equals(e.getTitle()))
                            .findAny()
                            .get()),
    ...
    /**
     * Test for modifying the number (index) of a numeric citation. The numeric index
     * should change to the provided "current number". The rest of the citation should
     * stay as it is (other numbers in the body shouldn't be affected).
     *
     * @implSpec
     * <ol>
     * <li>Assumes that
     * {@link CitationStyleGenerator#generateBibliography(List, String, CitationStyleOutputFormat, BibDatabaseContext, BibEntryTypesManager)
     * generateBibliography} works as expected.</li>
     * <li>Assumes that the method {@link CSLFormatUtils#transformHTML(String)
     * transformHTML} works as expected.</li>
    ...
    /**
     * Tests if a citation (LaTeX "\cite") is converted into an in-text citation (LaTeX
     * "\citet") as expected.
     */
    @ParameterizedTest
    @MethodSource
    ...
    private static Stream<Arguments> ChangeToInText() {
        return Stream.of(
                ...
                // IEEE Style
                Arguments.of("[1]", "[1]"), Arguments.of("[2], [3]", "[2], [3]"),

                // Vancouver Style
                Arguments.of("(1)", "(1)"), Arguments.of("(1,2)", "(1,2)"),
    ...
    /**
     * Test for proper generation of alphanumeric citations (currently supported: DIN
     * 1505-2).
     *
     * @implSpec Assumes that the method
     * {@link org.jabref.logic.citationkeypattern.BracketedPattern#authorsAlpha
     * authorsAlpha} works as expected.
     * </p>
     */
    @ParameterizedTest
    @MethodSource
    ...
    private static Stream<Arguments> generateAlphanumericCitationTest() {
        BibEntry entry1 = new BibEntry(StandardEntryType.Article).withField(StandardField.AUTHOR, "Garcia, Maria")
            .withField(StandardField.TITLE, "Quantum Entanglement in Superconductors")
            .withField(StandardField.JOURNAL, "International Review of Physics")
            .withField(StandardField.VOLUME, "28")
            .withField(StandardField.NUMBER, "6")
    ...
    private static Stream<Arguments> generateAuthorPrefixTest() {
        return Stream.of(
                // Single author
                Arguments.of("Garcia ",
                        new BibEntry(StandardEntryType.Article).withField(StandardField.AUTHOR, "Garcia, Maria")
                            .withField(StandardField.YEAR, "2021")),

                // Two authors
                Arguments.of("Smith et al. ",
                        new BibEntry(StandardEntryType.Article)
                            .withField(StandardField.AUTHOR, "Smith, John and Johnson, Emily")
                            .withField(StandardField.YEAR, "2020")),

                // Three authors
                Arguments.of("Johnson et al. ",
                        new BibEntry(StandardEntryType.Article)
                ...
                Arguments.of("Smith et al. ",
                        new BibEntry(StandardEntryType.Article)
                            .withField(StandardField.AUTHOR,
                                    "Smith, John and Johnson, Emily and Lee, David and Williams, Jessica")
                            .withField(StandardField.YEAR, "2018")),
                ...
                Arguments.of("Smith et al. ",
                        new BibEntry(StandardEntryType.Article)
                            .withField(StandardField.AUTHOR,
                                    "Smith and James, John and Johnson, Emily and Lee, David and Williams, Jessica")
                            .withField(StandardField.YEAR, "2018")),

                // Missing author
                Arguments.of("",
                        new BibEntry(StandardEntryType.Article).withField(StandardField.TITLE, "No Author Paper")
                            .withField(StandardField.YEAR, "2020")));
    }

    /**
     * Test for proper generation of in-text citations for alphanumeric styles (currently
     * supported: DIN 1505-2)
     */
    @ParameterizedTest
    @MethodSource
    ...
    private static Stream<Arguments> generateAlphanumericInTextCitationTest() {
        return Stream.of(
                // Single author
                Arguments.of("Garcia [Ga21]",
                        new BibEntry(StandardEntryType.Article).withField(StandardField.AUTHOR, "Garcia, Maria")
                            .withField(StandardField.YEAR, "2021")),

                // Two authors
                Arguments.of("Smith et al. [SJ20]",
                        new BibEntry(StandardEntryType.Article)
                            .withField(StandardField.AUTHOR, "Smith, John and Johnson, Emily")
                            .withField(StandardField.YEAR, "2020")),

                // Three authors
                Arguments.of("Johnson et al. [JWL19]",
                        new BibEntry(StandardEntryType.Article)
                            .withField(StandardField.AUTHOR, "Johnson, Emily and Williams, Jessica and Lee, David")
                            .withField(StandardField.YEAR, "2019")),

                // Four or more authors
                Arguments.of("Smith et al. [SJLW18]",
                        new BibEntry(StandardEntryType.Article)
                            .withField(StandardField.AUTHOR,
                                    "Smith, John and Johnson, Emily and Lee, David and Williams, Jessica")
                            .withField(StandardField.YEAR, "2018")),
                ...
                Arguments.of("Smith et al. [SJJL18]",
                        new BibEntry(StandardEntryType.Article)
                            .withField(StandardField.AUTHOR,
                                    "Smith and James, John and Johnson, Emily and Lee, David and Williams, Jessica")
                            .withField(StandardField.YEAR, "2018")),

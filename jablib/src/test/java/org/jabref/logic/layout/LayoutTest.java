...
class LayoutTest {
    ...
    void nameFormatter() throws IOException {
        BibEntry entry = new BibEntry(StandardEntryType.Article).withField(StandardField.AUTHOR,
                "Joe Doe and Jane, Moon");
    ...
    void HTMLCharsWithDotlessIAndTiled() throws IOException {
        BibEntry entry = new BibEntry(StandardEntryType.Article).withField(StandardField.ABSTRACT,
                "\\~{n} \\~n \\'i \\i \\i");
        ...
        assertEquals("<font face=\"arial\"><BR><BR><b>Abstract: </b> &ntilde; &ntilde; &iacute; &imath; &imath;</font>",
                layoutText);
    }
    ...
    void beginConditionals() throws IOException {
        ...
        // combined (!a&&b)
        layoutText = layout("\\begin{!editor&&author}\\format[HTMLChars]{\\author}\\end{!editor&&author}"
    ...
    /**
     * Test for http://discourse.jabref.org/t/the-wrapfilelinks-formatter/172 (the example
     * in the help files)
     */
    @Test
    void wrapFileLinksExpandFile() throws IOException {
        ...
        String layoutText = layout("\\begin{file}\\format[WrapFileLinks(\\i. \\d (\\p))]{\\file}\\end{file}",
                List.of(Path.of("src/test/resources/pdfs/")), entry);

        assertEquals("1. Test file (" + Path.of("src/test/resources/pdfs/encrypted.pdf").toRealPath() + ")",
                layoutText);
    }
    ...
    void customNameFormatter() throws IOException {
        when(layoutFormatterPreferences.getNameFormatterPreferences()).thenReturn(new NameFormatterPreferences(
                List.of("DCA"), List.of("1@*@{ll}@@2@1..1@{ff}{ll}@2..2@ and {ff}{l}@@*@*@more")));
        BibEntry entry = new BibEntry(StandardEntryType.Article).withField(StandardField.AUTHOR,

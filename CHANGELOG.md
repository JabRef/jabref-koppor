...
- The main window is now laid out in docks: the side pane shows groups, web search and OpenOffice/LibreOffice as tabs on the left (drag a tab to reorder, close it with its X), and the entry editor opens on the right instead of below the entry table. Checkboxes are drawn as filled boxes without a border. TODO
